 ...
 * Returns: 0 on success, -1 on failure
 */
int md5_vector_non_fips_allow(size_t num_elem, const uint8_t *addr[],
 ...
 */
int fips186_2_prf(const uint8_t *seed, size_t seed_len, uint8_t *x,
			       size_t xlen);
 ...
 * @cypher: 8 octets (out)
 */
void des_encrypt(const uint8_t *clear, const uint8_t *key, uint8_t *cypher);
