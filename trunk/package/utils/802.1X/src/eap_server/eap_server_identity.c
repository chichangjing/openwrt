					     ...
					     uint8_t id)
{
	...
	if (req == NULL) 
    {
		LOG_ERROR("EAP-Identity: Failed to allocate memory for request");
		data->state = FAILURE;
		return NULL;
	}
    ...
    LOG_NORMAL("Build EAP-Request-Identify data");
				  ...
				  struct wpabuf *respData)
{
	...
	pos = eap_hdr_validate(EAP_VENDOR_IETF, EAP_TYPE_IDENTITY,
			       respData, &len);
	if (pos == NULL) {
		LOG_NORMAL("EAP-Identity: Invalid frame");
		return TRUE;
	}
				 ...
				 struct wpabuf *respData)
{
	...
	if (data->pick_up) {
		if (eap_identity_check(sm, data, respData)) {
			LOG_NORMAL("EAP-Identity: failed to pick "
