...
static void eapol_sm_step_cb(struct uloop_timeout *timer);
...
static void eapol_auth_tx_req(struct eapol_state_machine *sm)
{
    ...
    {
		LOG_ERROR("TxReq called, but there is no EAP request "
    ...
    {
		LOG_ERROR("EAPOL: Drop EAPOL TX to " MACSTR " while waiting for EAPOL-Start",MAC2STR(sm->addr));
		return;
	}

	sm->last_eap_id = eap_get_id(sm->eap_if->eapReqData);
	LOG_NORMAL("Sending EAP Packet (id = %d)",sm->last_eap_id);
	sm->eapol->cb.eapol_send(sm->eapol->conf.ctx, sm->sta,
				 IEEE802_1X_TYPE_EAP_PACKET,
 ...
 */
static void eapol_port_timers_tick(struct uloop_timeout *timer)
{
	struct eapol_state_machine *state = container_of(timer,struct eapol_state_machine,port_timer);

	if (state->aWhile > 0) 
    {
		state->aWhile--;
		if (state->aWhile == 0) 
			LOG_W("IEEE 802.1X: " MACSTR " - aWhile --> 0",MAC2STR(state->addr));
	}
	...
	if (state->quietWhile > 0) 
    {
		state->quietWhile--;
		if (state->quietWhile == 0) 
			LOG_W("IEEE 802.1X: " MACSTR " - quietWhile --> 0",MAC2STR(state->addr));
	}
	...
	if (state->reAuthWhen > 0) 
    {
		state->reAuthWhen--;
		if (state->reAuthWhen == 0) 
			LOG_NORMAL("IEEE 802.1X: " MACSTR " - reAuthWhen --> 0",MAC2STR(state->addr));
	}
	...
	if (state->eap_if->retransWhile > 0) 
    {
		...
		if (state->eap_if->retransWhile == 0) 
			LOG_W("IEEE 802.1X: " MACSTR " - (EAP) retransWhile --> 0",MAC2STR(state->addr));
	}

	eapol_sm_step_run(state);

    uloop_timeout_set(&state->port_timer,1000);
}
...
SM_STATE(AUTH_PAE, HELD)
{
	...
	LOG_D("authentication failed - EAP type: %d (%s)",
			   sm->eap_type_authsrv,
			   eap_server_get_name(0, sm->eap_type_authsrv));
	if (sm->eap_type_authsrv != sm->eap_type_supp) {
		LOG_D("Supplicant used different EAP type: "
...
SM_STATE(AUTH_PAE, AUTHENTICATED)
{
	...
	if (sm->flags & EAPOL_SM_PREAUTH)
		...
		extra = " (PMKSA cache)";
	LOG_D("authenticated - EAP type: %d (%s)%s",sm->eap_type_authsrv,eap_server_get_name(0, sm->eap_type_authsrv),extra);
}
...
static void eapol_auth_initialize(struct eapol_state_machine *sm)
{
    ...
    sm->port_timer.cb = eapol_port_timers_tick;
    uloop_timeout_set(&sm->port_timer,1000);
}
...
eapol_auth_alloc(struct eapol_authenticator *eapol, const uint8_t *addr,void *sta_ctx)
{
	...
	sm = calloc(1,sizeof(struct eapol_state_machine));
	if (sm == NULL) 
		return NULL;
    ...
    /**<    register a callback func for step timer */
    sm->step_timer.cb = eapol_sm_step_cb;

    /**<    create EAP state machine */
    ...
    /**<    init interactive interface between EAP and EAPOL level */
	sm->eap_if = eap_get_interface(sm->eap);
    //sm->eap_if->portEnabled = TRUE;

    /**<    run all state machine in EAP and EAPOL  level */
...
void eapol_auth_free(struct eapol_state_machine *sm)
{
    ...
    uloop_timeout_cancel(&sm->port_timer);
    uloop_timeout_cancel(&sm->step_timer);
	if (sm->eap)
...
static void eapol_sm_step_run(struct eapol_state_machine *sm)
{
	...
	unsigned int prev_auth_pae, prev_be_auth, prev_reauth_timer,prev_ctrl_dir;
	int max_steps = 100;
...
restart:
	...
	if (sm->initializing || eapol->cb.sta_entry_alive(eapol->conf.ctx, addr))
		SM_STEP_RUN(BE_AUTH);
	if (sm->reAuthEnabled && (sm->initializing || eapol->cb.sta_entry_alive(eapol->conf.ctx, addr)))
	...
	if (eapol->cb.sta_entry_alive(eapol->conf.ctx, addr) && sm->eap) 
    {
		...
		if (sm->eap_if->aaaEapResp) 
        {
            ...
            {
				LOG_ERROR("EAPOL: aaaEapResp set, but no aaaEapRespData available");
				return;
			}
...
static void eapol_sm_step_cb(struct uloop_timeout *timer)
{
	struct eapol_state_machine *state = container_of(timer,struct eapol_state_machine,step_timer);
	eapol_sm_step_run(state);
}
...
void eapol_auth_step(struct eapol_state_machine *sm)
{
    ...
    uloop_timeout_set(&sm->step_timer,0);
}


static int eapol_sm_get_eap_user(void *ctx, const uint8_t *identity,
...
/* a manage structor for eapol create ,
 * then set configuration and callback func
 * */
					     ...
					     struct eapol_auth_cb *cb)
{
	...
	eapol = calloc(1,sizeof(struct eapol_authenticator));
	if (eapol == NULL)
		return NULL;

	memcpy(&eapol->conf, conf,sizeof(struct eapol_auth_config));
...
/* free a manage structor for eapol
 */
