...
int radius_msg_add_eap(struct radius_msg *msg, const uint8_t *data,
		       size_t data_len);
uint8_t *radius_msg_get_eap(struct radius_msg *msg, size_t *len);
...
static inline int radius_msg_add_attr_int32(struct radius_msg *msg, uint8_t type,
					    uint32_t value)
{
	uint32_t val = htonl(value);
	return radius_msg_add_attr(msg, type, (uint8_t *) &val, 4) != NULL;
}

static inline int radius_msg_get_attr_int32(struct radius_msg *msg, uint8_t type,
					    uint32_t *value)
{
	uint32_t val;
	int res;
	res = radius_msg_get_attr(msg, type, (uint8_t *) &val, 4);
	if (res != 4)
		return -1;
