...
#ifdef KERNEL_MODULE
#include "sal/os/linux_kernel/aos_mem_pvt.h"
#else
 ...
 */
static inline void *aos_mem_alloc(aos_size_t size)
{
 ...
 * @param[in] size    buffer size
 */
static inline void aos_mem_free(void *buf)
{
    __aos_mem_free(buf);
}
 ...
 * @param[in] size    buffer size
 */
static inline void aos_mem_copy(void *dst, void *src, aos_size_t size)
{
 ...
 * @param[in] size  buffer size
 */
static inline void aos_mem_set(void *buf, a_uint8_t b, aos_size_t size)
{
 ...
 * @param[in] size  buffer size
 */
static inline void aos_mem_zero(void *buf, aos_size_t size)
{
 ...
 * @retval    1     not equal
 */
static inline int aos_mem_cmp(void *buf1, void *buf2, aos_size_t size)
{
