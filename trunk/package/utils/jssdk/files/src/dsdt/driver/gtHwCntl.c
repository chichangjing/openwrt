...
)
{
    ...
    *data = tmpData;
	...
	//printf("Read from phy(%d) register: regAddr 0x%x, ",portNum,regAddr);
	//printf("fOff %d, fLen %d, data 0x%04x.\n",fieldOffset,fieldLength,*data);
...
)
{
	...
	if(data == 0) { /* disable PPU */
		for(retryCount = 0x100; retryCount > 0; retryCount--) {
			...
			if((ppuState & 0x8000) == 0x8000)
				break;
		}

		if (retryCount == 0) {
			DBG_INFO(("Failed.\n"));
			printf("retryCount Failed. ppuState = %04x\n",ppuState);
			return GT_FAIL;
		}
	}
#endif
...
)
{
    ...
    DBG_INFO(("Read from phy(%d) register: phyAddr 0x%x, regAddr 0x%x, data=0x%04x ", portNum,phyAddr,regAddr, *data));
...
)
{
	...
	if(IS_IN_DEV_GROUP(dev,DEV_EXTERNAL_PHY))
	{
		...
		if(orgPPU)
		{
			...
			if((retPPU=hwWritePPU(dev, 0)) != GT_OK)
			{
				printf("00 retPPU Error: retPPU = %d\n",retPPU);
				return retPPU;
			}
		}
	}
