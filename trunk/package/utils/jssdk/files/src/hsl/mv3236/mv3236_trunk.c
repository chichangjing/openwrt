...
static a_uint32_t Pag_Mode = FAL_TRUNK_MODE_STATIC;
...
sw_error_t trunk_add_number(fal_trunk_id_t trunk_id, fal_port_t lport)
{
	...
	CPSS_TRUNK_MEMBER_STC trunk_member;

	SW_RTN_ON_ERROR(map_lport2hport(lport, &hport));
	
	/*FIXME. wrlDxChTrunkMemberAddingConditionChecking not consider right now*/
	trunk_member.port = hport;
	trunk_member.hwDevice = DEVICENUM;
	if((status = cpssDxChTrunkMemberAdd(DEVICENUM, trunk_id, &trunk_member)) != GT_OK) {
		printf("cpssDxChTrunkMemberAdd error\n");
		return SW_FAIL;
	}
...
sw_error_t trunk_delete_number(fal_trunk_id_t trunk_id, fal_port_t lport)
{
	...
	if((status = cpssDxChTrunkMemberRemove(DEVICENUM, trunk_id, &trunk_member)) != GT_OK) {
		printf("cpssDxChTrunkMemberAdd error\n");
		return SW_FAIL;
	}
...
sw_error_t trunk_link_status_change(fal_port_t lport, fal_port_link_status_t link_status)
{
	...
	switch(link_status) {
		...
		ret = trunk_delete_number(trunk_id, lport);
		break;

		default: /* LINK_UP */
		ret = trunk_add_number(trunk_id, lport);
		break;
	}
...
static inline sw_error_t _mv3236_trunk_pag_mode_set(a_uint32_t dev_id,fal_trunk_mode_t pag_mode)
{
    SW_PRINTFL(DETAIL,"====enter====\n");
	a_uint32_t i, j;
	a_uint32_t trunk_id;
		...
		case FAL_TRUNK_MODE_DISABLE:
			...
			for(i=1; i<=MAX_TRUNK_GROUP_ID; i++) {
				...
				for(j=1; j<11; j++) {
					...
					lport = j;
					SW_RTN_ON_ERROR(map_lport2hport(lport, &hport));
					trunk_member.hwDevice = dev_id;
					trunk_member.port = hport;
					if((ret = cpssDxChTrunkMemberRemove(dev_id, trunk_id, &trunk_member)) != GT_OK) {
						printf("gprtSetTrunkPort error");
						return SW_FAIL;
					}	
				}
...
static inline sw_error_t _mv3236_trunk_pag_mode_get(a_uint32_t dev_id,fal_trunk_mode_t * pag_mode)
{
    SW_PRINTFL(DETAIL,"====enter====\n");
	*pag_mode = Pag_Mode ;
	return SW_OK;
...
static inline sw_error_t _mv3236_trunk_load_balance_set(a_uint32_t dev_id, fal_trunk_load_balance_t mode)
{
    SW_PRINTFL(DETAIL,"====enter====\n");
	GT_STATUS ret;

	if(Pag_Mode == FAL_TRUNK_MODE_STATIC) {
		switch(mode) {
			/* FIXME. this enum values art still need verified*/
			case FAL_TRUNK_LOAD_BALANCE_XOR:		
				if((ret = cpssDxChTrunkHashDesignatedTableModeSet(dev_id, CPSS_DXCH_TRUNK_DESIGNATED_TABLE_USE_INGRESS_HASH_E)) != GT_OK) {
					return SW_FAIL;
				}			
				break;
				
			case FAL_TRUNK_LOAD_BALANCE_HASH:
				if((ret = cpssDxChTrunkHashDesignatedTableModeSet(dev_id, CPSS_DXCH_TRUNK_DESIGNATED_TABLE_USE_SOURCE_INFO_E)) != GT_OK) {
					return SW_FAIL;
				}
				break;
...
static inline sw_error_t _mv3236_trunk_load_balance_get(a_uint32_t dev_id, fal_trunk_load_balance_t *mode)
{
    SW_PRINTFL(DETAIL,"====enter====\n");
	GT_STATUS ret;
	CPSS_DXCH_TRUNK_DESIGNATED_TABLE_MODE_ENT	load_balance;

	if(Pag_Mode == FAL_TRUNK_MODE_STATIC) {
		if((ret = cpssDxChTrunkHashDesignatedTableModeGet(dev_id, &load_balance)) != GT_OK) {
			return SW_FAIL;
		}
		SW_PRINTFL(DETAIL,"======== load_balance is %d\n",load_balance);
		/* load_balance == GT_TRUE,HASH, else xor */
		if(load_balance == CPSS_DXCH_TRUNK_DESIGNATED_TABLE_USE_SOURCE_INFO_E) {
...
static inline sw_error_t _mv3236_trunk_id_set(a_uint32_t dev_id, fal_trunk_id_t trunk_id)
{
	SW_PRINTFL(DETAIL,"====enter====\n");
	return SW_OK;
...
static inline sw_error_t _mv3236_trunk_members_set(a_uint32_t dev_id, fal_trunk_members_t *trunk_members)
{
	SW_PRINTFL(DETAIL,"====enter====\n");
	return SW_OK;
...
static inline sw_error_t _mv3236_trunk_row_create(a_uint32_t dev_id,fal_trunk_id_t trunk_id,fal_trunk_members_t *trunk_members,a_uint32_t member_size)
{
    SW_PRINTFL(DETAIL,"====enter====\n");
	GT_STATUS ret;
	...
	if(Pag_Mode == FAL_TRUNK_MODE_STATIC) {
		...
		for(i=0; i<MV3236_MAX_PORT_NUM_TMP-1; i++) {
			if((lport_vec >> i) & 0x01) {
				CPSS_TRUNK_MEMBER_STC trunk_member;
			    SW_PRINTFL(DETAIL,"========\n");
				lport = (i+1);
				...
				trunk_member.hwDevice = dev_id;
				trunk_member.port = hport_id;
				if((ret = cpssDxChTrunkMemberRemove(dev_id, trunk_id, &trunk_member)) != GT_OK) {
					printf("cpssDxChTrunkMemberRemove error\n");
					return SW_FAIL;
				}
		...
		for(i=0; i<MV3236_MAX_PORT_NUM_TMP-1; i++) {
			if((tru_members >> i) & 0x01) {
				...
				printf("    lport_%d , hport_id %d , trunk_id %d \n",lport, hport_id, trunk_id);
				...
				trunk_member.hwDevice = hwDevNum;
				trunk_member.port = hport_id;
				if((ret = cpssDxChTrunkMemberAdd(dev_id, trunk_id, &trunk_member)) != GT_OK) {
					printf("cpssDxChTrunkMemberAdd error\n");
					return SW_FAIL;
				}
...
static inline sw_error_t _mv3236_trunk_row_destroy(a_uint32_t dev_id,fal_trunk_id_t trunk_id)
{
    SW_PRINTFL(DETAIL,"====enter====\n");
	GT_STATUS ret;
	a_uint32_t i, index=0;
...
sw_error_t mv3236_trunk_pag_mode_set(a_uint32_t dev_id,fal_trunk_mode_t pag_mode)
{
    SW_PRINTFL(DETAIL,"====enter====\n");
    sw_error_t rv;

    HSL_API_LOCK;
    rv = _mv3236_trunk_pag_mode_set(dev_id, pag_mode);
    HSL_API_UNLOCK;
	
    return rv;
}

sw_error_t mv3236_trunk_pag_mode_get(a_uint32_t dev_id,fal_trunk_mode_t * pag_mode)
{
    SW_PRINTFL(DETAIL,"====enter====\n");
    sw_error_t rv;

    HSL_API_LOCK;
    rv = _mv3236_trunk_pag_mode_get(dev_id, pag_mode);
    HSL_API_UNLOCK;
	
    return rv;
}

sw_error_t mv3236_trunk_load_balance_set(a_uint32_t dev_id, fal_trunk_load_balance_t mode)
{
    SW_PRINTFL(DETAIL,"====enter====\n");
    sw_error_t rv;

    HSL_API_LOCK;
    rv = _mv3236_trunk_load_balance_set(dev_id, mode);
    HSL_API_UNLOCK;
	
    return rv;
}

sw_error_t mv3236_trunk_load_balance_get(a_uint32_t dev_id, fal_trunk_load_balance_t *mode)
{
    SW_PRINTFL(DETAIL,"====enter====\n");
    sw_error_t rv;

    HSL_API_LOCK;
    rv = _mv3236_trunk_load_balance_get(dev_id, mode);
    HSL_API_UNLOCK;
...
sw_error_t mv3236_trunk_id_set(a_uint32_t dev_id, fal_trunk_id_t trunk_id)
{
    SW_PRINTFL(DETAIL,"====enter====\n");
    sw_error_t rv;

    HSL_API_LOCK;
    rv = _mv3236_trunk_id_set(dev_id, trunk_id);
    HSL_API_UNLOCK;
...
sw_error_t mv3236_trunk_members_set(a_uint32_t dev_id, fal_trunk_members_t *trunk_members)
{
    SW_PRINTFL(DETAIL,"====enter====\n");
    sw_error_t rv;

    HSL_API_LOCK;
    rv = _mv3236_trunk_members_set(dev_id, trunk_members);
    HSL_API_UNLOCK;
...
sw_error_t mv3236_trunk_row_create(a_uint32_t dev_id, fal_trunk_id_t trunk_id, fal_trunk_members_t *trunk_members, a_uint32_t member_size)
{  
    SW_PRINTFL(DETAIL,"====enter====\n");
    sw_error_t rv;

    HSL_API_LOCK;
    rv = _mv3236_trunk_row_create(dev_id, trunk_id, trunk_members, member_size);
    HSL_API_UNLOCK;
...
sw_error_t mv3236_trunk_row_destroy(a_uint32_t dev_id, fal_trunk_id_t trunk_id)
{
    SW_PRINTFL(DETAIL,"====enter====\n");
    sw_error_t rv;

    HSL_API_LOCK;
    rv = _mv3236_trunk_row_destroy(dev_id, trunk_id);
    HSL_API_UNLOCK;

	return rv;
}

sw_error_t mv3236_trunk_show_row_status(a_uint32_t dev_id, fal_trunk_id_t trunk_id)
{
    SW_PRINTFL(DETAIL,"====enter====\n");
	int i, j, k;
...
sw_error_t mv3236_port_trunk_init(a_uint32_t dev_id)
{
    SW_PRINTFL(DETAIL,"====enter====\n");
    hsl_api_t *p_api;
	hsl_dev_t *pdev = NULL;
	...
	p_api->trunk_pag_mode_set = mv3236_trunk_pag_mode_set;
	p_api->trunk_pag_mode_get = mv3236_trunk_pag_mode_get;
	p_api->trunk_load_balance_set = mv3236_trunk_load_balance_set;
	p_api->trunk_load_balance_get = mv3236_trunk_load_balance_get;
	p_api->trunk_id_set = mv3236_trunk_id_set;
	p_api->trunk_members_set = mv3236_trunk_members_set;
	p_api->trunk_row_create = mv3236_trunk_row_create;
	p_api->trunk_row_destroy = mv3236_trunk_row_destroy;
	p_api->trunk_show_row_status = mv3236_trunk_show_row_status;
