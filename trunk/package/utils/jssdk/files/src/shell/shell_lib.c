...
static char *cmd_promptp = "(jssdk-shell)# ";	/* command prompt pointer */
...
static void print_cmd_all (void) 
{
    int cmd_no = 0;
...
static void print_sub_all (int cmd_id) 
{
    ...
    for (cmd_sub_no = 0; GCMD_SUB_DESC_VALID(cmd_id, cmd_sub_no); cmd_sub_no++) {
        ...
        if (!GCMD_SUB_NAME(cmd_id, cmd_sub_no))
            continue;
        if(cmd_sub_no == 0 || strcasecmp(GCMD_SUB_NAME(cmd_id, cmd_sub_no), GCMD_SUB_NAME(cmd_id, cmd_sub_no-1))) {      
...
static int search_cmd_type(char *name, int *pmatch_id, int is_print)
{
...
static int search_cmd_sub(int cmd_id, int *pmatch_sub_id, char *sub_name, int is_print)
{
...
static int search_cmd_action(int cmd_id, int *pmatch_act_id, char *sub_name, char *action, int is_print)
{
...
static void _cmd_complete(char *matchBuf, char *fullName)
{
