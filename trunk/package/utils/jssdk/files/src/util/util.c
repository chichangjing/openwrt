...
static sll_node_t *sll_nd_new(sll_head_t * sll)
{
    sll_node_t *new_nd;
...
static void sll_nd_free(sll_head_t * sll, sll_node_t * nd)
{
    if (LL_FIX_NDNR & sll->flag) {
        nd->next = sll->free_nd;
        sll->free_nd = nd;
...
//#define SLL_DENUG
#ifdef SLL_DENUG
static void sll_nd_dump(sll_head_t * sll, const char * info)
{
    sll_node_t * curr;
...
sll_head_t *sll_creat(ll_nd_cmp cmp_func, ll_nd_dump dump_func, a_uint32_t flag, a_uint32_t nd_nr)
{
    sll_head_t *head;
    sll_node_t *node;
    a_uint32_t i, size;
...
void *sll_nd_find(const sll_head_t * sll, void *data, a_uint32_t * iterator)
{
    sll_node_t *node;
    ll_cmp_rslt_t rslt;
...
#define SW_DEBUG_MSG_BUF_SIZE 128
void SW_Debug(const char *fmt,...)
{
    char msgbuf[SW_DEBUG_MSG_BUF_SIZE] = {0};
    va_list args;
    va_start(args, fmt);
