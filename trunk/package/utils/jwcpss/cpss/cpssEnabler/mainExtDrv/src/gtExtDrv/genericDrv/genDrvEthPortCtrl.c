...
*
* DESCRIPTION:
*       This file includes functions for receiving packet buffers
...
)
{
    ...
    return bspEthPortRxInit(rxBufPoolSize, rxBufPool_PTR, rxBufSize,
...
)
{
    ...
    return bspEthPortTxInit(numOfTxBufs);
#else
    return GT_NOT_SUPPORTED;
#endif
}
...
#ifdef GT_SMI
#ifdef XCAT_DRV
    return bspEthPortTxQueue(segmentList, segmentLen, numOfSegments, txQueue);
#else
...
{
#ifdef GT_SMI
    return bspEthInputHookAdd((BSP_RX_CALLBACK_FUNCPTR)userRxFunc);
#else
    return GT_NOT_SUPPORTED;
#endif
}
...
{
#ifdef GT_SMI
    return bspEthTxCompleteHookAdd((BSP_TX_COMPLETE_CALLBACK_FUNCPTR) userTxFunc);
#else
    return GT_NOT_SUPPORTED;
#endif
}
...
{
#ifdef GT_SMI
    return bspEthRxPacketFree(segmentList, numOfSegments, queueNum);
#else
    return GT_NOT_SUPPORTED;
#endif
}
...
#ifdef GT_SMI
#ifdef XCAT_DRV
    return bspEthPortTxModeSet(txMode);
#else 
...
{
#if defined (GT_SMI) && defined (XCAT_DRV) 
    return bspEthCpuCodeToQueue(dsaCpuCode,rxQueue);
#else
    return GT_NOT_SUPPORTED;
#endif
}
...
{
#if defined (GT_SMI) && defined (XCAT_DRV) 
    return bspEthPrePendTwoBytesHeaderSet(enable);
#else
    return GT_NOT_SUPPORTED;
#endif
}
