...
#ifdef  KERNEL_DMA_RW
    int rc;
    struct GT_DmaReadWrite_STC params;

    params.address = (mv_kmod_uintptr_t)address;
    params.length = (mv_kmod_size_t)length;
    params.burstLimit = (mv_kmod_size_t)burstLimit;
    params.buffer = (mv_kmod_uintptr_t)((uintptr_t)buffer);
