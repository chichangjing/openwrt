...
********************************************************************************
* gtDbFx900Mx620Config.c
*
* DESCRIPTION:
*       Initialization functions for the DB-FX910-EX120 boards.
*
* DEPENDENCIES:
*       None.
*
...
)
{
    ...
    for(devNum = 0; devNum < PRV_CPSS_MAX_PP_DEVICES_CNS; devNum++)
    {
        ...
        for(i = 0; i < 12; i++)
        {
/*            retVal = appDemoSetPortPhyAddr(appDemoPpConfigList[devNum].devNum, i,
                                           (GT_U8) (8 + i));
            if(retVal != GT_OK)
            {
                return retVal;
...
{
    /* Link status & port activity  */
    /*ledSetStreamParams(36,59);*/ /* [FINDME][SergeyR]: wasn't found in CPSS code */
...
*
* DESCRIPTION:
*       Registeration function for the DBFx910Ex120 board.*
* INPUTS:
*       boardRevId      - The board revision Id.
*
* OUTPUTS:
