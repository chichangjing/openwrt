...
*
*******************************************************************************/
static int l_cmdNetTcp_Connect(lua_State *L)
{
    GT_U8*          ipAddr;
    ...
    /* resolve address */
    ret = cmdOsSocketCreateAddr((GT_CHAR*)ipAddr, ipPort, &sockAddr, &sockAddrLen);
    if (ret != GT_OK)
    {
        lua_pushnil(L);
...
*
*******************************************************************************/
static int l_cmdNetTcp_Close(lua_State *L)
{
    GT_SOCKET_FD    sock;
...
*
*******************************************************************************/
static int l_cmdNetTcp_FlushInput(lua_State *L)
{
    GT_SOCKET_FD    sock;
    GT_VOID*        rfds;
...
*
*******************************************************************************/
static int l_cmdNetTcp_Send(lua_State *L)
{
    GT_SOCKET_FD    sock;
    const char      *s;
    size_t          len;
    ...
    if (s)
        bytes_sent = cmdOsSocketSend(sock, (GT_8*)s, (GT_SIZE_T)len);
...
*
*******************************************************************************/
static int l_cmdNetTcp_Wait(lua_State *L)
{
    GT_SOCKET_FD    sock;
...
*
*******************************************************************************/
static int l_cmdNetTcp_Recv(lua_State *L)
{
    GT_SOCKET_FD    sock;
    int             max_bytes = 1024;
    char            *buf;
    ...
    buf = (char*)cmdOsMalloc(max_bytes);
    if (!buf)
    {
        lua_pushnil(L);
    ...
    k = cmdOsSocketRecv(sock, buf, max_bytes, GT_TRUE);
    if (k < 0)
    {
        cmdOsFree(buf);
        lua_pushnil(L);
...
*
*******************************************************************************/
static int l_cmdNetTcp_WaitString(lua_State *L)
{
    GT_SOCKET_FD    sock;
    GT_U32          timeout = 0xffffffff;
    char            buf[64];
...
static const luaL_Reg luatcplib[] = {
  {"connect",   l_cmdNetTcp_Connect },
  {"close",     l_cmdNetTcp_Close },
  {"flushinput",l_cmdNetTcp_FlushInput },
  {"send",      l_cmdNetTcp_Send },
  {"wait",      l_cmdNetTcp_Wait },
  {"recv",      l_cmdNetTcp_Recv },
  {"waitline",  l_cmdNetTcp_WaitString },
  {NULL, NULL}
};

/*******************************************************************************
* cmdLuaNetRegisterAPI
*
* DESCRIPTION:
