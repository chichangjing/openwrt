...
{
    LUACLI_UINTPTR          paramsArray[MAX_PARAMS];
    LUACLI_U32              valuesArray[MAX_PARAMS];
    CPSS_API_PARAMETER_TYPE ptypesArray[MAX_PARAMS];
    int                     vtypesArray[MAX_PARAMS];
    GT_CHAR                 utypesArray[MAX_PARAMS][MAX_TYPE_LENGTH];
    LUACLI_RETSTATUS retval;
    typedef LUACLI_RETSTATUS (*CPSS_API_FUNC_PTR)(LUACLI_UINTPTR, ...);
    ...
    funcPtr = (CPSS_API_FUNC_PTR) LUACLI_GETFUNCTIONBYNAME(lua_tostring(L, 1));
    if (!funcPtr)
    {
    ...
    for (k = kv = 0; k < nparams; k++, kv++)
    {
        ...
        cmdOsStrCpy(utypesArray[k], lua_tostring(L, -1));
        ...
        else
        {
            ...
            if (!lua_isfunction(L, -1))
            {
                ...
                cvtFunc = (lua_CFunction) LUACLI_GETFUNCTIONBYNAME(lua_tostring(L, -2));
                if (cvtFunc)
                {
                    lua_pop(L, 1);
        ...
        if (ptypesArray[k] == CPSS_API_PARAMETER_TYPE_IN)
        {
            ...
            {
                paramsArray[k] = (LUACLI_UINTPTR)((LUACLI_U32)lua_tonumber(L, -1));
                lua_pop(L, 1);
            }
...
#define PUSH_PTR_TO_PARAMS(ptr) \
                paramsArray[k] = (LUACLI_UINTPTR)(ptr)
...
#define PUSH_PTR_TO_VALUES(ptr) \
        ...
        else /* ptypesArray[k] == OUT || INOUT */
        {
            if (lua_type(L, -1) == LUA_TNUMBER)
            {
                ...
                valuesArray[kv] = (LUACLI_U32)lua_tonumber(L, -1);
                lua_pop(L, 1);
            }
            else if (lua_type(L, -1) == LUA_TUSERDATA)
            {
    ...
    for (k = kv = 0; k < nparams; k++, kv++)
    {
        ...
        if (cmdOsStrCmp("GT_U16", utypesArray[k]) == 0) {  
            valuesArray[kv] = valuesArray[kv] >> 16;
        }
        if (cmdOsStrCmp("GT_U8", utypesArray[k]) == 0) { 
...
)
{
    ...
    if (lua_isstring(L, -1))
    {
        ...
        while(descr->string)
        {
            if (LUACLI_STRCMP(s, descr->string) == 0)
...
)
{
    ...
    if (descr)
    {
        for(; descr->string; descr++)
        {
            if (LUACLI_STRCMP(str, descr->string) == 0)
            {
                lua_pushboolean(L, 1);
                return 1;
            }
...
)
{
    ...
    if (descr)
    {
        for(; descr->string; descr++)
        {
            if (LUACLI_STRNCMP(str, descr->string, len) == 0)
            {
                lua_pushstring(L, descr->string);
...
{
    if (lua_isnumber(L, 1))
        lua_pushlightuserdata(L, (void*)((LUACLI_UINTPTR)lua_tonumber(L, 1)));
    else if (lua_islightuserdata(L, 1))
        lua_pushvalue(L, 1);
    else
        lua_pushlightuserdata(L, NULL);
...
)
{
    ...
    if (!lua_islightuserdata(L, 1))
        ...
        lua_pushnumber(L, (lua_Number)((LUACLI_UINTPTR)lua_touserdata(L, 1)));
    return 1;
}
