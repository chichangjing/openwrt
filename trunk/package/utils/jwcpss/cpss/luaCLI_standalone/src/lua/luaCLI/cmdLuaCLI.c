...
#ifdef ENV_MAINCMD
# include "../common/cmdLua_os_mainCmd.inc"
...
#endif
#ifdef ENV_POSIX
# include "../common/cmdLua_os_posix.inc"
# include <cmdStream.h>
...
static int luaCLI_readkey(cmdStreamPTR IOStream)
{
    ...
    while (IOStream->connected(IOStream))
    {
        ...
        {
            if (LUACLI_MEMCMP(keyseq[k].seq, buf, state) == 0)
            {
                matches++;
            }
        }
        ...
        {
            /* one key matched */
            if (LUACLI_MEMCMP(keyseq[k].seq, buf, state + 1) == 0)
                return keyseq[k].key;
        }
...
)
{
    char buf[20];
    int  l;
    l = LUACLI_SPRINTF(buf, fmt, param);
...
)
{
...
#ifdef  LUACLI_MEMSTAT
    LUACLI_MEMSET(&memStat, 0, sizeof(memStat));
#endif
    ...
    LUACLI_SPRINTF(line, " Memory used by Lua %d  nChunks=%d\n",
    ...
    while (IOStream->connected(IOStream))
    {
        if (state == STATE_CLI_E)
        {
            ...
            {
                LUACLI_SPRINTF(line, "Memory statistic:\n\ttotalUsed=%d\n\tnChunks=%d\n",
                        memStat.totalUsed, memStat.nChunks);
                IOStream->writeLine(IOStream, line);
                continue;
            }
