        ...
        case LUA_TUSERDATA:
            {
                ...
                if (bufPtr)
                {
                    cpssOsMemCpy(bufPtr+TL, lua_touserdata(L, -1), len);
                }
                return TL + (int)len;
            }
...
#define TL  (1+sizeof(size_t))
    ...
    switch (type)
    {
        ...
        case SER_TYPE_USERDATA:
            ...
            cpssOsMemCpy(lua_touserdata(L, -1), bufPtr + TL, len);
            break;
...
)
{
    ...
    node = (STORAGE_NODE_STCT*) cpssOsMalloc(sizeof(*node));
    if (node == NULL)
        return NULL;
