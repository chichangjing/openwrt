    ...
    if (cb == global->entity_cbs[i])
    {
      ...
      */
      ...
      if (i < global->num_entity_cbs)
        _mxml_memmove(global->entity_cbs + i, global->entity_cbs + i + 1,
...
_mxml_entity_cb(const char *name)	/* I - Entity name */
{
  ...
  */
  ...
  while ((last - first) > 1)
  {
    ...
    if ((diff = _mxml_strcmp(name, entities[current].name)) == 0)
  ...
  */
  ...
  if (!_mxml_strcmp(name, entities[first].name))
    return (entities[first].val);
  else if (!_mxml_strcmp(name, entities[last].name))
