 ...
 *        Partially removed file operations
 *
    ...
    mxml_save_cb_t cb)			/* I - Whitespace callback or MXML_NO_CALLBACK */
{
  ...
  */
  ...
  if ((s = _mxml_malloc(bytes + 1)) == NULL)
    return (NULL);
	      ...
	      int  *bufsize)		/* IO - Current buffer size */
{
  ...
  if (*bufptr >= (*buffer + *bufsize - 4))
  {
    ...
    */
    ...
    if ((newbuffer = _mxml_realloc(*buffer, *bufsize)) == NULL)
    {
      _mxml_free(*buffer);
...
mxml_fd_read(_mxml_fdbuf_t *buf)		/* I - File descriptor buffer */
{
  ...
  */
  ...
  if ((bytes = ((genFS_PTR)(buf->fs))->read(buf->fd, buf->buffer, sizeof(buf->buffer))) < 0)
      return (-1);
...
mxml_fd_write(_mxml_fdbuf_t *buf)	/* I - File descriptor buffer */
{
  ...
  */
  ...
  for (ptr = buf->buffer; ptr < buf->current; ptr += bytes)
    if ((bytes = ((genFS_PTR)(buf->fs))->write(buf->fd, ptr, buf->current - ptr)) < 0)
      return (-1);
					...
					/* I  - Get character function */
{
  ...
  while ((ch = (*getc_cb)(p, encoding)) != EOF)
    if (ch > 126 || (!_mxml_isalnum((char)ch) && ch != '#'))
      break;
  ...
  *entptr = '\0';
  ...
  {
    if (entity[1] == 'x')
      ch = _mxml_strtol(entity + 2, NULL, 16);
    else
      ch = _mxml_strtol(entity + 1, NULL, 10);
  }
    ...
    void            *sax_data)		/* I - SAX user data */
{
  ...
  */
  ...
  if ((buffer = _mxml_malloc(64)) == NULL)
  {
  ...
  while ((ch = (*getc_cb)(p, &encoding)) != EOF)
  {
        ...
        bufptr > buffer)
    {
      ...
      *bufptr = '\0';
      ...
      {
	case MXML_INTEGER :
            node = mxmlNewInteger(parent, _mxml_strtol(buffer, &bufptr, 0));
	    break;
    ...
    */
	...
	else if (ch == '&')
	{
	  ...
	  goto error;
	         ...
	         ((bufptr - buffer) == 3 && !_mxml_strncmp(buffer, "!--", 3)) ||
	         ((bufptr - buffer) == 8 && !_mxml_strncmp(buffer, "![CDATA[", 8)))
	  break;

      *bufptr = '\0';

      if (!_mxml_strcmp(buffer, "!--"))
      {
       /*
	...
	*bufptr = '\0';
	...
	if (node && !first)
      ...
      else if (!_mxml_strcmp(buffer, "![CDATA["))
      {
       /*
	...
	*/
	...
	while ((ch = (*getc_cb)(p, &encoding)) != EOF)
	{
	  if (ch == '>' && !_mxml_strncmp(bufptr - 2, "]]", 2))
	    break;
	  else if (mxml_add_char(ch, &bufptr, &buffer, &bufsize))
	    goto error;
	}
	...
	*/
        ...
        if (!parent || _mxml_strcmp(buffer + 1, parent->value.element.name))
	{
	 /*
    ...
    switch (node->type)
    {
      case MXML_ELEMENT :
	    ...
	    return (-1);
	      ...
	      !_mxml_strncmp(node->value.element.name, "!--", 3) ||
	    ...
	    */
	    ...
	    return (-1);
          ...
          col += _mxml_strlen(node->value.element.name) + 1;

	  for (i = node->value.element.num_attrs, attr = node->value.element.attrs;
	       i > 0;
	       i --, attr ++)
	  {
	    width = _mxml_strlen(attr->name);

	    if (attr->value)
	      width += _mxml_strlen(attr->value) + 3;
	  ...
	  if (node->child)
	  {
	    ...
	    */
		...
		return (-1);
              ...
              col += _mxml_strlen(node->value.element.name) + 3;
	  ...
	  if (node->prev)
	  {
	    if (global->wrap > 0 && col > global->wrap)
	    {
          ...
          _mxml_sprintf(s, "%d", node->value.integer);
      ...
      case MXML_OPAQUE :
	    ...
	    return (-1);
          ...
          col += _mxml_strlen(node->value.opaque);
          break;
	  ...
	  col += _mxml_strlen(node->value.text.string);
          break;
          ...
          if (global->custom_save_cb)
	  {
	    ...
	    const char	*newline;	/* Last newline in string */
	      ...
	      return (-1);
            ...
            if ((newline = _mxml_strrchr(data, '\n')) == NULL)
	      col += _mxml_strlen(data);
	    else
              col = _mxml_strlen(newline);
