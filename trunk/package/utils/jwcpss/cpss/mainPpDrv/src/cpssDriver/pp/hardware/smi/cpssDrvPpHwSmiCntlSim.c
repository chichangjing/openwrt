...
*
* DESCRIPTION:
*       Prestera driver Hardware Simulation for SMI read and write functions
*       implementation.
*
...
)
{
    ...
    (void)isDiag;
    (void)internalPciBase;

    simuDevId = scibGetDeviceId(baseAddr);

    prvCpssDrvPpConfig[devNum]->hwCtrl[portGroupId].baseAddr           = baseAddr;
    prvCpssDrvPpConfig[devNum]->hwCtrl[portGroupId].compIdx            = (GT_U8)simuDevId;
...
)
{
    ...
    scibReadMemory( prvCpssDrvPpConfig[devNum]->hwCtrl[portGroupId].compIdx,regAddr,1, &temp);
...
)
{
    ...
    scibWriteMemory(prvCpssDrvPpConfig[devNum]->hwCtrl[portGroupId].compIdx, regAddr, 1, &temp);
...
)
{
    ...
    for (i = 0; i < length; i++)
    {
        scibReadMemory(prvCpssDrvPpConfig[devNum]->hwCtrl[portGroupId].compIdx,
                                    addr, 1, &temp);
...
)
{
    ...
    for (i = 0; i < length; i++)
    {
        ...
        scibWriteMemory(prvCpssDrvPpConfig[devNum]->hwCtrl[portGroupId].compIdx,
                                     addr, 1, &temp);
