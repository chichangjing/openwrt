...
#    os_linux_patch_kernel || return 1
 ...
 #   os_linux_kernel_is_already_built && return 0
    ...
    if [ "$LSP_KERNEL_CONFIG" != "oldconfig" ]; then
        #clean
    echo  "  I make -C ${DIST} mrproper"
    fi
    #now make it
    echo "I make ${MAKE_FLAGS} -C ${DIST} $config"
    if [ $? -ne 0 ]
    then
...
#    if [ "x$DONT_CREATE_ROOTFS" != "xYES" ]; then
 #       os_linux_make_rootfs || return 1
  ...
  #      chmod a+x ${DIST}/scripts/$LINUX_CRFS_SCRIPT_NAME
   ...
   #     I ${DIST}/scripts/${LINUX_CRFS_SCRIPT_NAME} ${DIST}/rootfs \
                     ${TOOLKIT_PATH} ${LINUX_CRFS_ARCH_ARG} ${LINUX_DIST_PATH}
    #    if [ $? -ne 0 ]
     #   then
   ...
   # fi
    ...
    # now build kernel
    echo "make ${MAKE_FLAGS} -C ${DIST} $LSP_KERNEL_TYPE $mkopts"
    if [ $? -ne 0 ]; then
