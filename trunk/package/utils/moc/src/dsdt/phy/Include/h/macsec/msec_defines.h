...
typedef struct {
  ...
  int en_hdr;   /*/< Enable proprietary header */
} MATCH_EN;
...
typedef struct {
  ...
  MAD_U16 hdr_fld; /*/< proprietary header field */
} MATCH_FLD;
...
typedef struct {
  ...
  MAD_U64 sa [8]; /*/< Redirect SA for Port 0,1,2 and 3 ...8*/
} REDIR_HDR_T;
...
typedef struct {
  ...
  int hdr_as_pn;   /*/< If set, take 2LSB of pn from macsec header to match hdl_fld in ELU */
} EGR_CFG_T;
...
typedef struct {
  ...
  int ins_hdr_pn;  /*/ < If set, replace the 2LSB of PN in macsec header with hdr_fld in ILU */
} IGR_DEF_T;
 ...
 */
typedef struct {
  MAD_U64 macsec_vec [0x180]; /*/< 64 bit Vector array to store all the MACSec Statistics. */
} MACSEC_STAT_T;
...
#define GET_REV_DATA(_dev, a, b, c)    \
 (((_dev)->deviceId==MAD_88E1540 || (_dev)->deviceId==MAD_88E1548 || (_dev)->deviceId==MAD_88E1680M || (_dev)->deviceId==MAD_88E1680)?a:    \
...
/* #if defined (REV_A0_UP) || defined(PROD_1540L) */
  ...
  #define VN_SYSMAC_STATS(_dev) GET_REV_DATA(_dev, 0x2080, 0x2080, 0x1080) /*/< MAC Statistics */
  #define VN_GLOBAL(_dev)    GET_REV_DATA(_dev, 0x2100, 0x2100, 0x1100) /*/< Global registers */
  #define VN_PET_STATS(_dev) (((_dev)->deviceId==MAD_88E1540||(_dev)->deviceId==MAD_88E1548||(_dev)->deviceId==MAD_88E1680M||(_dev)->deviceId==MAD_88E1680)?0x2200:0)  /*/<Per EtherType statistics */
  #define VN_SEC_STATS(_dev)    GET_REV_DATA(_dev, 0x2800, 0x2400, 0x1200) /*/< MACsec Statistics  */
  #define NUM_OF_SA(_dev)       GET_REV_DATA(_dev, 0x20, 0x10, 0x8) /* */
  #define MEM_PER_SLICE(_dev)   GET_REV_DATA(_dev, 0x800, 0x800, 0x400) /* */
  #define MEM_PER_SA(_dev)      GET_REV_DATA(_dev, 0x80, 0x80, 0x20) /* */
  #define NUM_OF_MAC_STAT       0x20 /* */
/* For new PTP */
  #define VN_GLOBAL_PTP    0x2100 /*/< PTP Global  */
  #define VN_TAI_CFG       0x2300 /*/< TAI configuration  */
  #define VN_EG0_PTP       0x3000 /*/< PTP configuration for Egress port 0 */
  #define VN_IG0_PTP       0x3400 /*/< PTP configuration for Ingress port 0 */
  #define VN_EG1_PTP       0x3800 /*/< PTP configuration for Egress port 1 */
  #define VN_IG1_PTP       0x3c00 /*/< PTP configuration for Ingress port 1 */
  #define VN_EG2_PTP       0x4000 /*/< PTP configuration for Egress port 2 */
  #define VN_IG2_PTP       0x4400 /*/< PTP configuration for Ingress port 2 */
  #define VN_EG3_PTP       0x4800 /*/< PTP configuration for Egress port 3 */
  #define VN_IG3_PTP       0x4c00 /*/< PTP configuration for Ingress port 3 */
/* */
...
/* Per-Port Memory Map */
  ...
  #define VN_CFG_EEE_FLOW_CTRL(_dev)      GET_REV_DATA(_dev, 0x3c0, 0x3c0, 0x2c0)  /*/< Egress Context (ECT) Table */
/*@}*/
...
/*@}*/
/* EEE/Flow Control register Defines */
#define EEE_LPI_TIMEOUT_REG(_dev)         VN_CFG_EEE_FLOW_CTRL(_dev)+0x0
#define EEE_LPI_CONFIG_REG(_dev)          VN_CFG_EEE_FLOW_CTRL(_dev)+0x1
#define EEE_LPI_INTERRUPTS_REG(_dev)      VN_CFG_EEE_FLOW_CTRL(_dev)+0x2
#define EEE_LPI_INTERRUPT_MASK_REG(_dev)  VN_CFG_EEE_FLOW_CTRL(_dev)+0x3
#define EEE_LPI_IGR_BUFF_THRESH_REG(_dev) VN_CFG_EEE_FLOW_CTRL(_dev)+0x4
#define EEE_LPI_EEE_BUFF_THRESH_REG(_dev) VN_CFG_EEE_FLOW_CTRL(_dev)+0x6
#define EEE_LPI_IGR_DSC_THRESH_REG(_dev)  VN_CFG_EEE_FLOW_CTRL(_dev)+0x7
...
#define MSEC_BIT(x)           ((unsigned int)0x00000001 << (x))
#define bit_get(p,x)     (((p) &  (MSEC_BIT(x))) >> (x))
#define bit_set(p,x)     ((p) |=  (MSEC_BIT(x)))
#define bit_clear(p,x)   ((p) &= ~(MSEC_BIT(x)))
#define bit_flip(p,x)    ((p) ^=  (MSEC_BIT(x)))
#define bit_write(p,c,x) (c ? bit_set(p,x) : bit_clear(p,x))
/*@}*/
int msec_bits_write (unsigned int *data, int in_data, int bit_to, int bit_from);
int msec_bits_get (int in_data, int bit_to, int bit_from);
#define msec_bit_set bit_set

#if 0
/* New PTP Register Defines */
/* TAI register list */
#define TOD_0(_dev)          VN_TAI_CFG(_dev)+0x0
#define TOD_1(_dev)          VN_TAI_CFG(_dev)+0x1
#define TOD_2(_dev)          VN_TAI_CFG(_dev)+0x2
#define TOD_3(_dev)          VN_TAI_CFG(_dev)+0x3
#define FRC_0(_dev)          VN_TAI_CFG(_dev)+0x4
#define FRC_1(_dev)          VN_TAI_CFG(_dev)+0x5
#define FRC_2(_dev)          VN_TAI_CFG(_dev)+0x6
#define FRC_3(_dev)          VN_TAI_CFG(_dev)+0x7
#define DRIFT_ADJ_CFG(_dev)  VN_TAI_CFG(_dev)+0x8
#define TOD_STEP_NS(_dev)    VN_TAI_CFG(_dev)+0x9
#define TOD_STEP_FS(_dev)    VN_TAI_CFG(_dev)+0xA
#define TOD_CFG_GEN(_dev)    VN_TAI_CFG(_dev)+0xB
#define PULSE_DELAY(_dev)    VN_TAI_CFG(_dev)+0xC
#define TRIG_GEN_TOD_0(_dev) VN_TAI_CFG(_dev)+0xD
#define TRIG_GEN_TOD_1(_dev) VN_TAI_CFG(_dev)+0xE
#define TRIG_GEN_TOD_2(_dev) VN_TAI_CFG(_dev)+0xF
#define TRIG_GEN_TOD_3(_dev) VN_TAI_CFG(_dev)+0x10
#define TRIG_GEN_MASK_0(_dev) VN_TAI_CFG(_dev)+0x11
#define TRIG_GEN_MASK_1(_dev) VN_TAI_CFG(_dev)+0x12
#define TRIG_GEN_MASK_2(_dev) VN_TAI_CFG(_dev)+0x13
#define TRIG_GEN_MASK_3(_dev) VN_TAI_CFG(_dev)+0x14
#define TOD_LOAD_0(_dev)      VN_TAI_CFG(_dev)+0x15
#define TOD_LOAD_1(_dev)      VN_TAI_CFG(_dev)+0x16
#define TOD_LOAD_2(_dev)      VN_TAI_CFG(_dev)+0x17
#define TOD_LOAD_3(_dev)      VN_TAI_CFG(_dev)+0x18
#define TOD_CAP_0(_dev)       VN_TAI_CFG(_dev)+0x19
#define TOD_CAP_1(_dev)       VN_TAI_CFG(_dev)+0x1a
#define TOD_CAP_2(_dev)       VN_TAI_CFG(_dev)+0x1b
#define TOD_CAP_3(_dev)       VN_TAI_CFG(_dev)+0x1c
#define TOD_CAP1_0(_dev)      VN_TAI_CFG(_dev)+0x1d
#define TOD_CAP1_1(_dev)      VN_TAI_CFG(_dev)+0x1e
#define TOD_CAP1_2(_dev)      VN_TAI_CFG(_dev)+0x1f
#define TOD_CAP1_3(_dev)      VN_TAI_CFG(_dev)+0x20
#define TOD_CAP_CFG(_dev)     VN_TAI_CFG(_dev)+0x21
#define PULSE_IN_CNT(_dev)    VN_TAI_CFG(_dev)+0x22
#define TOD_FUNC_CFG(_dev)    VN_TAI_CFG(_dev)+0x23
#define PCLK_CFG(_dev)        VN_TAI_CFG(_dev)+0x24
#define PCLK_CNT_1(_dev)      VN_TAI_CFG(_dev)+0x25
#define PCLK_CNT_0(_dev)      VN_TAI_CFG(_dev)+0x26
#define DRIFT_THR_CFG(_dev)   VN_TAI_CFG(_dev)+0x27
#define NEXT_RT_0(_dev)       VN_TAI_CFG(_dev)+0x28
#define NEXT_RT_1(_dev)       VN_TAI_CFG(_dev)+0x29
#define NEXT_RT_2(_dev)       VN_TAI_CFG(_dev)+0x2a
#define NEXT_RT_3(_dev)       VN_TAI_CFG(_dev)+0x2b
#define CLOCK_CYC(_dev)       VN_TAI_CFG(_dev)+0x2c
#define EXTR_CLOCK_PROP_D(_dev)    VN_TAI_CFG(_dev)+0x2d
#define CLK_IN_CNT(_dev)           VN_TAI_CFG(_dev)+0x2e
#define TAI_INTR(_dev)        VN_TAI_CFG(_dev)+0x28
#define TAI_INTR_MASK(_dev)   VN_TAI_CFG(_dev)+0x28

/* PTP Inline Resters */
#define CFG_GEN			0
#define CFG_GEN2		1
#define INTR_SET		2
#define INTR_SET_MASK	3
#define TIMEOUT_CTL		4
#define STT_CTL			5	
#define DISP			6	
#define DISP_RMA_S		7
#define TS_QUEUE		8
