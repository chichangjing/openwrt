...
extern int madSmiRead (unsigned int smiPair, unsigned int phyAddr, 
                    unsigned int reg, unsigned int* value);
extern int madSmiWrite (unsigned int smiPair, unsigned int phyAddr, 
                    unsigned int reg, unsigned int value);
...
static MAD_U32        nMADDevices = 0;
 ...
 MAD_BOOL bspSpecificReadMii(MAD_DEV* dev, unsigned int phyAddr, 
               unsigned int reg, unsigned int* value)
{
int ret;
/* printf ("^^^^^^^^ bspSpecificReadMii smiPair %x phyAddr %x reg %x value %x\n", (unsigned int)dev->appData, phyAddr, reg, *value); */
         ret=madSmiRead((unsigned int)dev->appData, phyAddr, reg, value);
     if (ret)
        return MAD_TRUE;
     else
        return MAD_FALSE;
 ...
 MAD_BOOL bspSpecificWriteMii(MAD_DEV* dev, unsigned int phyAddr,
                      unsigned int reg, unsigned int value)
{
int ret;
        ret = madSmiWrite((unsigned int)dev->appData, phyAddr, reg, value);
     if (ret)
        return MAD_TRUE;
     else
        return MAD_FALSE;
...
/*        case MAD_88E3019: return ("MAD_88E3019 "); */
        ...
        case MAD_88E1545: return ("MAD_88E1545 ");
...
#if 0
static MAD_BOOL madSMIRead(MAD_DEV* dev, unsigned int phyAddr, 
              unsigned int reg, unsigned int* value)
{
    return (MAD_BOOL)madSmiRead((unsigned int)dev->appData, phyAddr, reg, value);
}

static MAD_BOOL madSMIWrite(MAD_DEV* dev, unsigned int phyAddr, 
              unsigned int reg, unsigned int value)
{
    return (MAD_BOOL)madSmiWrite((unsigned int)dev->appData, phyAddr, reg, value);
}
#endif
...
static MAD_STATUS madStart(MAD_DEV* dev,  int smiPair)
{
    ...
    MSG_PRINT(("Smi Pair         : %x\n", (unsigned int)dev->appData));
