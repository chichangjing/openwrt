...
/*  int        port, cx; */
  ...
  for (port=0; port<dev->numOfPorts; port++) 
  {
    ...
    retVal = msecReadPortReg (dev, port, SLC_RAM_CTRL1(devp), &my_data); 
    bit_set (my_data, 10);
    retVal = msecWritePortReg (dev, port, SLC_RAM_CTRL1(devp), my_data);
  }
  ...
  for (port=0; port<dev->numOfPorts; port++)
  {
    ...
    retVal = msecWritePortReg (dev, port, FIPS_CTL(devp), 0x1);

    /* Disable Egress Ghash FIPS CTL */
    retVal = msecWritePortReg (dev, port, FIPS_CTL(devp), 0x0);

    /* Bring MACSEC Block out of soft reset */
    retVal = msecWritePortReg(dev, port, RESET_CTL(devp), 0x0);
...
//  else if ((dev->deviceId == MAD_88E1540)||(dev->deviceId == MAD_88E1548)||(dev->deviceId == MAD_88E1680)||(dev->deviceId == MAD_88E1680M))
        test_act_fld.sec_level=ent_num; /*/< Index to the context to use */
...
if (dev->deviceId == MAD_88E1340)
  ...
  printf(" sec_level %x - %x \n", test_act_fld.sec_level, check_act_fld.sec_level);
