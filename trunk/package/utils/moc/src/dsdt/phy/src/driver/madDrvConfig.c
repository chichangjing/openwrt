...
/*          if(dev->deviceId == MAD_88E1540)
		  {
            ...
            *hwMode = MAD_PHY_MODE_AUTO_DETECT_SC|MAD_PHY_MODE_COPPER|MAD_PHY_MODE_QSGMII;
            break;        
        case 7:
/*          if(dev->deviceId == MAD_88E1540)
		  {
            ...
            *hwMode = MAD_PHY_MODE_AUTO_DETECT_XC|MAD_PHY_MODE_COPPER|MAD_PHY_MODE_QSGMII;
            break;        
        case 2:
        case 3:
...
)
{
    ...
    phyInfo->swPhyType = MAD_PHY_IN_SWITCH_TYPE_NO;  /* Default is independent Phy */
...
/*            phyInfo->pageType = MAD_PHY_PAGE_WRITE_BACK;  */
        ...
        case MAD_88E1240:
            ...
            if ((dev->deviceId == MAD_88E1149R)||(dev->deviceId == MAD_SWG65G)            ||(dev->deviceId == MAD_MELODY))
            {
        ...
        case MAD_88E1545:
            ...
            phyInfo->macIfLoopType = MAD_PHY_LOOPBACK_TYPE8;
            phyInfo->lineLoopType = MAD_PHY_LINE_LB_TYPE6;
            phyInfo->exLoopType = MAD_PHY_EX_LB_TYPE3;
            ...
            phyInfo->featureSet1 = MAD_PHY_SNOOPING_CONTROL | MAD_PHY_SYNC_FIFO                
                                 ...
                                 | MAD_PHY_UNIDIRECT_TRABSMIT | MAD_PHY_FLP_EXCH_COMP_NO_LNK    
                                 | MAD_PHY_DUPLEX_MISMATCH_INDC    | MAD_PHY_LINK_DISCONNECT_INDC    
                                 | MAD_PHY_ALT_VCT_CAPABLE | MAD_PHY_PREFERRED_MEDIA | MAD_PHY_AUTO_SELECT;
            phyInfo->featureSet1 |= MAD_PHY_EEE_LP_MODE;

            for(port=0; port<dev->numOfPorts; port++)
            {
                if((retVal=madHwMode_20_18(dev,port,&phyInfo->hwMode[port])) != MAD_OK)
                  return retVal;
            }
            phyInfo->featureSet |= MAD_PHY_FULL_PAGE_ADDR;
           break;
        ...
        case MAD_88E1540:
            ...
            for(port=0; port<dev->numOfPorts; port++)
            {
		      if((phyId==MAD_88E1540)||(dev->subDevId == MAD_SUB_88E1512))
			  {
                if((retVal=madHwMode_20_18(dev,port,&phyInfo->hwMode[port])) != MAD_OK)
                  return retVal;
			  }
		...
		case MAD_88E151x:
            ...
            phyInfo->featureSet1 = MAD_PHY_SNOOPING_CONTROL | MAD_PHY_SYNC_FIFO                
                                 ...
                                 | MAD_PHY_DUPLEX_MISMATCH_INDC    | MAD_PHY_LINK_DISCONNECT_INDC    
                                 | MAD_PHY_ALT_VCT_CAPABLE | MAD_PHY_PREFERRED_MEDIA | MAD_PHY_AUTO_SELECT
								 | MAD_PHY_PTP_TAI_CAPABLE;
			if(phyId==MAD_88E151x)
              phyInfo->featureSet1 |= MAD_PHY_WOL;
            ...
            for(port=0; port<dev->numOfPorts; port++)
            {
		      if((dev->subDevId == MAD_SUB_88E1512))
			  {
                if((retVal=madHwMode_20_18(dev,port,&phyInfo->hwMode[port])) != MAD_OK)
                  return retVal;
			  }
        ...
        case MAD_88E1548:
            ...
            phyInfo->featureSet1 = MAD_PHY_SNOOPING_CONTROL | MAD_PHY_SYNC_FIFO                
                                 ...
                                 | MAD_PHY_UNIDIRECT_TRABSMIT | MAD_PHY_FLP_EXCH_COMP_NO_LNK    
                                 | MAD_PHY_DUPLEX_MISMATCH_INDC    | MAD_PHY_LINK_DISCONNECT_INDC    
                                 | MAD_PHY_ALT_VCT_CAPABLE | MAD_PHY_AUTO_SELECT 
								 | MAD_PHY_PREFERRED_MEDIA;
            phyInfo->featureSet1 |= MAD_PHY_MACSEC_CAPABLE;    
              phyInfo->featureSet1 |= MAD_PHY_1STEP_PTP_CAPABLE;                
              phyInfo->featureSet1 |= MAD_PHY_SYNC_ETHNET;
              phyInfo->featureSet1 |= MAD_PHY_EEE_LP_MODE;
              phyInfo->featureSet1 |= MAD_PHY_XMDIO_CAPABLE;
        ...
        case MAD_88E1680M:
            ...
            phyInfo->interruptType = MAD_PHY_INTERRUPT_TYPE_2;
            phyInfo->featureSet |= MAD_PHY_LED_CONTROL;
            phyInfo->featureSet1 = MAD_PHY_SYNC_FIFO                
                                 | MAD_PHY_1G_LINK_DOWN_INDICAT    | MAD_PHY_RX_ER_BYTE_CAPTURE        
                                 | MAD_PHY_QSGMII_PRBS_GEN             
                                 | MAD_PHY_UNIDIRECT_TRABSMIT | MAD_PHY_FLP_EXCH_COMP_NO_LNK    
                                 | MAD_PHY_DUPLEX_MISMATCH_INDC    | MAD_PHY_LINK_DISCONNECT_INDC    
                                 | MAD_PHY_ALT_VCT_CAPABLE;
            phyInfo->featureSet1 |= MAD_PHY_MACSEC_CAPABLE;    
              ...
              phyInfo->featureSet1 |= MAD_PHY_XMDIO_CAPABLE;
            phyInfo->featureSet |= MAD_PHY_DSP_VCT;
            phyInfo->dspVctType = MAD_PHY_DSP_VCT_TYPE2;

            for(port=0; port<dev->numOfPorts; port++)
            {
                phyInfo->hwMode[port] = MAD_PHY_MODE_COPPER | MAD_PHY_MODE_QSGMII;
            }
            phyInfo->featureSet |= MAD_PHY_FULL_PAGE_ADDR;
           break;
        case MAD_SW1680:
...
*
*******************************************************************************/
MAD_U8 mad_lport2port
(
    IN MAD_U16    portVec,
    IN MAD_LPORT     port
)
{
    MAD_U8    hwPort, tmpPort;
...
)
{
    ...
    if (hwPort == MAD_DRV_INVALID_PORT)
        return (MAD_LPORT)hwPort;

    if (!MAD_IS_PORT_SET(portVec, hwPort))
        return (MAD_LPORT)MAD_DRV_INVALID_PORT;
