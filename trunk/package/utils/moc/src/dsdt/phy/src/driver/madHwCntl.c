...
*
* DESCRIPTION:
*       This function reads a Marvell phy register through XMDIO or through MDIO.
*       If dev->fmadReadXMii is NULL, the reading through MDIO indirectly.
*
* INPUTS:
*       portNum - Port number to read the register for.
...
)
{
  ...
  regAddr = (MAD_U16)_regAddr&0xffff;
  devAddr = (MAD_U16)((_regAddr>>16)&0xff);
  phyAddr = MAD_CALC_SMI_DEV_ADDR(dev, portNum);

  madSemTake(dev,dev->multiAddrSem,OS_WAIT_FOREVER);
  if(dev->fmadReadXMii==NULL)
  {
    /* Set MMD access control address is Address | devAddr */
    tmpData = MAD_MMD_ADDR | devAddr;
    retVal = madSmiIfWriteRegister(dev,phyAddr, 13,tmpData);
    if(retVal != MAD_OK)
    {
        madSemGive(dev,dev->multiAddrSem);
        MAD_DBG_ERROR(("Failed to write register (portNum %i)\n",portNum));
    ...
    tmpData = regAddr;
    retVal = madSmiIfWriteRegister(dev,phyAddr, 14,tmpData);
    if(retVal != MAD_OK)
    {
        madSemGive(dev,dev->multiAddrSem);
        MAD_DBG_ERROR(("Failed to write register (portNum %i)\n",portNum));
    ...
    tmpData = MAD_MMD_DATA_NO_INC | devAddr;
    retVal = madSmiIfWriteRegister(dev,phyAddr, 13,tmpData);
    if(retVal != MAD_OK)
    {
        madSemGive(dev,dev->multiAddrSem);
    ...
    retVal = madSmiIfReadRegister(dev,phyAddr, 14,&tmpData);
    if(retVal != MAD_OK)
    {
        madSemGive(dev,dev->multiAddrSem);
        MAD_DBG_ERROR(("Failed to read register (portNum %i)\n",portNum));
  ...
  {
    /* Read XMDIO access Data */
    retVal = madXSmiIfReadRegister(dev,phyAddr, devAddr, regAddr,&tmpData);
    if(retVal != MAD_OK)
    {
        madSemGive(dev,dev->multiAddrSem);
        MAD_DBG_ERROR(("Failed to read XMIO register (portNum %i)\n",portNum));
        return MAD_DRV_FAIL_READ_REG;
    }
  }
  ...
  *data = tmpData;
    ...
    MAD_DBG_INFO(("Read from  devAddr %d, regAddr %d\n",devAddr, regAddr));
...
*
* DESCRIPTION:
*       This function writes to a port phy register through XMDIOor through MDIO.
*       If dev->fmadWriteXMii is NULL, the writing through MDIO indirectly.
*
* INPUTS:
*       portNum - Port number to read the register for.
*       regAddr - The device address + register's address.
...
)
{
  ...
  if(dev->fmadWriteXMii==NULL)
  {
    ...
    retVal = madSmiIfWriteRegister(dev,phyAddr, 14, data);
    if(retVal != MAD_OK)
    {
        madSemGive(dev,dev->multiAddrSem);
        MAD_DBG_ERROR(("Failed to read register (portNum %i)\n",portNum));
  ...
  {
    /* Write XMDIO access Data */
    retVal = madXSmiIfWriteRegister(dev,phyAddr, devAddr, regAddr,data);
    if(retVal != MAD_OK)
    {
        madSemGive(dev,dev->multiAddrSem);
        MAD_DBG_ERROR(("Failed to write XMIO register (portNum %i)\n",portNum));
        return MAD_DRV_FAIL_WRITE_REG;
    }
  }
...
/*******************************************************************************
* madHw45GetPhyRegField
*
* DESCRIPTION:
*       This function reads a specified field from a port phy register through XMDIO
*       or through MDIO.
*       If dev->fmadWriteXMii is NULL, the reading through MDIO indirectly.
*
* INPUTS:
*       portNum - Port number to read the register for.
*       regAddr - The device address + register's address.
...
*
*******************************************************************************/
MAD_STATUS madHw45GetPhyRegField
(
    IN  MAD_DEV    *dev,
    IN  MAD_U8     portNum,
    IN  MAD_U32    _regAddr,
...
)
{
    ...
    retVal = madHw45ReadPhyReg(dev, portNum, _regAddr, &tmpData);
    if(retVal != MAD_OK)
    {
        MAD_DBG_ERROR(("Failed to read register \n"));
        return (MAD_DRV_FAIL_READ_REG|MAD_FAIL);
    }
...
/*******************************************************************************
* madHw45SetPhyRegField
*
* DESCRIPTION:
*       This function writes a specified field to a port phy register through XMDIO
*       or through MDIO.
*       If dev->fmadWriteXMii is NULL, the writing through MDIO indirectly.
*
* INPUTS:
...
*
*******************************************************************************/
MAD_STATUS madHw45SetPhyRegField
(
    IN  MAD_DEV    *dev,
    IN  MAD_U8     portNum,
    IN  MAD_U32       _regAddr,
...
)
{
    ...
    retVal = madHw45WritePhyReg(dev, portNum, _regAddr, tmpData);
    ...
    if(retVal != MAD_OK)
    {
        ...
        return (MAD_DRV_FAIL_WRITE_REG|MAD_FAIL);
    }
