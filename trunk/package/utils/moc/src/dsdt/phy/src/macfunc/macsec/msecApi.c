...
#ifdef DEBUG_CPU
  MAD_CRITIC_INFO( ("msecWriteReg      port %d  : Addr %04x Data %08x\n", 
                         port, (unsigned short)addr, (unsigned int)data));
#endif

  retVal = msecWriteReg (dev, port, addr, data);
  if(retVal != MAD_OK)
  {
...
#ifdef DEBUG
  MAD_CRITIC_INFO( ("msecReadReg       port %d   : Addr %04x Data %08x\n", port, addr, (unsigned int)*data));
#endif

  return MAD_OK;

}
/* check test chip revision on deviceId MAD_88E1540/1548 and MAD_88E1340 to differentiate MACSec changes */
MAD_MSEC_REV check_testchip_revision (MAD_DEV* dev,int port)
{
  MAD_STATUS   retVal;
  unsigned short data;
  int phyaddr;
...
/*  phyaddr = dev->msec_ctrl.port_num + dev->msec_ctrl.dev_num * dev->numOfPorts; */
  ...
  } else if (dev->revision == 0 && 
	  ((dev->deviceId == MAD_88E1540)||(dev->deviceId == MAD_88E1548)||(dev->deviceId == MAD_88E1680)||(dev->deviceId == MAD_88E1680M))) {
...
*
* DESCRIPTION:
*        Configuration of Phy Mac for Macsec application.
*
* INPUTS:
*       dev  - pointer to MAD driver structure returned from mdLoadDriver
...
)
{
  ...
  retVal = msecWriteReg(dev, port, IMB_CTL(dev), 0x8000);
  if(retVal != MAD_OK)
  {
     MAD_DBG_ERROR(("Failed to call msecWritePortReg \n"));
     return retVal;
...
)
{
  ...
  retVal = msec_clr_macsec_stat (dev, port);
  if(retVal != MAD_OK)
  {
  ...
  retVal = msec_clr_mib_stat (dev, port);
  if(retVal != MAD_OK)
  {
...
)
{
  ...
  addr += 1;
  /* prop. header  15:0*/
  data = (MAD_U32) ((egr_lkup->lk_match_fld->hdr_fld) & 0xFFFFLL);
...
#endif
  if (dev->deviceId == MAD_88E1540||dev->deviceId == MAD_88E1548||dev->deviceId == MAD_88E1680||dev->deviceId == MAD_88E1680M)
  ...
  bit_write  (data,  egr_lkup->lk_match_en->en_hdr, 25);
...
)
{
  ...
  msec_bits_write(&data, match_fld->hdr_fld, 15, 0);
...
{  
  ...
  bit_write (data,  match_en->en_hdr,  25);
...
)
{
  ...
  if (dev->deviceId == MAD_88E1340)
    data = egr_ctx->tci;
  else if ((dev->deviceId == MAD_88E1540)||(dev->deviceId == MAD_88E1548)||(dev->deviceId == MAD_88E1680)||(dev->deviceId == MAD_88E1680M))
...
)
{
  ...
  egr_lkup->lk_match_fld->hdr_fld = (MAD_U16) msec_bits_get (data, 15, 0);
  ...
  egr_lkup->lk_match_en->en_hdr   = bit_get  (data, 25);
...
#endif
  msec_bits_write(&data, igr_lkup->lk_match_fld->hdr_fld, 31,16);
...
{  
  ...
  addr += 1;
  data  = 0x0;
  msec_bits_write (&data, match_fld->hdr_fld, 31, 16);
...
)
{
  ...
  else if (dev->deviceId == MAD_88E1540 || dev->deviceId == MAD_88E1548 ||
...
)
{
  ...
  retVal = msecWriteReg (dev, i, addr, data);
  if(retVal != MAD_OK)
  {
     MAD_DBG_ERROR(("Failed to write register \n"));
     return 0;
...
)
{
  ...
  addr             = ENCAP_DA0(dev);
  retVal             = msecReadReg (dev, i, addr, &data);
...
)
{
  ...
  bit_write (data, egr_cfg->hdr_as_pn, 17);
  retVal = msecWritePortReg (dev, port, addr, data);
  if(retVal != MAD_OK)
  {
     MAD_DBG_ERROR(("Failed to call msecWritePortReg \n"));
...
)
{
  ...
  egr_cfg->hdr_as_pn = bit_get(data, 17);
...
)
{
  ...
  bit_write (data, igr_cfg->def_act->ins_hdr_pn, 30);
  retVal = msecWritePortReg (dev, port, addr, data);
  if(retVal != MAD_OK)
  {
     MAD_DBG_ERROR(("Failed to call msecWritePortReg \n"));
...
)
{
  ...
  bit_write (data, def_act->def_redir,   28);
  bit_write (data, def_act->ins_hdr_pn, 30);
  retVal = msecWritePortReg (dev, port, addr, data);
  if(retVal != MAD_OK)
  {
     MAD_DBG_ERROR(("Failed to call msecWritePortReg \n"));
...
)
{
  ...
  igr_cfg->def_act->ins_hdr_pn          = bit_get (data, 30);
...
)
{
  ...
  def_act->ins_hdr_pn = bit_get (data, 30);
...
)
{
  ...
  addr = EEE_LPI_TIMEOUT_REG(dev);
  data = lpiTimer->fastExitTime;
  data = data<<8;
  data |= lpiTimer->slowExitTime;
  data = data<<8;
  data |= lpiTimer->fastEnterTime;
  data = data<<8;
  data |= lpiTimer->slowEnterTime;
  retVal = msecWritePortReg (dev, port, addr, data);
  if(retVal != MAD_OK)
  {
     MAD_DBG_ERROR(("Failed to call msecWritePortReg \n"));
  ...
  addr = EEE_LPI_CONFIG_REG(dev);
  data = (lpiCfg->force_sys_lpi<<3)|(lpiCfg->force_wre_lpi<<2);
  data |= (lpiCfg->cfg_slave_lpi<<1)|(lpiCfg->cfg_master_lpi<<0);
  retVal = msecWritePortReg (dev, port, addr, data);
  if(retVal != MAD_OK)
  {
     MAD_DBG_ERROR(("Failed to call msecWritePortReg \n"));
...
)
{
  ...
  lpiTimer->fastExitTime = (MAD_U8)(data >> 24)&0xFF;
  lpiTimer->slowExitTime = (MAD_U8)(data >> 16)&0xFF;
  lpiTimer->fastEnterTime = (MAD_U8)(data >> 8)&0xFF;
  lpiTimer->slowEnterTime = (MAD_U8)(data >> 0)&0xFF;
  ...
  lpiCfg->force_sys_lpi = (MAD_U8)((data >> 3)&1);
  lpiCfg->force_wre_lpi = (MAD_U8)((data >> 2)&1);
  lpiCfg->cfg_slave_lpi = (MAD_U8)((data >> 1)&1);
  lpiCfg->cfg_master_lpi = (MAD_U8)((data >> 0)&1);

  return retVal;
}

/*******************************************************************************
* msec_SetEEELPIInterrupt
*
* DESCRIPTION:
...
*
*******************************************************************************/
MAD_STATUS msec_SetEEELPIInterrupt 
(
    IN  MAD_DEV        *dev,
    IN    int          port,
    IN MAD_BOOL      rxEn,
...
)
{
  ...
  addr = EEE_LPI_INTERRUPTS_REG(dev);
  data = 0;
  if(rxEn==MAD_TRUE)
    data = 2;
  if(txEn==MAD_TRUE)
...
*
*******************************************************************************/
MAD_STATUS msec_GetEEELPIInterrupt 
(
    IN  MAD_DEV        *dev,
    IN    int          port,
    OUT MAD_BOOL      *rxEn,
...
*
*******************************************************************************/
MAD_STATUS msec_SetEEELPIInterruptMask 
(
    IN  MAD_DEV        *dev,
    IN    int          port,
    IN MAD_BOOL      rxEn,
...
)
{
  ...
  addr = EEE_LPI_INTERRUPT_MASK_REG(dev);
  data = 0;
  if(rxEn==MAD_TRUE)
    data = 2;
  if(txEn==MAD_TRUE)
...
*
*******************************************************************************/
MAD_STATUS msec_GetEEELPIInterruptMask 
(
    IN  MAD_DEV        *dev,
    IN    int          port,
    OUT MAD_BOOL      *rxEn,
...
*
*******************************************************************************/
MAD_STATUS msec_SetEEELPIIgrBuffThresh 
(
    IN  MAD_DEV        *dev,
    IN    int          port,
...
)
{
  ...
  addr = EEE_LPI_IGR_BUFF_THRESH_REG(dev);
  data = 0;
  data |= igr_buf_on_thresh;
  data = data<<8;
  data |= igr_buf_off_thresh;
...
*
*******************************************************************************/
MAD_STATUS msec_GetEEELPIIgrBuffThresh 
(
    IN  MAD_DEV        *dev,
    IN    int          port,
...
)
{
  ...
  *igr_buf_on_thresh = (MAD_U8)((data>>8)&0xff);
  *igr_buf_off_thresh = (MAD_U8)((data)&0xff);

  return retVal;
}

/*******************************************************************************
* msec_SetEEELPIEEEBuffThresh
*
* DESCRIPTION:
...
*
*******************************************************************************/
MAD_STATUS msec_SetEEELPIEEEBuffThresh 
(
    IN  MAD_DEV        *dev,
    IN    int          port,
...
)
{
  ...
  addr = EEE_LPI_EEE_BUFF_THRESH_REG(dev);
  data = 0;
  data |= eee_buf_on_thresh;
  data = data<<8;
  data |= eee_buf_off_thresh;
...
*
*******************************************************************************/
MAD_STATUS msec_GetEEELPIEEEBuffThresh 
(
    IN  MAD_DEV        *dev,
    IN    int          port,
...
)
{
  ...
  *eee_buf_on_thresh = (MAD_U8)((data>>8)&0xff);
  *eee_buf_off_thresh = (MAD_U8)((data)&0xff);

  return retVal;
}

/*******************************************************************************
* msec_SetEEELPIIgrDscThresh
*
* DESCRIPTION:
...
*
*******************************************************************************/
MAD_STATUS msec_SetEEELPIIgrDscThresh 
(
    IN  MAD_DEV        *dev,
    IN    int          port,
...
)
{
  ...
  addr = EEE_LPI_IGR_DSC_THRESH_REG(dev);
  data = 0;
  data |= igr_dsc_on_thresh;
  data = data<<8;
  data |= igr_dsc_off_thresh;
...
*
*******************************************************************************/
MAD_STATUS msec_GetEEELPIIgrDscThresh 
(
    IN  MAD_DEV        *dev,
    IN    int          port,
...
)
{
  ...
  *igr_dsc_on_thresh = (MAD_U8)((data>>8)&0xff);
  *igr_dsc_off_thresh = (MAD_U8)((data)&0xff);
