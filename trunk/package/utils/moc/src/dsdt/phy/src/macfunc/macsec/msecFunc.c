...
)
{
  ...
  */
...
#ifdef MSEC_USE_PLATFORM
  {
    ...
    retVal = madSmiIfWriteRegister(dev, port, 22, 16);
    if(retVal != MAD_OK)
    {
    ...
    retVal = madSmiIfWriteRegister(dev, port, 1, addr);
    if(retVal != MAD_OK)
    {
      MAD_DBG_ERROR(("Failed to write register \n"));
      return MAD_DRV_FAIL_WRITE_REG;
    ...
    dataS = (MAD_U16)(data & 0xFFFF);
    retVal = madSmiIfWriteRegister(dev, port, 2, dataS);
    if(retVal != MAD_OK)
    {
      MAD_DBG_ERROR(("Failed to write register \n"));
      return MAD_DRV_FAIL_WRITE_REG;
    ...
    dataS = (MAD_U16)((data >> 16) & 0xFFFF);
    retVal = madSmiIfWriteRegister(dev, port,3, dataS);
    if(retVal != MAD_OK)
    {
      MAD_DBG_ERROR(("Failed to write register \n"));
      return MAD_DRV_FAIL_WRITE_REG;
    ...
    retVal = madSmiIfWriteRegister(dev, port, 22, 0);
    if(retVal != MAD_OK)
    {
      MAD_DBG_ERROR(("Failed to write register \n"));
      return MAD_DRV_FAIL_WRITE_REG;
...
#else
  {
       if((retVal = madHwWritePagedPhyReg(dev,phyaddr,16,1,addr))
        ...
        dataS = (MAD_U16)(data & 0xFFFF);
        if((retVal = madHwWritePagedPhyReg(dev,phyaddr,16,2,dataS))
            != MAD_OK)
        {
            MAD_DBG_ERROR(("Writing to paged phy reg (%i,%i) failed.\n",phyaddr,2));
            return (retVal | MAD_API_FAIL_WRITE_PAGED_REG);
        ...
        dataS = (MAD_U16)((data >> 16) & 0xFFFF);
        if((retVal = madHwWritePagedPhyReg(dev,phyaddr,16,3,dataS))
            != MAD_OK)
        {
            MAD_DBG_ERROR(("Writing to paged phy reg (%i,%i) failed.\n",phyaddr,3));
            return (retVal | MAD_API_FAIL_WRITE_PAGED_REG);
...
{
  MAD_DBG_ERROR(("RW U1 p0 R22 h10\n"));
  MAD_DBG_ERROR(("RW U1 p0 R1 h%x\n", addr));
  dataS = madLong2Short(data & 0xFFFF);
  MAD_DBG_ERROR(("RW U1 p0 R2 h%x\n", dataS));
  dataS = madLong2Short((data >> 16)&0xffff);
  MAD_DBG_ERROR(("RW U1 p0 R3 h%x\n", dataS));
  /*MAD_DBG_ERROR(("msecWriteReg: Addr %x Data %x\n", addr, (unsigned int)data));*/
}
...
/* page number 11 and register 0~3 are defined for indirect access to MACsec address space:
  ...
  */
...
#ifdef MSEC_USE_PLATFORM
  {
    ...
    retVal = madSmiIfWriteRegister(dev,port, 0, addr);
    if(retVal != MAD_OK)
    {
      MAD_DBG_ERROR(("Failed to write register \n"));
      return MAD_DRV_FAIL_WRITE_REG;
    }
    retVal = madSmiIfReadRegister(dev, port, 2, &dataL);
    if(retVal != MAD_OK)
    {
      MAD_DBG_ERROR(("Failed to read register \n"));
      return MAD_DRV_FAIL_WRITE_REG;
    }
    retVal = madSmiIfReadRegister(dev, port, 3, &dataH);
    if(retVal != MAD_OK)
    {
      MAD_DBG_ERROR(("Failed to read register \n"));
      return MAD_DRV_FAIL_WRITE_REG;
...
if (DEBUG_MSEC_READ == 1)
  MAD_DBG_ERROR(("msecReadReg: Port %d Addr %x Data %x\n", port, addr, (unsigned int)*data));
  return MAD_OK;
  /*return dataL; */
}
...
#ifdef MSEC_USE_PLATFORM
  {
    retVal = madSmiIfWriteRegister (dev, port, 22, 253);
    if(retVal != MAD_OK)
    {
      MAD_DBG_ERROR(("Failed to write register \n"));
      return MAD_DRV_FAIL_WRITE_REG;
    }
    retVal = madSmiIfWriteRegister (dev, port, 13, 0xc000);
    if(retVal != MAD_OK)
    {
      MAD_DBG_ERROR(("Failed to write register \n"));
      return MAD_DRV_FAIL_WRITE_REG;
    }
    retVal = madSmiIfWriteRegister (dev, port, 13, 0x8000);
    if(retVal != MAD_OK)
    {
      MAD_DBG_ERROR(("Failed to write register \n"));
      return MAD_DRV_FAIL_WRITE_REG;
...
MAD_STATUS msec_port_get_macsec_stat_value (MAD_DEV *dev,  int port, int stat_name, MAD_U32 *data)
{
  ...
  retVal = msecReadReg (dev, port, stat_name, data);
  if(retVal != MAD_OK)
  {
...
MAD_STATUS msec_clr_macsec_stat (MAD_DEV *dev, int port)
{
  MAD_STATUS   retVal;
  MACSEC_STAT_T  macsec_stat;
  int            orig_portnum;
...
MAD_STATUS msec_clr_mib_stat (MAD_DEV *dev, int port)
{
  MAD_STATUS   retVal;
  MIB_STAT_T  mib_stat;
  int         orig_portnum;
...
)
{
  ...
  MAD_U16 data;
  port /= 4;
  if((retVal = madHwReadPagedPhyReg(dev,port,253,13,&data)) != MAD_OK)
  {
    MAD_DBG_ERROR(("Writing to paged phy reg (%i,%i) failed.\n",0,1));
    return (retVal | MAD_API_FAIL_WRITE_PAGED_REG);
  }
  ...
  if((retVal = madHwWritePagedPhyReg(dev,port,253,13,0x800))!= MAD_OK)
  {
    MAD_DBG_ERROR(("Writing to paged phy reg (%i,%i) failed.\n",0,2));
    return (retVal | MAD_API_FAIL_WRITE_PAGED_REG);
  }
...
)
{
  ...
  if((retVal = madHwWritePagedPhyReg(dev,port,253,13,0x0))!= MAD_OK)
  {
    MAD_DBG_ERROR(("Writing to paged phy reg (%i,%i) failed.\n",0,2));
    return (retVal | MAD_API_FAIL_WRITE_PAGED_REG);
  }
...
)
{
  ...
  if (en==1)
  {
    if((retVal = msecReadPortReg (dev, port/4, SLC_RAM_CTRL1(dev), &data))!= MAD_OK)
    {
      MAD_DBG_ERROR(("Reading to paged phy reg (%i,%i) failed.\n",0,1));
      return (retVal | MAD_API_FAIL_WRITE_PAGED_REG);
    }
    bit_clear (data, 13);
    if((retVal = msecWritePortReg (dev, port/4, SLC_RAM_CTRL1(dev), data))!= MAD_OK)
    {
      MAD_DBG_ERROR(("Writing to paged phy reg (%i,%i) failed.\n",0,1));
      return (retVal | MAD_API_FAIL_WRITE_PAGED_REG);
    }
