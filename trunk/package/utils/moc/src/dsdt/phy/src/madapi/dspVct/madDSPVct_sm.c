...
typedef struct {
  ...
  int gain_substate;  /* gain sub state = MAD_DSPVCT_GAIN_L_ID x MAD_DSPVCT_GAIN_M_ID */
...
static MAD_STATUS madDspVctAct_gain(MAD_DEV *dev, int port, MAD_DSP_VCT_RESULT_STRUCT *madDspVctResultP, MAD_DSPVCT_SM_CFG *madDspVctSmCfgP);
...
static MAD_STATUS madDspVctAct_init(MAD_DEV *dev, int port, MAD_DSP_VCT_RESULT_STRUCT *madDspVctResultP, MAD_DSPVCT_SM_CFG *madDspVctSmCfgP)
{
  ...
  madDspVctSmCfgP->gain_substate = 0;
...
/*	MAD_CRITIC_INFO(("@@@ setting done, wait reg 17_0 = 0x400 \n")); */
    if((retVal = madHwReadPagedPhyReg(dev,port ,0, 17, &data)) != MAD_OK)
	{
      MAD_DBG_ERROR(("Reading paged phy reg failed.\n"));
      return (retVal | MAD_API_FAIL_GET_PAGED_REG_FIELD);
	}
    if( (data&0x0400)==0x400) 
	{
      madDspVctSmCfgP->currentSt=DSPVCT_GAIN_ST;
      madDspVctSmCfgP->gain_substate = 0;
      return madDspVctAct_gain(dev, port, madDspVctResultP, madDspVctSmCfgP);
	}
	else
	{
      return MAD_PENDING;
...
static MAD_STATUS madDspVctAct_gain(MAD_DEV *dev, int port, MAD_DSP_VCT_RESULT_STRUCT *madDspVctResultP, MAD_DSPVCT_SM_CFG *madDspVctSmCfgP)
{
  ...
  unsigned short data;
  MAD_STATUS  retVal;
  if(madDspVctSmCfgP->currentSt==DSPVCT_GAIN_ST)
  {
    if(madDspVctSmCfgP->gain_substate==0)
	{
	...
	l = madDspVctSmCfgP->gain_substate /MAD_DSPVCT_GAIN_M_ID;
	m = madDspVctSmCfgP->gain_substate %MAD_DSPVCT_GAIN_M_ID;

	temp = 8 +l;
  
    if((retVal = madHwWritePagedPhyReg(dev,port,0xff, 16,0x1110+temp)) != MAD_OK)
	{
...
/*	 MAD_CRITIC_INFO(("@@@ wait 16_ff = 0x8000\n")); */
	...
	{
      madDspVctSmCfgP->gain_substate += 1;
...
/*	  MAD_CRITIC_INFO(("@@@ regGain[%d][%d] = %d \n", l, m, madDspVctResultP->regGain[l][m])); */
      if(madDspVctSmCfgP->gain_substate>=(MAD_DSPVCT_GAIN_L_ID*MAD_DSPVCT_GAIN_M_ID))
	  {
...
)
{
  ...
  {
    MAD_CRITIC_INFO(("@@@ Start madDspVct (pending mode) collect data \n"));
    madDspVctSmCfgP->currentSt=DSPVCT_INIT_ST;
...
/* MAD_CRITIC_INFO(("@@@ Start madDspVct_sm (pending mode) collect data \n")); */
