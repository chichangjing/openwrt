...
)
{
    ...
    if(!(MAD_DEV_PORT_CHECK(dev, hwPort)))
    {
      MAD_DBG_INFO(("The port %i is over range.\n", (int)port));
      return MAD_API_ERROR_OVER_PORT_RANGE;
    }
...
*
* DESCRIPTION:
*       This routine transltes auto nego status, speed, and duplex mode into auto nego mode.
*
* INPUTS:
        ...
        switch (speedMode)
        {
            ...
            case MAD_SPEED_AUTO:
                *autoMode = MAD_AN_MOD_AUTO_SPEED_AUTO_DUPLEX;
                break;
            default:
            /* MAD_DBG_ERROR(("Get wrong speed status error\t"));  */
                return MAD_FAIL;
        }
    }
...
)
{
  ...
  MAD_DBG_INFO(("mdCopperSetSpeedDuplex called.\n"));
  switch (speed)
  {
    case 10:
        if (duplex==MAD_TRUE)
...
)
{
    ...
    MAD_DBG_INFO(("mdCopperGet1000TMasterMode called.\n"));
...
)
{
    ...
    if((retVal=madHwReadPagedPhyReg(dev,(MAD_U8)portNum,MAD_PAGE0,
...
)
{
    ...
    if((retVal=madHwWritePagedPhyReg(dev,(MAD_U8)portNum, MAD_PAGE0,
                                (MAD_U8)regAddr,(MAD_U16)data)) != MAD_OK)
    {
