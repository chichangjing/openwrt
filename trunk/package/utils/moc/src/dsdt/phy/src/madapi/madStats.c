...
)
{
    ...
    {
        MAD_DBG_ERROR(("The unknown mode not supported.\n"));
...
)
{
    ...
    MAD_DBG_INFO(("mdStatsGetFrameCounter called.\n"));
...
)
{
    ...
    if(!MAD_DEV_CAPABILITY1(dev, MAD_PHY_RX_ER_BYTE_CAPTURE))
    {
        MAD_DBG_ERROR(("feature not supported.\n"));
        return MAD_API_CRC_FRM_COUNT_NOT_SUPPORT;
    }
