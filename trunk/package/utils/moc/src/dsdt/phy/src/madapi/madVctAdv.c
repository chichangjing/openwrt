...
)
{
    ...
    if (u16Data & 0x8000)
    {
        MAD_DBG_INFO(("Reading MAD_REG_ADV_VCT_CONTROL_5 is %x.\n", u16Data));
        return MAD_PENDING;
    }
...
)
{
  ...
  MAD_32  channel;
  int  count;
  #define  MAD_ADVVCT_CHK_NUM_MAX      2000
...
/*  for (channel=0; channel<MAD_MDI_PAIR_NUM; channel++) */
  {
     ...
     * loop until test completion and result is valid
     */
    count = MAD_ADVVCT_CHK_NUM_MAX;
    do 
    {
      if (((retVal=runAdvCableTest_1240_check(dev,hwPort)) != MAD_OK)&&
            ((retVal&MAD_RUN_ST_MASK) != MAD_PENDING))
      {
        MAD_DBG_ERROR(("Running advanced VCT failed, check error.\n"));
        return (retVal | MAD_API_FAIL_ADVVCT_SM_CHECK);
      }
      if((count--)==0)
      {
...
/*        case MAD_88E1510:	old 88E1510 */
        ...
        case MAD_88E1680M: 
         ...
         break;
        default:
            MAD_DBG_ERROR(("Running advanced VCT failed error dev id %x.\n", dev->deviceId));
            return MAD_API_ADVTDRVCT_NOT_SUPPORT;
    }
