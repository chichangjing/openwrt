...
typedef enum
{
	...
	GT_88E6320    = 0x115, /* Pearl */
    GT_88E6321    = 0x310, /* Pearl */

    GT_88E6999    = 0x999, /* Opus Temp definition(won't be used)*/

	GT_88E6390    = 0x390,  /* Peridot Full Featured Device  */
	GT_88E6390X	  = 0x0a1,
	GT_88E6360    = 0x360,  /* Peridot 5 GE PHY version  */
	GT_88E6290    = 0x290,  /* Peridot FE PHY version  */
	GT_88E6290A   = 0x260,  /* Peridot Forced FE PHY (no AutoNeg) for Automotive */
	GT_88E6191    = 0x191,  /* Peridot No AVB */
	GT_88E6190    = 0x190,   /* Peridot No AVB, Z80 NIC, TCAM nor Cut Through */
	GT_88E6190X	  = 0x0a0
...
typedef enum
{
	...
	PORT_SPEED_10_GBPS = 3,		/* valid only if device support */
...
typedef enum
{
    ...
    PORT_FORCE_SPEED_10_GBPS = 3,     /* valid only if device support (Opus and later)*/
 ...
 *        exPrio     - extended priority information. If device support extended 
...
*/
#define GT_VTU_TABLE_FULL_VIOLATION    0x8
 ...
 *                        88E6097, 88E6096, ...and mode of non frame, such as
...
#define GT_PIRL3_COUNT_MODE GT_PIRL2_COUNT_MODE
 ...
 *  Fields:
 *      accountQConf    - account discarded frames due to queue congestion
 *      accountFiltered - account filtered frames. Account for All (after Agate)
...
/*
 *  typedef: struct GT_PIRL3_RESOURCE
 *
 *  Description: data structure that represents a PIRL Resource
 *
 *  Fields:
 *      accountQConf    - account discarded frames due to queue congestion
 *      accountFiltered - Account for All (after Agate)
 *      samplingMode    - sample one out of so many frames/bytes for a stream of frames
 *      colorAware      - 0 = Color Blind mode, 1 = Color Aware mode
 *      acct4GrnOvflow  - Account for Green Overflow
 *      actionMode        - action should be taken when there are not enough tokens
 *                          to accept the entire incoming frame
 *                                PIRL_ACTION_ACCEPT - accept the frame
 *                                PIRL_ACTION_USE_LIMIT_ACTION - use limitAction
 ...
 *                                  GT_TRUE - typeMask OR priMask
 *                                  GT_FALSE - typeMask AND priMask
 *      framePrio       - 0 = use the QPri assigned to the frame, 1 = use the FRri assigned to the frame
 *      priSelect       - Priority Select. For each PriSelect bit:
 *                              0 = Don�t account for this priority
 *                              1 = Account for this priority
 *                        priority bit mask that each bit indicates one of the four
 *                        queue priorities. Setting each one of these bits indicates
 *                        that this particular rate resource is slated to account for
 *                        incoming frames with the enabled bits' priority.
 *      byteTobeCounted    - bytes to be counted for accounting
 ...
 *                                GT_PIRL2_COUNT_ALL_LAYER2, or
 *                                GT_PIRL2_COUNT_ALL_LAYER3
 *      tcamFlow           - TCAM Flows. Enable the TCAM for Flow identifiers for Flows 0 to 3.
 *      bktIncrement    - the amount of tokens that need to be added for each 
 *                          byte of packet information.
 *      bktRateFactorGreen - bucket rate factor Green = bucketDecrement/updateInterval,
 *                          where updateInterval indicates the rate at which the 
 *                          bucket needs to be updated with tokens, or 1/CIR, 
 *                          where CIR is the committed information rate in kbps.
 *                          bucketDecrement indicates the amount of tokens that 
 ...
 *      cbsLimit        - Committed BUrst Size limit (expected to be 2kBytes)
 *      ebsLimit        - Excess Burst Size limit ( 0 ~ 0xFFFFFF)
 *      bktRateFactorYel- bucket rate factor Yellow = bucketDecrement/updateInterval,
 *                          where updateInterval indicates the rate at which the 
 *                          bucket needs to be updated with tokens, or 1/CIR, 
 *                          where CIR is the committed information rate in kbps.
 *                          bucketDecrement indicates the amount of tokens that 
 *                          need to be removed per each bucket decrement.
 *      fcPriority      - Flow Control Priority.
 *      fcMode          - Flow Control Mode.
 *      fcAction        - Flow Control Action.
 *                        0 = Frames get discarded when IRL limits reached
 *                        1 = Flow Control gets asserted when IRL limits reached
 *      mgmtNrlEn       - exclude management frame from ingress rate limiting calculation
 *      saNrlEn         - Source Address AVB / Non Rate Limit enable.
 *                        exclude from ingress rate limiting calculation if the SA of the
 *                        frame is in ATU with EntryState that indicates Non Rate Limited.
 *      daNrlEn         - Destination Address AVB / Non Rate Limit enable.
 *                        exclude from ingress rate limiting calculation if the DA of the
 *                        frame is in ATU with EntryState that indicates Non Rate Limited.
 *      bktTypeMask     - used if bktRateType is BUCKET_TYPE_TRAFFIC_BASED. 
 *                        any combination of the following definitions:
 *                          BUCKET_TRAFFIC_UNKNOWN_UNICAST,
...
typedef struct
{
    ...
    GT_ESB_LIMIT_ACTION ebsLimitAction;
    GT_BOOL             colorAware;
    GT_BOOL             acct4GrnOvflow;
    GT_BOOL             priORpt;
    GT_BOOL             framePrio;
    GT_U8               priSelect;
    GT_PIRL3_COUNT_MODE byteTobeCounted;
    GT_BOOL             tcamFlow;
    GT_U32              bktIncrement;
    GT_U32              bktRateFactorGreen;
    GT_U32              ebsLimit;
    GT_U32              cbsLimit;
    GT_U32              bktRateFactorYel;
    GT_U8               fcPriority; /*      - Flow Control Priority. */
    GT_PIRL_FC_DEASSERT fcMode;
    GT_U8               fcAction;  /*        - Flow Control Action. */
    GT_BOOL             mgmtNrlEn;
    GT_BOOL             saNrlEn;
    GT_BOOL             daNrlEn;
    GT_U32              bktTypeMask;
...
#define GT_PIRL3_OP_DATA GT_PIRL2_OP_DATA
#define GT_PIRL3_OPERATION GT_PIRL2_OPERATION
...
typedef enum
{
    GT_AVB_BLOCK_PTP        = 0x0,
    GT_AVB_BLOCK_AVB_POLICY = 0x1,
    GT_AVB_BLOCK_QAV        = 0x2,
    GT_AVB_BLOCK_QBV        = 0x3
} GT_AVB_BLOCK;
...
/* PTP global configuration by pointer/data set. After Opus */
...
typedef enum
{
    GT_PTP_GLOBAL_CONFIG_PTP_MODE_REG                  = 0x0,
    GT_PTP_GLOBAL_CONFIG_PTP_EXTERNAL_TIME_FORM        = 0x1,
    GT_PTP_GLOBAL_CONFIG_1588_ING_ACT_VEC              = 0x60,
    GT_PTP_GLOBAL_CONFIG_1588_EG_ACT_VEC               = 0x68,
    GT_PTP_GLOBAL_CONFIG_AS_ING_ACT_VEC                = 0x70,
    GT_PTP_GLOBAL_CONFIG_AS_EG_ACT_VEC                 = 0x78
} GT_PTP_GLOBAL_CONFIG_POINTER;

/*
 *  typedef: struct GT_PTP_MODE_CONFIG
 *
...
/*
 *  typedef: struct GT_PTP_EXTERNAL_TIME_FORM
 *
 ...
 *
 * Fields:
 *      GT_PTP_TS_MODE_IN_REG         - Time stamp in TS register (original)
 *      GT_PTP_TS_MODE_IN_RESERVED_2  - Time stamp in Frame reserved 2
...
/*
 *  typedef: struct GT_PTP_INTERRUPT_STATUS
 *
...
typedef struct
{
    GT_U8     ptpTrigGenInt;
    GT_U8     ptpEventInt;
    GT_U8     ptpUpperPortInt;
    GT_U8     ptpResInt;
    GT_U32    ptpIntStPortVect;
} GT_PTP_INTERRUPT_STATUS;

/*
 * Typedef: enum GT_PTP_TOD_OP
 *
 * Description: Defines the PTP (Precise Time Protocol) Time of Day Opcode.
 *
 * Fields:
 *  GT_PTP_TOD_OP_COMP_REG_TS_ARRAY = Store Comp register only to selected TimeArray (clean148)
 *  GT_PTP_TOD_OP_ALL_REG_TS_ARRAY = Store All Registers to selected TimeArray @ ToDLoadPt (dirty149)
 *  GT_PTP_TOD_OP_COMP_REG_TS_ARRAY_150 = Store Comp register only to selected TimeArray150 (dirty)
 *  GT_PTP_TOD_OP_ALL_REG_TS_ARRAY_151 = Store All Registers to selected TimeArray @ ToDLoadPt151 (clean)
 *  GT_PTP_TOD_OP_CAP_TS_ARRAY_COMP = Capture selected TimeArray, Comp=Comp & ToDLoadPt=PTPGT152
 *  GT_PTP_TOD_OP_CAP_TS_ARRAY_ARRERR = Capture selected TimeArray, Comp=AccErr & ToDLoadPt=PTPGT153 
*/
typedef enum
{
  GT_PTP_TOD_OP_COMP_REG_TS_ARRAY,
  GT_PTP_TOD_OP_ALL_REG_TS_ARRAY,
  GT_PTP_TOD_OP_COMP_REG_TS_ARRAY_150,
  GT_PTP_TOD_OP_ALL_REG_TS_ARRAY_151,
  GT_PTP_TOD_OP_CAP_TS_ARRAY_COMP,
  GT_PTP_TOD_OP_CAP_TS_ARRAY_ARRERR
} GT_PTP_TOD_OP;

/*
 *  typedef: struct GT_PTP_TOD_OP_CTRL
 *
...
typedef struct
{
    GT_U8     ptpOpCtrlTodOp;
    GT_U8     ptpOpCtrlTimeArray;
    GT_U8     ptpOpCtrlClkValid;
    GT_U8     ptpOpCtrlDomainNum;
} GT_PTP_TOD_OP_CTRL;

/*
 *  typedef: struct GT_PTP_PULSE_STRUCT
 *
...
typedef struct
{
    ...
    GT_BOOL        filterAct;    /* Filter 802.1 Act from LEDs */
    GT_BOOL        hwAccPtp;     /* Hardware Accelerated PTP */
    GT_BOOL        kpFramwSa;    /* KS = Keep Frame�s SA */
    GT_BOOL        fullCheck;    /* Full check  */
    GT_BOOL        noCorrupt;    /* do not corrupt */
	GT_BOOL		   extHwAcc;	 /* External Hardware Acceleration */
    GT_PTP_TS_MODE    arrTSMode;
    GT_U8         arrLedCtrl; /*LED control for packets entering the device. */
    GT_U8         depLedCtrl; /*LED control for packets departing the device. */
} GT_PTP_PORT_CONFIG;
 ...
 *		PTP_ARR2_TIME			- PTP Arrival Time 2
 */
typedef enum
{
    PTP_ARR0_TIME = 0x0,
    PTP_ARR1_TIME = 0x1,
    PTP_DEP_TIME = 0x2,
    PTP_ARR2_TIME = 0x3  /* After Opus */
} GT_PTP_TIME;

/*
 * Typedef: enum GT_PTP_EG_ACT_VEC
 *
...
typedef enum
{
    ...
    GT_PTP_EG_SYNC_KEEP_RX_DATA		= 0x1,
    GT_PTP_EG_SYNC_RESIDENCE_TIME	= 0x2,
    GT_PTP_EG_SYNC_EG_TS			= 0x4,
	GT_PTP_EG_FOLLOW_UP_RES_TIME	= 0x20,
    GT_PTP_EG_FOLLOW_UP_EG_TS		= 0x40,
	/* Egress Action Vector 1 */
	GT_PTP_EG_PDELAY_RESP_KP_RX_DATA			= 0x100,
    GT_PTP_EG_PDELAY_RESP_RES_TIME				= 0x200,
    GT_PTP_EG_PDELAY_RESP_EG_TS_CRT				= 0x800,
	GT_PTP_EG_PDELAY_RESP_FOLLOW_UP_RES_TIME	= 0x2000,
	GT_PTP_EG_PDELAY_RESP_FOLLOW_UP_EG_TS		= 0x4000,
    GT_PTP_EG_PDELAY_RESP_FOLLOW_UP_EG_TS_CRT	= 0x8000,
	/* Egress Action Vector 2 */
	GT_PTP_EG_PDELAY_REQ_KEEP_RX_DATA	= 0x10000,
	GT_PTP_EG_PDELAY_REQ_RES_TIME		= 0x20000,
	GT_PTP_EG_PDELAY_REQ_EG_TS			= 0x40000,
	/* Egress Action Vector 3 */
	GT_PTP_EG_DELAY_REQ_KEEP_RX_DATA	= 0x10000000,
	GT_PTP_EG_DELAY_REQ_RES_TIME		= 0x20000000,
	GT_PTP_EG_DELAY_REQ_EGR_TS			= 0x40000000
} GT_PTP_EG_ACT_VEC;
...
{
    GT_U8    depTSDisCount;
    GT_U8    depNOTSDisCount;
    GT_U8    arrTSDisCount;
    GT_U8    arrNOTSDisCount;
} GT_PTP_TS_DISCARD_COUNTER;
...
typedef enum
{
    GT_TAI_TRIG_PERIODIC_PULSE = 0,    /* generate periodic pulse */
    GT_TAI_TRIG_ON_GIVEN_TIME        /* generate pulse when 
...
/*
 *  typedef: struct GT_TAI_CLOCK_CONFIG
 *
 ...
 *      soCClkPer     - System on a chip Clock Period.
 *      soCClkComp    - System on a Chip Clock Compensation Amount in pico seconds.
 *		irlClkGenAmt  - IRL Clock Generation Amount.
 *		irlClkComp	  - IRL Clock Compensation Amount.
 */
typedef struct
{
	GT_BOOL   irlClkEn;
	GT_BOOL   compDir;
	GT_U8     irlClkCompSubPs;
    GT_U16    tsClkPer;
    GT_U16    soCClkPer;
    GT_U16    soCClkComp;
	GT_U16    irlClkGenAmt;
 ...
 *      trigGenEn     - Trigger Generation Request enable
 ...
 *      lockCorrect2  - Trig Lock 2 Correction amount 
 *      irlLockCorrect  - IRL clk Correction amount
 *      trigClkCompDir  - Trigger Clock Comp Direction. 
 *      trig2Select     - Trigger Triger2 Signal selection. 
 ...
 *      mode        - trigger mode, either GT_TAI_TRIG_PERIODIC_PULSE or 
 *                        GT_TAI_TRIG_ON_GIVEN_TIME
 *      trigGenReq      - Trigger Generation Request. 
 *      trigGenAmt      - if mode is GT_TAI_TRIG_PERIODIC_PULSE,
 ...
 *                      this value is valid only in GT_TAI_TRIG_PERIODIC_PULSE mode.
...
typedef struct
{
	...
	GT_BOOL   trigWindow;
	GT_BOOL   trig2Signal;
	GT_BOOL   irlCorrValid;
    GT_U8     trigLockRange;
    GT_U8     lockCorrect;
    GT_U8     lockCorrect2;
	GT_U8     irlLockCorrect;
    GT_U8     trigGenReq;
    GT_U8     trigClkCompDir;
    GT_U8     trig2Select;
	GT_U8     clkCompDir;
    GT_U8     pulseWidth;
    GT_U8     pulseWidthRange;
    GT_TAI_MULTI_PTP_SYNC_MODE  multiSyncMode;
    GT_TAI_TRIG_MODE            mode;
    GT_U32    trigGenAmt;
    GT_U32    trigClkComp;
    GT_U32    trigClkCompSub;
    GT_U32    incDecAmt;
    GT_U32    trigGenTime;
    GT_U32    trigGenDelay;
    GT_U32    trigGen2Time;
...
typedef enum
{
    ...
    GT_AVB_MID_HI_FPRI,        /* AVB Medium Hi Frame Priority */
    GT_AVB_MID_HI_QPRI,        /* AVB Medium Hi Queue Priority */
    GT_AVB_MID_LO_FPRI,        /* AVB Medium Lo Frame Priority */
    GT_AVB_MID_LO_QPRI,        /* AVB Medium Lo Queue Priority */
    GT_LEGACY_MID_HI_FPRI,    /* Legacy Medium Hi Frame Priority */
    GT_LEGACY_MID_HI_QPRI,    /* Legacy Medium Hi Queue Priority */
    GT_LEGACY_MID_LO_FPRI,    /* Legacy Medium Lo Frame Priority */
    GT_LEGACY_MID_LO_QPRI    /* Legacy Medium Lo Queue Priority */
} GT_AVB_PRI_TYPE;
...
typedef struct {
      unsigned short avbMode:2;	/* */
      unsigned short avbOverride:1;	/* */
      unsigned short filterbadAvb:1;	/* */
      unsigned short avbTunnel:1;	/* */
      unsigned short disBadAvb:1;	/* */
      unsigned short resv1:4;	/* */
      unsigned short ingAllAvb:1;	/* */
      unsigned short egAllAvb:1;	/* */
      unsigned short avbHiPolicy:2;	/* */
      unsigned short avbLowPolicy:2;	/* */
} GT_AVB_POLICY_STRUCT; 

typedef struct _GT_AVB_CONFIG
{
    struct {
        unsigned char hiFPri;
        unsigned char loFPri;
        unsigned char hiQPri;
        unsigned char loQPri;
    } avbPri, legacyPri, avbMidPri, legacyMidPri; 	/**< INPUT param if set, OUTPUT param if get */
    unsigned int avbHLimit;	/**< INPUT param if set, OUTPUT param if get */
    unsigned char avbOuiByte[3];	/**< INPUT param if set, OUTPUT param if get */
    GT_AVB_POLICY_STRUCT avbPolicy;	/**< INPUT param if set, OUTPUT param if get */
} GT_AVB_CONFIG;

typedef struct _GT_QAV_CONFIG
{
  struct {
    unsigned short qpri_dblock:1;
    unsigned short qpri_rate:15;
    unsigned short qpri_timeAwareBlock:1;
    unsigned short qpri_hilimit:15;
  } qavPriData[4]; 
  unsigned short admin_mgmt:1;
  unsigned short res_0:5;
  unsigned short global_iso_ptrs:10;
  unsigned char iso_ints;
  unsigned char iso_int_en;
  unsigned char iso_int_port;
  unsigned char iso_mon_en;
  unsigned char iso_mon_port;
  unsigned char iso_hi_discard_count;
  unsigned char iso_lo_discard_count;
  unsigned char q_num;
} GT_QAV_CONFIG;

typedef struct
{
    GT_U8  use_def_que;  /* after Opus the bit is useless */
    GT_U8  def_que;      /* after Opus que is 3 bits rather than 2 bits */
} GT_DEF_QUE_PRI;
...
{
    TCAM_MODE_DISABLE    = 0,
    TCAM_MODE_ENABLE_48  = 1,
    TCAM_MODE_ENABLE_96  = 2,
} GT_TCAM_MODE;
...
#define     TCAM_PURGE_ENTRY     TCAM_LOAD_ENTRY
...
/* The P2 register 31 work as follows in the following example sequence:
 ...
 *      lowHit       - TCAM Entry for Low 48-byte Hit..
 *   after 6390  
 *      dpvDataHi;
 *      egActPoint;
 *      unKnownFilter;
...
typedef union
{
  ...
  /* Pg0 registers */
    ...
    GT_U16       spvRes:3;             /* used after 6390 */
    GT_U16       type0Res:3;
    GT_U16       frame0Type:2;
    GT_U16       spvMaskHi:3;            /* used for  SPV[10:8] after 6390  */
    GT_U16       spvMaskHiRes:3;            /* used for  SPV[10:8] after 6390  */
    GT_U16       maskType:2;           /* used for mask type   */
...
typedef union
{
  ...
  /* Pg2 registers */
    ...
    GT_U16       vidData:12;
    GT_U16       vidOverride:1;
    GT_U16       IncTcamCtr:1;
    GT_U16       interrupt:1;
    GT_U16       continu:1;

    GT_U16        fpriData:3;
    GT_U16        fpriOverride:1;
    GT_U16        qpriData:3;
    GT_U16        qpriOverride:1;
    GT_U16        nextId:8;

    GT_U16        dpvData:11;
    GT_U16        dpvSF:1;
    GT_U16        qpriAvbData:3;
    GT_U16        qpriAvbOverride:1;

    GT_U16        ldBalanceData:3;
    GT_U16        ldBalanceOverride:1;
    GT_U16        factionData:11;
    GT_U16        factionOverride:1;
    ...
    GT_U16        DSCP:6;
    GT_U16        ldBalanceRes:6;
    GT_U16        ldBalanceData1:3;
    GT_U16        ldBalanceOverride1:1;

    GT_U16        factionData1:15;
    GT_U16        factionOverride1:1;
...
typedef union
{
  ...
  /* Pg3 registers */
    ...
    GT_U16       saMode:3;
    GT_U16       saModeOverride:1;
    GT_U16       daMode:2;
    GT_U16       daModeRes:2;
    GT_U16       tagMode:2;
    GT_U16       tagModeRes:2;
    GT_U16       frameMode:2;
    GT_U16       frameModeRes:2;

    GT_U16       egVidData:12;
    GT_U16       egVidMode:2;
    GT_U16       egVidModeRes:2;

    GT_U16       egFPRI:3;
    GT_U16       egEc:1;
    GT_U16       FPRIMode:2;
    GT_U16       FPRIModeRes:2;
    GT_U16        egDSCP:6;
    GT_U16        egDSCPMode:2;
...
typedef struct 
{
  ...
  /* Pg3 registers. after 6390 */
  GT_TCAM_FRM3_DATA frame3;
} GT_TCAM_FRM_DATA;
...
typedef struct 
{
  GT_TCAM_FRM_DATA  rawFrmData[2];  /* first part is 0-48 bytes of frame */
    ...
    GT_U8        dpvMode;
    GT_U8        DSCP;
    GT_U8        ldBalanceData1;
    GT_U8        ldBalanceOverride1;
    GT_U16       factionData1;
    GT_U8        factionOverride1;
    GT_U8        saMode;
    GT_U8        daMode;
    GT_U8        tagMode;
    GT_U8        frameMode;
 ...
 *      serPortList  - SERDES port list where GT_DEV_INT_SERDES_LINK interrupt needs
...
*    GT_DEV_INT_WAKE_EVENT     - Wake Event interrupt.
...
#define GT_DEV_INT_SERDES_LINK         0x20
 ...
 *										scheduling
 *	       GT_PORT_SCHED_STRICT_PRI7 - use a strict for priority 7 and weighted 
 *										round robin for priorities 6,5,4,3,2,1 and 0
 *		   GT_PORT_SCHED_STRICT_PRI7_6 - use a strict for priority 7,6 and weighted 
 *										round robin for priorities 5,4,3,2,1 and 0
 *		   GT_PORT_SCHED_STRICT_PRI7_6_5 - use a strict for priority 7,6,5 and weighted
 *										round robin for priorities 4,3,2,1 and 0
 *		   GT_PORT_SCHED_STRICT_PRI7_6_5_4 - use a strict for priority 7,6,5,4 and 
 *										weighted round robin for priorities 3,2,1 and 0
 *		   GT_PORT_SCHED_STRICT_PRI7_6_5_4_3 - use a strict for priority 7,6,5,4,3 and 
 *										weighted round robin for priorities 2,1 and 0
 *		   GT_PORT_SCHED_STRICT_PRI7_6_5_4_3_2 - use a strict for priority 7,6,5,4,3,2 
...
typedef enum
{
	...
	GT_PORT_SCHED_WEIGHTED_RRB_1 = 0x0,
	GT_PORT_SCHED_STRICT_PRI7,
	GT_PORT_SCHED_STRICT_PRI7_6,
	GT_PORT_SCHED_STRICT_PRI7_6_5,
	GT_PORT_SCHED_STRICT_PRI7_6_5_4,
	GT_PORT_SCHED_STRICT_PRI7_6_5_4_3,
	GT_PORT_SCHED_STRICT_PRI7_6_5_4_3_2,
	GT_PORT_SCHED_STRICT_PRI_1
} GT_PORT_SCHED_MODE;
...
typedef GT_STATUS (*FGT_SEM_CREATE)(
        GT_SEM * semId,
...
{
  GT_U32 cmd;  /*INPUT:HW_REG_READ, HW_REG_WRITE, HW_REG_WAIT_TILL_0 or HW_REG_WAIT_TILL_1 */
  GT_U32 addr; /*INPUT:SMI Address */
  GT_U32 reg;  /*INPUT:Register offset */
  GT_U32 data; /*INPUT,OUTPUT:Value in the Register or Bit number */
} HW_DEV_RW_REG;
...
typedef GT_STATUS (*RMU_TX_RX_PAK)(
						GT_U8  *req_pkt,
						GT_UINT  req_pkt_len,
						GT_U8  **rsp_pkt,
						GT_UINT  *rsp_pkt_len);
...
#ifdef GT_RMGMT_ACCESS
    FGT_READ_MII     rmuReadMii;      /* read MII Registers using RMU */
    FGT_WRITE_MII    rmuWriteMii;    /* write MII Registers using RMU*/
    RMU_TX_RX_PAK    rmuTxRxPacket; /* send and receive rmu_packet */
...
*/
typedef struct _GT_STATS_COUNTER_SET4
{
	/* Bank 0 */
    GT_U32    InGoodOctetsLo;     /* offset 0 */
    ...
    */
    ...
    GT_U32    InDroppedAvbA;    /* For Opus, reserved_c for Pearl; offset 0x0c */
    GT_U32    InDroppedAvbB;     /* For Opus, reserved_d for Pearl; offset 0x0d */
    ...
    GT_U32    OutYellow;         /* For Opus, reserved_1c for Pearl; offset 0x1c */
    GT_U32    OutDroppedYel;    /* For Opus, reserved_1d for Pearl; offset 0x1d */
    GT_U32    OutDiscards;      /* For Opus, reserved_1e for Pearl; offset 0x1e */
    GT_U32    OutMGMT;          /* offset 0x1f */

} GT_STATS_COUNTER_SET4;


#define GT_TYPE4_BANK1 0x80
typedef enum
{
	/* Bank 0 */
    STATS4_InGoodOctetsLo = 0,
    STATS4_InGoodOctetsHi,
    STATS4_InBadOctets,
    
    STATS4_OutFCSErr,
    STATS4_InUnicasts,
    STATS4_Deferred,            /* offset 5 */
    STATS4_InBroadcasts,
    STATS4_InMulticasts,
    STATS4_64Octets,
    STATS4_127Octets,
    STATS4_255Octets,            /* offset 10 */
    STATS4_511Octets,
    STATS4_1023Octets,
    STATS4_MaxOctets,
    STATS4_OutOctetsLo,
    STATS4_OutOctetsHi,
    STATS4_OutUnicasts,        /* offset 16 */
    STATS4_Excessive,
    STATS4_OutMulticasts,
    STATS4_OutBroadcasts,
    STATS4_Single,
    STATS4_OutPause,
    STATS4_InPause,
    STATS4_Multiple,
    STATS4_Undersize,            /* offset 24 */
    STATS4_Fragments,
    STATS4_Oversize,
    STATS4_Jabber,
    STATS4_InMACRcvErr,
    STATS4_InFCSErr,
    STATS4_Collisions,
    STATS4_Late,                    /* offset 31 */
	/* Bank 1 */
    STATS4_InDiscards      = GT_TYPE4_BANK1+0x00,
    STATS4_InFiltered      = GT_TYPE4_BANK1+0x01,
    STATS4_InAccepted      = GT_TYPE4_BANK1+0x02,
    STATS4_InBadAccepted   = GT_TYPE4_BANK1+0x03,
    STATS4_InGoodAvbClassA = GT_TYPE4_BANK1+0x04,
    STATS4_InGoodAvbClassB = GT_TYPE4_BANK1+0x05,
    STATS4_InBadAvbClassA  = GT_TYPE4_BANK1+0x06,
    STATS4_InBadAvbClassB  = GT_TYPE4_BANK1+0x07,
    STATS4_TCAMCounter0    = GT_TYPE4_BANK1+0x08,
    STATS4_TCAMCounter1    = GT_TYPE4_BANK1+0x09,
    STATS4_TCAMCounter2    = GT_TYPE4_BANK1+0x0a,
    STATS4_TCAMCounter3    = GT_TYPE4_BANK1+0x0b,
    STATS4_InDroppedAvbA   = GT_TYPE4_BANK1+0x0c, /* For Opus */
    STATS4_InDroppedAvbB   = GT_TYPE4_BANK1+0x0d, /* For Opus */
    STATS4_InDaUnknown     = GT_TYPE4_BANK1+0x0e,
    STATS4_InMGMT          = GT_TYPE4_BANK1+0x0f,
    STATS4_OutQueue0       = GT_TYPE4_BANK1+0x10,
    STATS4_OutQueue1       = GT_TYPE4_BANK1+0x11,
    STATS4_OutQueue2       = GT_TYPE4_BANK1+0x12,
    STATS4_OutQueue3       = GT_TYPE4_BANK1+0x13,
    STATS4_OutQueue4       = GT_TYPE4_BANK1+0x14,
    STATS4_OutQueue5       = GT_TYPE4_BANK1+0x15,
    STATS4_OutQueue6       = GT_TYPE4_BANK1+0x16,
    STATS4_OutQueue7       = GT_TYPE4_BANK1+0x17,
    STATS4_OutCutThrough   = GT_TYPE4_BANK1+0x18,
    STATS4_OutOctetsA      = GT_TYPE4_BANK1+0x1a,
    STATS4_OutOctetsB      = GT_TYPE4_BANK1+0x1b,
    STATS4_OutYellow       = GT_TYPE4_BANK1+0x1c, /* For Opus */
    STATS4_OutDroppedYel   = GT_TYPE4_BANK1+0x1d, /* For Opus */
    STATS4_OutDiscards     = GT_TYPE4_BANK1+0x1e, /* For Opus */
    STATS4_OutMGMT         = GT_TYPE4_BANK1+0x1f
...
#define  GT_WOL_CTRL_MODE_DISABLE         0 /* WoL disabled */
#define  GT_WOL_CTRL_MODE_EN_CHK_8       01 /* WoL enabled checking for 8 instances of the WoL Address */
#define  GT_WOL_CTRL_MODE_EN_CHK_16      02 /* WoL enabled checking for 16 instances of the WoL Address */
#define  GT_WOL_CTRL_MODE_EN_CHK_16_PASS 03 /* WoL enabled checking for 16 instances of the WoL Address followed by the WoL Password*/

#define  GT_WAKE_INT_SOURCE_WOF         0x80 /* Wake on Frame Interrupt */
#define  GT_WAKE_INT_SOURCE_WOL_0       0x00 /* Wake on LAN Port 0 */
#define  GT_WAKE_INT_SOURCE_WOL_1       0x01 /* Wake on LAN Port 1 */
#define  GT_WAKE_INT_SOURCE_WOL_2       0x02 /* Wake on LAN Port 2 */
#define  GT_WAKE_INT_SOURCE_WOL_3       0x03 /* Wake on LAN Port 3 */
#define  GT_WAKE_INT_SOURCE_WOL_4       0x04 /* Wake on LAN Port 4 */
#define  GT_WAKE_INT_SOURCE_WOL_5       0x05 /* Wake on LAN Port 5 */
#define  GT_WAKE_INT_SOURCE_WOL_6       0x06 /* Wake on LAN Port 6 */
#define  GT_WAKE_INT_SOURCE_WOL_7       0x07 /* Wake on LAN Port 7 */
#define  GT_WAKE_INT_SOURCE_WOL_8       0x08 /* Wake on LAN Port 8 */
#define  GT_WAKE_INT_SOURCE_WOL_9       0x09 /* Wake on LAN Port 9 */
#define  GT_WAKE_INT_SOURCE_WOL_10      0x10 /* Wake on LAN Port 10 */
#define  GT_WAKE_INT_SOURCE_WOL_11      0x11 /* Wake on LAN Port 11 */
#define  GT_WAKE_INT_SOURCE_WOL_NO      0x0f /* Wake on LAN Port, 0xf: no port, other number=port */
 ...
 *
 * Enumerations:
 *		  GT_LED_CFG_GLOBAL_CONTROL - read/write global control
 ...
 *        GT_LED_CFG_SPECIAL_LED1	- read/write special led1 value
 *	      GT_LED_CFG_SPECIAL_LED2	- read/write special led2 value
 *	      GT_LED_CFG_SPECIAL_LED3	- read/write special led3 value
 *	      GT_LED_CFG_SPECIAL_LED4	- read/write special led4 value
...
typedef enum
{
	GT_LED_CFG_GLOBAL_CONTROL,	/* Peridot, valid for port 0 */
    GT_LED_CFG_LED0,
    GT_LED_CFG_LED1,
    GT_LED_CFG_LED2,
    GT_LED_CFG_LED3,
	GT_LED_CFG_SPECIAL_LED1,	/* Peridot, valid for port 0 */
	GT_LED_CFG_SPECIAL_LED2,	/* Peridot, valid for port 0 */
	GT_LED_CFG_SPECIAL_LED3,	/* Peridot, valid for port 0 */
	GT_LED_CFG_SPECIAL_LED4,	/* Peridot, valid for port 0 */
...
/* After Opus */
/*
 * typedef: enum GT_FLOW_CFG
 *
 * Description: Enumeration for Flow configuration type
 *
 * Enumerations:
 *        GT_FLOW_CFG_PORT_LIMIT_IN - Port Limit In
 *        GT_FLOW_CFG_PORT_LIMIT_OUT - Port Limit Out
 *        GT_FLOW_CFG_PORT_FLOW_CTRL_MODE - 0x10: Port Flow Control Mode
 *        GT_FLOW_CFG_8023x_Q_MAPPING - 0x17: IEEE 802.3x Queues to Pause
 *        GT_FLOW_CFG_PFC_2_TX_Q_MAPPING_01 - 0x18: PFC Priority to Tx Queue Mapping
 *        GT_FLOW_CFG_PFC_2_TX_Q_MAPPING_23 - 0x19: PFC Priority to Tx Queue Mapping
 *        GT_FLOW_CFG_PFC_2_TX_Q_MAPPING_45 - 0x1A: PFC Priority to Tx Queue Mapping
 *        GT_FLOW_CFG_PFC_2_TX_Q_MAPPING_67 - 0x1B: PFC Priority to Tx Queue Mapping
 *        GT_FLOW_CFG_RX_2_PFC_Q_MAPPING_01 - 0x1C: Rx Queue to PFC Priority Mapping
 *        GT_FLOW_CFG_RX_2_PFC_Q_MAPPING_23 - 0x1D: Rx Queue to PFC Priority Mapping
 *        GT_FLOW_CFG_RX_2_PFC_Q_MAPPING_45 - 0x1E: Rx Queue to PFC Priority Mapping
 *        GT_FLOW_CFG_RX_2_PFC_Q_MAPPING_67 - 0x1F: Rx Queue to PFC Priority Mapping
 */
typedef enum
{
    GT_FLOW_CFG_PORT_LIMIT_IN,
    GT_FLOW_CFG_PORT_LIMIT_OUT,
    GT_FLOW_CFG_PORT_FLOW_CTRL_MODE = 0x10,
    GT_FLOW_CFG_8023x_Q_MAPPING = 0x17,
    GT_FLOW_CFG_PFC_2_TX_Q_MAPPING_01 =0x18,
    GT_FLOW_CFG_PFC_2_TX_Q_MAPPING_23 =0x19,
    GT_FLOW_CFG_PFC_2_TX_Q_MAPPING_45 =0x1A,
    GT_FLOW_CFG_PFC_2_TX_Q_MAPPING_67 =0x1B,
    GT_FLOW_CFG_RX_2_PFC_Q_MAPPING_01 = 0x1C,
    GT_FLOW_CFG_RX_2_PFC_Q_MAPPING_23 = 0x1D,
    GT_FLOW_CFG_RX_2_PFC_Q_MAPPING_45 = 0x1E,
    GT_FLOW_CFG_RX_2_PFC_Q_MAPPING_67 = 0x1F
} GT_FLOW_CFG;

/*
 * typedef: enum GT_QUEUE_CFG
 *
 * Description: Enumeration for Queue configuration type
 *
 * Enumerations:
 *        GT_QUEUE_CFG_PORT_SCHEDULE - 0x00: Port Schedule
 *        GT_QUEUE_CFG_FILTER_Q_EN - 0x01: Filter QPri Enables
 *        GT_QUEUE_CFG_PORT_SCRATCH_REG - 0x02 to 0x03: Per Port Scratch registers
 *        GT_QUEUE_CFG_DANAMIC_Q_PRUNNING - 0x04: Dynamic Queue Pruning
 *        GT_QUEUE_CFG_DANAMIC_Q_STATUS - 0x05: Dynamic Queue Status
 *        GT_QUEUE_CFG_DANAMIC_Q_YELLOW_CTRL - 0x06: Dynamic Queue Yellow Control � New Index!
 *        GT_QUEUE_CFG_H_Q_LIMIT_REG - 0x10 to 0x17: Hard Queue Limit registers
 *        GT_QUEUE_CFG_Y_Q_LIMIT_REG - 0x20 to 0x27: Yellow Queue Limit registers
 *        GT_QUEUE_CFG_PFC_THRESHOLD - 0x30 to 0x3F: (Static) Begin & End PFC Thresholds � New!
 *        GT_QUEUE_CFG_IN_Q_COUNT - 0x40 to 0x47: (Virtual) Input Queue Counters � New!
 *        GT_QUEUE_CFG_QOS_GAIN_SET - 0x60 to 0x6F: Dynamic QoS Gain Settings (default = on)
 *        GT_QUEUE_CFG_PFC_GAIN_SET - 0x70 to 0x7F: Dyanmic PFC Gain Settings (default = off) */
typedef enum
{
    GT_QUEUE_CFG_PORT_SCHEDULE            =0,
    GT_QUEUE_CFG_FILTER_Q_EN              =1,
    GT_QUEUE_CFG_PORT_SCRATCH_REG         =0x2, /* port scratch 0 - 1 */
    GT_QUEUE_CFG_DANAMIC_Q_PRUNNING       =0x4,
    GT_QUEUE_CFG_DANAMIC_Q_STATUS         =0x5,
    GT_QUEUE_CFG_DANAMIC_Q_YELLOW_CTRL    =0x6,
    GT_QUEUE_CFG_H_Q_LIMIT_REG         =0x10,  /* Hard queue limit Q0 - Q7 inc: 1*/
    GT_QUEUE_CFG_Y_Q_LIMIT_REG         =0x20,  /* Yellow queue limit Q0 - Q7 inc: 1*/
    GT_QUEUE_CFG_PFC_THRESHOLD_BEGIN   =0x30,  /* Q0 - Q7 inc: 2 */
    GT_QUEUE_CFG_PFC_THRESHOLD_END     =0x3F,  /* Q0 - Q7 inc: 2 */
    GT_QUEUE_CFG_IN_Q_COUNT            =0x40,  /* PFC Receive Queue 0-7 Counter, inc: 1 */
    GT_QUEUE_CFG_QOS_GAIN_SET = 0x60,
    GT_QUEUE_CFG_PFC_GAIN_SET = 0x70
} GT_QUEUE_CFG;


typedef struct
{
	GT_U8   ipYellow;   /* When the UseIpYellow bit is set to a one (Port offset 0x19) this bit is
used to indicate the color of frames
	When the UseIpYellow bit is cleared to a zero (Port offset 0x19) this bit is
used to indicate these frames are AVB stream frames.*/
	GT_U8   disIpQpri;   /*  */
	GT_U8   ipQpri;   /*  */
	GT_U8   disIpFpri;   /*  */
	GT_U8   ipFpri;   /*  */
} GT_PRIORITY_MAP_DATA;


/* Supported Frame Priority Table set For IEEE priority mapping */
typedef enum
{
  GT_IEEE_PRIORITY_TABLE_ING_PCP_2_F_Q_PRI,   /* 0 Ingress PCP to IEEE_FPRI & IEEE_QPRI mapping */
  GT_IEEE_PRIORITY_TABLE_EG_GREEN_FPRI_2_PCP, /* 0x1 Egress Green Frame FPRI to PCP mapping (non-AVB) */
  GT_IEEE_PRIORITY_TABLE_EG_YEL_FPRI_2_PCP,   /* 0x2 = Egress Yellow Frame FPRI to PCP mapping (non-AVB) */
  GT_IEEE_PRIORITY_TABLE_EG_AVB_FPRI_2_PCP,   /* 0x3 = Egress AVB Frame FPRI to PCP mapping */
  GT_IEEE_PRIORITY_TABLE_RESERVED         ,   /* 0x4 = Reserved for future use */
  GT_IEEE_PRIORITY_TABLE_EG_GREEN_FPRI_2_DSCP,/* 0x5 = Egress Green Frame FPRI to DSCP mapping (non-AVB) */
  GT_IEEE_PRIORITY_TABLE_EG_YEL_FPRI_2_DSCP,  /* 0x6 = Egress Yellow Frame FPRI to DSCP mapping (non-AVB) */
  GT_IEEE_PRIORITY_TABLE_EG_AVB_FPRI_2_DSCP,  /* 0x7 = Egress AVB Frame FPRI to DSCP mapping */
} GT_IEEE_PRIORITY_TABLE_SET;
...
typedef union
{
  GT_IEEE_PRIORITY_TABLE_ING_PCP_2_F_Q_PRI_STRUCT     ingTbl;
  GT_IEEE_PRIORITY_TABLE_EG_GREEN_FPRI_2_PCP_STRUCT   egGreeTblPcp;
  GT_IEEE_PRIORITY_TABLE_EG_YEL_FPRI_2_PCP_STRUCT     egYelTblPcp;
  GT_IEEE_PRIORITY_TABLE_EG_AVB_FPRI_2_PCP_STRUCT     egAvbTblPcp;
  GT_IEEE_PRIORITY_TABLE_EG_GREEN_FPRI_2_DSCP_STRUCT  egGreeTblDscp;
  GT_IEEE_PRIORITY_TABLE_EG_YEL_FPRI_2_DSCP_STRUCT    egYelTblDscp;
  GT_IEEE_PRIORITY_TABLE_EG_AVB_FPRI_2_DSCP_STRUCT    egAvbTbldscp;
} GT_IEEE_PRIORITY_TABLE_STRUCT;


/* Port control 3 definitions */
#define GT_PORT_CONTROL_3_USECFI_YELLOW     1
#define GT_PORT_CONTROL_3_USEDEI_YELLOW     1<<1
#define GT_PORT_CONTROL_3_UPDATECFI_YELLOW  1<<2
#define GT_PORT_CONTROL_3_UPDATEDEI_YELLOW  1<<3
#define GT_PORT_CONTROL_3_USEIP_YELLOW      1<<4
#define GT_PORT_CONTROL_3_IPAVB_EN			1<<4

#define GT_PORT_CONTROL_3_DROP_BC           1<<13
#define GT_PORT_CONTROL_3_PAV_4_INT_CPU     1<<14
#define GT_PORT_CONTROL_3_MAP_2_INT_CPU     1<<15

/* VTU control definitions */
#define GT_VTU_CONTROL_DONT_LEARN           1<<15
#define GT_VTU_CONTROL_FILTER_UC            1<<14
#define GT_VTU_CONTROL_FILTER_BC            1<<13
#define GT_VTU_CONTROL_FILTER_MC            1<<12
 ...
 *
 * Fields:
 *      GT_EEPROM_WRITE_DATA           - Write data to the EEPROM register
 *      GT_EEPROM_READ_DATA            - Read data from EEPROM register
 *      GT_EEPROM_RESTART              - Restart EEPROM oprition
 */
typedef enum
{
    GT_EEPROM_NO_OP                     = 0x0,
...
/*
 * Typedef: enum GT_HIDDENREG_OPERATION
 *
 * Description: Defines the HIDDENREG Operation type
 *
 * Fields:
 *   GT_HIDDENREG_WRITE_DATA           - Write data to the HIDDENREG register
 *   GT_HIDDENREG_READ_DATA            - Read data from HIDDENREG register
 *   GT_HIDDENREG_READ_DATA_INC_PORT   - Read data from HIDDENREG register, port + 1
 *   GT_HIDDENREG_READ_DATA_INC_REG    - Read data from HIDDENREG register, reg + 1
 */
typedef enum
{
    GT_HIDDENREG_NO_OP                  = 0x0,
    GT_HIDDENREG_WRITE_DATA             = 0x3,
    GT_HIDDENREG_READ_DATA              = 0x4,
    GT_HIDDENREG_READ_DATA_INC_PORT     = 0x5,
    GT_HIDDENREG_READ_DATA_INC_REG      = 0x6,
} GT_HIDDENREG_OPERATION;


/*
 *  typedef: struct GT_HIDDENREG_OP_DATA
 *
 *  Description: data required by HIDDENREG Operation
 *
 *  Fields:
 *      hiddenRegPort     - physical port of the device
 *      hiddenRegAddr     - register address
 *      hiddenRegData     - data for ptp register.
 */
typedef struct
{
    GT_U8     hiddenRegPort;
    GT_U8     hiddenRegAddr;
    GT_U16    hiddenRegData;
} GT_HIDDENREG_OP_DATA;
...
#define GT_SCRAT_MISC_REG_SCRAT_0    0x00 /* Scratch Byte 0 */
#define GT_SCRAT_MISC_REG_SCRAT_1    0x01 /* Scratch Byte 1 */
/* 0x02 to 0x05: Power Monitor control  */
#define GT_SCRAT_MISC_REG_PWR_MONIT_CTRL    0x02 /* Power Monitor Control */
#define GT_SCRAT_MISC_REG_PWR_MONIT_THR_0   0x04 /* Power Monitor Threshold 0 */
#define GT_SCRAT_MISC_REG_PWR_MONIT_THR_1   0x05 /* Power Monitor Threshold 1 */
/* 0x06 to 0x06: Voltage Regulator control  */
#define GT_SCRAT_MISC_REG_VOLTAGE_CTRL      0x06 /* Voltage Regulator control */
#define GT_SCRAT_MISC_REG_VOLTAGE_CTRL_2    0x07 /* Voltage Regulator control  2*/
#define GT_SCRAT_MISC_REG_VOLTAGE_TEST      0x08 /* Voltage Regulator test */
#define GT_SCRAT_MISC_REG_PWR_MANAGEMENT    0x0A /* Power Monitor Control */
/* 0x0B (0x0A) to 0x0F: Energy Efficient Ethernet (EEE) register space  */
#define GT_SCRAT_MISC_REG_EEE_TIMER_RATE     0x0B /* EEE Timer Rate */
#define GT_SCRAT_MISC_REG_EEE_WAKE_TIME_GE   0x0C /* EEE Wake Time GE */
#define GT_SCRAT_MISC_REG_EEE_ASERT_TIME     0x0D /* EEE Asertion Time */
#define GT_SCRAT_MISC_REG_EEE_WAKE_TIME_FE   0x0E /* EEE Wake Time FE */
#define GT_SCRAT_MISC_REG_EEE_TR_IDLE_TIME   0x0F /* EEE trans idle Time */
/* 0x10 to 0x1F: Reserved for future use  */
/* 0x20 to 0x3F: GPIO Port Stall Vectors  */
#define GT_SCRAT_MISC_REG_GPIO0_STALL_VEC0   0x20 /* GPIO 0 Port Stall Vector 0 */
#define GT_SCRAT_MISC_REG_GPIO0_STALL_VEC1   0x21 /* GPIO 0 Port Stall Vector 1 */
#define GT_SCRAT_MISC_REG_GPIO1_STALL_VEC0   0x22 /* GPIO 1 Port Stall Vector 0 */
#define GT_SCRAT_MISC_REG_GPIO1_STALL_VEC1   0x23 /* GPIO 1 Port Stall Vector 1 */
#define GT_SCRAT_MISC_REG_GPIO2_STALL_VEC0   0x24 /* GPIO 2 Port Stall Vector 0 */
#define GT_SCRAT_MISC_REG_GPIO2_STALL_VEC1   0x25 /* GPIO 2 Port Stall Vector 1 */
#define GT_SCRAT_MISC_REG_GPIO3_STALL_VEC0   0x26 /* GPIO 3 Port Stall Vector 0 */
#define GT_SCRAT_MISC_REG_GPIO3_STALL_VEC1   0x27 /* GPIO 3 Port Stall Vector 1 */
#define GT_SCRAT_MISC_REG_GPIO4_STALL_VEC0   0x28 /* GPIO 4 Port Stall Vector 0 */
#define GT_SCRAT_MISC_REG_GPIO4_STALL_VEC1   0x29 /* GPIO 4 Port Stall Vector 1 */
#define GT_SCRAT_MISC_REG_GPIO5_STALL_VEC0   0x2A /* GPIO 5 Port Stall Vector 0 */
#define GT_SCRAT_MISC_REG_GPIO5_STALL_VEC1   0x2B /* GPIO 5 Port Stall Vector 1 */
#define GT_SCRAT_MISC_REG_GPIO6_STALL_VEC0   0x2C /* GPIO 6 Port Stall Vector 0 */
#define GT_SCRAT_MISC_REG_GPIO6_STALL_VEC1   0x2D /* GPIO 6 Port Stall Vector 1 */
#define GT_SCRAT_MISC_REG_GPIO7_STALL_VEC0   0x2E /* GPIO 7 Port Stall Vector 0 */
#define GT_SCRAT_MISC_REG_GPIO7_STALL_VEC1   0x2F /* GPIO 7 Port Stall Vector 1 */
#define GT_SCRAT_MISC_REG_GPIO8_STALL_VEC0   0x30 /* GPIO 8 Port Stall Vector 0 */
#define GT_SCRAT_MISC_REG_GPIO8_STALL_VEC1   0x31 /* GPIO 8 Port Stall Vector 1 */
#define GT_SCRAT_MISC_REG_GPIO9_STALL_VEC0   0x32 /* GPIO 9 Port Stall Vector 0 */
#define GT_SCRAT_MISC_REG_GPIO9_STALL_VEC1   0x33 /* GPIO 9 Port Stall Vector 1 */
#define GT_SCRAT_MISC_REG_GPIO10_STALL_VEC0  0x34 /* GPIO 10 Port Stall Vector 0 */
#define GT_SCRAT_MISC_REG_GPIO10_STALL_VEC1  0x35 /* GPIO 10 Port Stall Vector 1 */
#define GT_SCRAT_MISC_REG_GPIO11_STALL_VEC0  0x36 /* GPIO 11 Port Stall Vector 0 */
#define GT_SCRAT_MISC_REG_GPIO11_STALL_VEC1  0x37 /* GPIO 11 Port Stall Vector 1 */
#define GT_SCRAT_MISC_REG_GPIO12_STALL_VEC0  0x38 /* GPIO 12 Port Stall Vector 0 */
#define GT_SCRAT_MISC_REG_GPIO12_STALL_VEC1  0x39 /* GPIO 12 Port Stall Vector 1 */
#define GT_SCRAT_MISC_REG_GPIO13_STALL_VEC0  0x3A /* GPIO 13 Port Stall Vector 0 */
#define GT_SCRAT_MISC_REG_GPIO13_STALL_VEC1  0x3B /* GPIO 13 Port Stall Vector 1 */
#define GT_SCRAT_MISC_REG_GPIO14_STALL_VEC0  0x2C /* GPIO 14 Port Stall Vector 0 */
#define GT_SCRAT_MISC_REG_GPIO14_STALL_VEC1  0x3D /* GPIO 14 Port Stall Vector 1 */
/* 0x40 to 0x4A: Per port EEE receive status counters  */
#define GT_SCRAT_MISC_REG_EEE_STAT_RX_CNT_P0    0x40 /* EEE Status counter(RxLPICnt) P0 */
#define GT_SCRAT_MISC_REG_EEE_STAT_RX_CNT_P1    0x41 /* EEE Status counter(RxLPICnt) P1 */
#define GT_SCRAT_MISC_REG_EEE_STAT_RX_CNT_P2    0x42 /* EEE Status counter(RxLPICnt) P2 */
#define GT_SCRAT_MISC_REG_EEE_STAT_RX_CNT_P3    0x43 /* EEE Status counter(RxLPICnt) P3 */
#define GT_SCRAT_MISC_REG_EEE_STAT_RX_CNT_P4    0x44 /* EEE Status counter(RxLPICnt) P4 */
#define GT_SCRAT_MISC_REG_EEE_STAT_RX_CNT_P5    0x45 /* EEE Status counter(RxLPICnt) P5 */
/* 0x50 to 0x5A: Per port EEE transmit status counters  */
#define GT_SCRAT_MISC_REG_EEE_STAT_TX_CNT_P0    0x50 /* EEE Status counter(TxLPICnt) P0 */
#define GT_SCRAT_MISC_REG_EEE_STAT_TX_CNT_P1    0x51 /* EEE Status counter(TxLPICnt) P1 */
#define GT_SCRAT_MISC_REG_EEE_STAT_TX_CNT_P2    0x52 /* EEE Status counter(TxLPICnt) P2 */
#define GT_SCRAT_MISC_REG_EEE_STAT_TX_CNT_P3    0x53 /* EEE Status counter(TxLPICnt) P3 */
#define GT_SCRAT_MISC_REG_EEE_STAT_TX_CNT_P4    0x54 /* EEE Status counter(TxLPICnt) P4 */
#define GT_SCRAT_MISC_REG_EEE_STAT_TX_CNT_P5    0x55 /* EEE Status counter(TxLPICnt) P5 */
/* 0x5D to 0x5F: EEE status registers  */
#define GT_SCRAT_MISC_REG_EEE_STAT_LPI_P0_P3    0x5D /* EEE Status LPI state P0 - P3 */
#define GT_SCRAT_MISC_REG_EEE_STAT_LPI_P4_P6    0x5E /* EEE Status LPI state P4 - P6 */
/* 0x60 to 0x6F: GPIO registers data and configuration  */
#define GT_SCRAT_MISC_REG_GPIO_CFG_1   0x60  /* GPIO Configuration 0-7 */
#define GT_SCRAT_MISC_REG_GPIO_CFG_2   0x61  /* GPIO configuration 8-14 */
#define GT_SCRAT_MISC_REG_GPIO_DIR_1   0x62  /* GPIO Direction 0-7 */
#define GT_SCRAT_MISC_REG_GPIO_DIR_2   0x63  /* GPIO Direction 8-14 */
#define GT_SCRAT_MISC_REG_GPIO_DAT_1   0x64  /* GPIO Data 0-7 */
#define GT_SCRAT_MISC_REG_GPIO_DAT_2   0x65  /* GPIO Data 8-14 */
#define GT_SCRAT_MISC_REG_GPIO_CTRL_0  0x68  /* GPIO Control 0 */
#define GT_SCRAT_MISC_REG_GPIO_CTRL_1  0x69  /* GPIO Control 1 */
#define GT_SCRAT_MISC_REG_GPIO_CTRL_2  0x6A  /* GPIO Control 2 */
#define GT_SCRAT_MISC_REG_GPIO_CTRL_3  0x6B  /* GPIO Control 3 */
#define GT_SCRAT_MISC_REG_GPIO_CTRL_4  0x6C  /* GPIO Control 4 */
#define GT_SCRAT_MISC_REG_GPIO_CTRL_5  0x6D  /* GPIO Control 5 */
#define GT_SCRAT_MISC_REG_GPIO_CTRL_6  0x6E  /* GPIO Control 6 */
#define GT_SCRAT_MISC_REG_GPIO_CTRL_7  0x6F  /* GPIO Control 7 */
...
#define GT_GPIO_PIN_GPIO       0 /* = GPIO[x] */
#define GT_GPIO_PIN_PTP_TRIG   1 /* = PTP_TRIG  Precise Timing Protocol Trigger Generate Output */
#define GT_GPIO_PIN_PTP_EVREQ  2 /* = PTP_EVREQ  Precise Timing Protocol Event Request Input */
#define GT_GPIO_PIN_PTP_EXTCLK 3 /* = PTP_EXTCLK  Precise Timing Protocol External Clock Input */
#define GT_GPIO_PIN_RX_CLK0    4 /* = RX_CLK0  SyncE Receive Clock 0 Output */
#define GT_GPIO_PIN_RX_CLK1    5 /* = RX_CLK1  SyncE Receive Clock 1 Output */
#define GT_GPIO_PIN_SDET       6 /* = Reserved for future SDET  SERDES interface�s Signal Detect Input */
#define GT_GPIO_PIN_CLK125     7 /* = CLK125  Free running 125 MHz Clock Output* */


/* Point_data Set/Get definitions */
/*          Flow Control  Per port */
/*          Scratch & Misc   Global 2 */
/*          Energy Management  Global 2 */
/*          IMP Comm/Debug  Global 2 */
/*          Watch Dog Control  Global 2 */
/*          QoS Weights  Global 2 */

typedef enum
{
    GT_MISC_SCRATCH                  = 0x0,
    GT_MISC_ENARGE_MANAG             = 0x1,
    GT_MISC_IMP_COMM_DEBUG           = 0x2,
    GT_MISC_WD_CTRL                  = 0x3,
    GT_MISC_QOS_WEIGHT               = 0x4,
    GT_MISC_UNKNOWN_MODE             = 0x5
} GT_MISC_POINT_DATA_MODE;
...
/* The Pointer bits are used to access the Index registers as follows: */
/* 0x00: Port Limit In */
#define GT_FLOW_CTRL_REG_PORT_LIMIT_IN    0x00 /* Port Limit In */
/* 0x01: Port Limit Out */
#define GT_FLOW_CTRL_REG_PORT_LIMIT_OUT   0x01 /* Port Limit Out */
/* 0x02 to 0x0F: Reserved for future use (per queue Limit In/Outs if needed) */
/* 0x10: Port Flow Control Mode */
#define GT_FLOW_CTRL_REG_PORT_CTRL_MODE   0x10 /* Port Flow Control Mode */
/* 0x11 to 0x17: Reserved for future use */
/* 0x17: IEEE 802.3x Queues to Pause */
#define GT_FLOW_CTRL_REG_8023_Q_TO_PAUSE   0x17 /* IEEE 802.3x Queues to Pause */
/* 0x18 to 0x1B: PFC Priority to Tx Queue Mapping */
#define GT_FLOW_CTRL_REG_PFC_PRI_TO_TX_Q_MAP   0x18 /* PFC Priority to Tx Queue Mapping */
/* 0x1C to 0x1F: Rx Queue to PFC Priority Mapping */
#define GT_FLOW_CTRL_REG_RX_Q_TO_PFC_PRI_MAP   0x1C /* Rx Queue to PFC Priority Mapping */
/* 0x20 to 0x7F: Reserved for future us */
#define GT_FLOW_CTRL_REG_MAX        0x20/* Maximun register pointer */

/* Queue Control */
/* Point definition */
/* The Pointer bits are used to access the Index registers as follows: */
/* 0x00: Port Schedule */
#define GT_QUEUE_CTRL_REG_PORT_SCHEDULE       0x0 
/* 0x01: Filter QPri Enables */
#define GT_QUEUE_CTRL_REG_FILT_QPRI_EN        0x1
/* 0x02 to 0x03: Per Port Scratch registers */
#define GT_QUEUE_CTRL_REG_PORT_SCRATCH_0      0x2 
#define GT_QUEUE_CTRL_REG_PORT_SCRATCH_1      0x3 
/* 0x04: Dynamic Queue Pruning */
#define GT_QUEUE_CTRL_REG_DNAMC_Q_PRUNNING    0x4
/* 0x05: Dynamic Queue Status */
#define GT_QUEUE_CTRL_REG_DNAMC_Q_STATUS      0x5
/* 0x06: Dynamic Queue Yellow Control � New Index! */
#define GT_QUEUE_CTRL_REG_DNAMC_YEL_CTRL      0x6
/* 0x10 to 0x17: Hard Queue Limit registers */
/* 0x20 to 0x27: Yellow Queue Limit registers */
/* 0x30 to 0x3F: (Static) Begin & End PFC Thresholds � New! */
/* 0x40 to 0x47: (Virtual) Input Queue Counters � New! */
/* 0x60 to 0x6F: Dynamic QoS Gain Settings (default = on) */
/* 0x70 to 0x7F: Dyanmic PFC Gain Settings (default = off) */

#define GT_QUEUE_CTRL_REG_MAX        0x80 /* Maximun register pointer */
...
/* Energy Management */
/* Point definition */
/* 0x40 to 0x5A: Per port EEE receive status counters  */
#define GT_EEE_RECV_STATUS_COUNT_REG_BASE    0x40 /* Per port EEE receive status counters */
/* 0x5B: Reserved for future use */
/* 0x5C to 0x5F: EEE status registers � Ports 0 to 15 */
#define GT_EEE_STATUS_REG_0_TO_15_BASE  0x5C /* EEE status registers  Ports 0 to 15 */
/* 0x60 to 0x7A: Per port EEE transmit status counters */
#define GT_EEE_TRANSMIT_STATUS_COUNT_REG_BASE     0x60 /* Per port EEE transmit status counters */
/* 0x7B: Reserved for future use */
/* 0x7C to 0x7E: EEE status registers � Ports 16 to 26 */
#define GT_EEE_STATUS_REG_16_TO_26_BASE  0x7C /* EEE status registers  Ports 16 to 26 */
/* 0x7F: Reserved for future use */
#define GT_ENERGE_MANAGMENT_REG_MAX        0x80/* Maximun register pointer */

/* IMP Comm/Debug */
/* Point definition */
/* The Pointer bits are used to access the Index registers as follows: */
/* 0x00 to 0x03: Communication Interface to/from the CPU */
#define GT_IMP_COMMUNIC_CPU    0x08 /* Communication Interface to/from the CPU */
/* 0x04 to 0x07: Reserved for future use */
/* 0x08 to 0x0F: Debug Control & Status */
#define GT_IMP_DEBUG_CTRL_STATUS    0x08 /* Debug Control & Status */
/* 0x10 to 0x1F: CPU�s Main and Alternate Registers (always readable) */
#define GT_IMP_CPU_MAIN_ALT_REGS    0x10 /* CPU�s Main and Alternate Registers */
/* 0x20 to 0x29: CPU�s Special Purpose Registers (always readable) */
#define GT_IMP_CPU_SPECIAL_REGS    0x20 /* CPU�s Special Purpose Registers */
/* 0x2A to 0x2E: Reserved for future use */
/* 0x2D : Hidden Debug Pointer (documented by Engineering) */
#define GT_IMP_HIDDEN_DEBUG_POINT    0x2D /* Hidden Debug Pointer */
/* 0x2E : Hidden Debug Data (documented by Engineering) */
#define GT_IMP_HIDDEN_DEBUG_DATA    0x2E /* Hidden Debug Data */
/* 0x2F : IMP I/O Page Register */
#define GT_IMP_IO_PAGE_REG    0x2F /* IMP I/O Page Register */
/* 0x30 to 0x33: Break Point Control */
#define GT_IMP_BREAK_POINT_CTRL    0x30 /* Break Point Control */
/* 0x38 : Break Point Status */
#define GT_IMP_BREAK_POINT_STATUS    0x38 /* Break Point Status */
/* 0x39 to 0x3F: Reserved for future use */
/* 0x40 to 0x47: Break Point 0 */
#define GT_IMP_BREAK_POINT_0    0x40 /* Break Point 0 */
/* 0x48 to 0x4F: Break Point 1 */
#define GT_IMP_BREAK_POINT_1    0x48 /* Break Point 1 */
/* 0x50 to 0x57: Break Point 2 */
#define GT_IMP_BREAK_POINT_2    0x50 /* Break Point 2 */
/* 0x58 to 0x5F: Break Point 3 */
#define GT_IMP_BREAK_POINT_3    0x58 /* Break Point 3 */
/* 0x60 to 0x6F: Reserved for future use */
/* 0x70 to 0x7F: IMP I/O Registers (always readable) */
#define GT_IMP_IO_REGS    0x79 /* IMP I/O Registers */

#define GT_IMP_REG_MAX        0x7A/* Maximun register pointer */


/* Watch Dog Control */
/* Point definition */
/* The Pointer bits are used to access the Index registers as follows: */
/* 0x00: Watch Dog Interrupt Source */
#define GT_WD_INTER_SRC    0x00 /* Watch Dog Interrupt Source */
/* 0x10 to 0x13: Data Path Watch Dog Interrupts, Masks, Events & History */
#define GT_WD_INTER_DATA_PATH    0x10 /* Data Path Watch Dog Interrupts, Masks, Events & History */
/* 0x20 to 0x23: Policy Interrupts, Masks, Events & History */
#define GT_WD_POLICY_INTER    0x20 /* Policy Interrupts, Masks, Events & History */
/* 0x40: Auto Fixing Enables */
#define GT_WD_AUTO_FIX_ENABLE    0x40 /* Auto Fixing Enables */
#define GT_WD_REG_MAX        0x50/* Maximun register pointer */

/* QoS Weight */
/* Point definition */
/*
Pointer to desired octet of QoS Weights. These bits select one of 64 possible
QoS Weight Data registers and the QoS Weight Length register for both read
and write operations. A write operation occurs if the Update bit is a one.
Otherwise a read operation occurs.
*/
#define GT_QOS_WEIGHT    0x00 /* Pointer to desired octet of QoS Weights */

typedef struct
{
    GT_U8         disable_reset_monit;
    GT_U8         disable_volt_monit;
    GT_U8         reset_time;
}GT_PW_MONIT_CTRL;
...
/*
EEE Time Rate:
PowerCtrl Timer at register index 0x0A as follows:
00 = number of core clock ticks (default)
01 = 1 uSec steps
10 = 1 mSec steps
11 = Reserved

Assertion Timer Rate. These bits determine the resolution of the Assertion
Timer at register index 0x0D as follows:
00 = 1 uSec steps (default)
01 = 1 mSec steps
10 = 1 Sec steps
11 = Reserved (15 Sec steps

Wake Timer Rate. These bits determine the resolution of the Wake Timer at
register index 0x0E as follows:
00 = 1 uSec steps (default)
01 = 1 mSec steps
10 = 1 Sec steps
11 = Reserved (15 Sec steps)
...
TxIdle Timer Rate. These bits determine the resolution of the Tx Idle Timer at
register index 0x0F as follows:
00 = 1 uSec steps
01 = 1 mSec steps (default)
10 = 1 Sec steps
11 = Reserved (15 Sec steps)
*/
...
#define IS_TRUNK_ID_VALID_1(_dev, _id)    (((_id) < 32) ? 1 : 0)
 ...
 *   is96Frame      - If use 48 or 96 TCAM frames. 1:96 and 0 48.
...
struct _GT_QD_DEV
{
    ...
    FGT_READ_MII  smiReadMii;
    FGT_WRITE_MII smiWriteMii;
    GT_RMU		  rmu;
    RMU_TX_RX_PAK rmuTxRx;
#endif
    GT_BOOL       use_smi;     /* use SMI only  */
