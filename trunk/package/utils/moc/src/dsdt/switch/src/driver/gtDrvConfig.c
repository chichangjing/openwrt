...
)
{
  ...
  else if (dev->validSerdesVec & (1<<port))
  {
    hwPort = (GT_U8)port;            
  }
  else
...
GT_STATUS driverGetSerdesPort(GT_QD_DEV *dev, GT_U8* hwPort)
{
    switch(dev->deviceId)
    {
        ...
        case GT_88E6240:
        case GT_88E6352:
            if (!((*hwPort==0x4)||(*hwPort==0x5)||(*hwPort==0xf)))
            {
              *hwPort = GT_INVALID_PORT;
            }
            else
        ...
        case GT_88E6320:
        case GT_88E6321:
			if (!((*hwPort==0xc)||(*hwPort==0xd)||(*hwPort<2)))
            {
              *hwPort = GT_INVALID_PORT;
            }
            else
...
)
{
    ...
    switch (phyId & PHY_MODEL_MASK)
    {
        ...
        case DEV_S15LV:
        	    if(dev->devName1 & G1_DEV_88E6390_FAMILY) /* Peridot Family */
        	    {
        	    	phyInfo->anyPage = 0x0;
        	    	phyInfo->flag = GT_PHY_GIGABIT|GT_PHY_EX_CABLE_STATUS|
        	    			        GT_PHY_DTE_CAPABLE|GT_PHY_ADV_VCT_CAPABLE|
        	    	                GT_PHY_MAC_IF_LOOP|GT_PHY_LINE_LOOP|GT_PHY_EXTERNAL_LOOP|
        	    	                GT_PHY_PKT_GENERATOR;
        	    	phyInfo->vctType = GT_PHY_ADV_VCT_TYPE2;
        	    	phyInfo->exStatusType = GT_PHY_EX_STATUS_TYPE6;
