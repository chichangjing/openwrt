...
)
{
    ...
    opData.ptpBlock = GT_AVB_BLOCK_AVB_POLICY;
...
)
{
    ...
    switch (priType)
    {
        case GT_AVB_HI_FPRI:
            ...
            break;
        case GT_AVB_HI_QPRI:
          if (!IS_IN_DEV_GROUP(dev,DEV_AVB_POLICY_1))
		  {
            mask = 0x3;
		  }
		  else
        ...
        default:
          switch (priType)
		  {
            case GT_AVB_MID_HI_FPRI:
              mask = 0x7;
              reg = 1;
              bitPos = 12;
              break;
            case GT_AVB_MID_HI_QPRI:
              mask = 0x7;
              reg = 1;
              bitPos = 8;
              break;
            case GT_AVB_MID_LO_FPRI:
              mask = 0x7;
              reg = 1;
              bitPos = 4;
              break;
            case GT_AVB_MID_LO_QPRI:
              mask = 0x7;
              reg = 1;
              bitPos = 0;
              break;
            case GT_LEGACY_MID_HI_FPRI:
              mask = 0x7;
              reg = 5;
              bitPos = 12;
              break;
            case GT_LEGACY_MID_HI_QPRI:
              mask = 0x7;
              reg = 5;
              bitPos = 8;
              break;
            case GT_LEGACY_MID_LO_FPRI:
              mask = 0x7;
              reg = 5;
              bitPos = 4;
              break;
            case GT_LEGACY_MID_LO_QPRI:
              mask = 0x7;
              reg = 5;
              bitPos = 0;
              break;
    ...
    opData.ptpBlock = GT_AVB_BLOCK_AVB_POLICY;    /* AVB Policy register space */
    ...
    *pri = qdLong2Char(((GT_U32)(opData.ptpData) >> bitPos) & mask);
...
)
{
    ...
    DBG_INFO(("gavbGetMidPriEn Called.\n"));
    ...
    if((retVal = ptpOperationPerform(dev, op, &opData)) != GT_OK)
    {
        DBG_INFO(("Failed reading medium enable.\n"));
        return GT_FAIL;
    }

    *enHi = (GT_U32)(opData.ptpData >> 15) & 0x1;
    *enLo = (GT_U32)(opData.ptpData >> 7) & 0x1;
...
)
{
    ...
    DBG_INFO(("gavbSetMidPriEn Called.\n"));
    ...
    if((retVal = ptpOperationPerform(dev, op, &opData)) != GT_OK)
    {
        DBG_INFO(("Failed writing medium enable.\n"));
        return GT_FAIL;
    }
...
)
{
    ...
    DBG_INFO(("gavbGetAvbPolicy Called.\n"));
    ...
    ptpData = qdLong2Short((opData.ptpData) & 0xffff);
    avbPolicy->avbMode = (ptpData >> 14) & 0x3;
    avbPolicy->avbOverride = (ptpData >> 13) & 0x1;
    avbPolicy->filterbadAvb = (ptpData >> 12) & 0x1;
    avbPolicy->avbTunnel = (ptpData >> 11) & 0x1;
    avbPolicy->disBadAvb = (ptpData >> 10) & 0x1;
    avbPolicy->ingAllAvb = (ptpData >> 5) & 0x1;
    avbPolicy->egAllAvb = (ptpData >> 4) & 0x1;
    avbPolicy->avbHiPolicy = (ptpData >> 2) & 0x3;
    avbPolicy->avbLowPolicy = (ptpData >> 0) & 0x3;
...
)
{
    ...
    DBG_INFO(("GT_STATUS gavbSetAvbPolicy Called.\n"));
    ...
    opData.ptpData = avbPolicy->avbMode;
	opData.ptpData <<= 1;
    opData.ptpData |= avbPolicy->avbOverride;
	opData.ptpData <<= 1;
    opData.ptpData |= avbPolicy->filterbadAvb;
	opData.ptpData <<= 1;
    opData.ptpData |= avbPolicy->avbTunnel;
	opData.ptpData <<= 1;
    opData.ptpData |= avbPolicy->disBadAvb;
	opData.ptpData <<= 5;
    opData.ptpData |= avbPolicy->ingAllAvb; 
	opData.ptpData <<= 1;
    opData.ptpData |= avbPolicy->egAllAvb;
	opData.ptpData <<= 2;
    opData.ptpData |= avbPolicy->avbHiPolicy;
	opData.ptpData <<= 2;
    opData.ptpData |= avbPolicy->avbLowPolicy;
...
)
{
    ...
    DBG_INFO(("gqavSetPortQpriTimeAwareBlock Called.\n"));
    ...
    if (!IS_IN_DEV_GROUP(dev, DEV_QAV_TIME_AWARE_BLK))
    {
        DBG_INFO(("GT_NOT_SUPPORTED\n"));
        return GT_NOT_SUPPORTED;
    }
    ...
    if (queue>0x3)
      {
        if (IS_IN_DEV_GROUP(dev,DEV_QAV_1))
		{
          if (queue>0x7)
		  {
    ...
    opData.ptpBlock = GT_AVB_BLOCK_QAV;    /* QAV register space */
    opData.ptpAddr = (queue+block)*2;
    ...
    if((retVal = ptpOperationPerform(dev, op, &opData)) != GT_OK)
    {
      DBG_INFO(("Failed reading block enable for port %d queue %d.\n",port,queue));
      return GT_FAIL;
    }
    ...
    if((retVal = ptpOperationPerform(dev, op, &opData)) != GT_OK)
    {
      DBG_INFO(("Failed writing QTS block enable for port %d queue %d.\n",port,queue));
      return GT_FAIL;
    }
...
)
{
    ...
    DBG_INFO(("gqavGetPortQpriTimeAwareBlock Called.\n"));
...
)
{
    ...
    if ((!IS_IN_DEV_GROUP(dev,DEV_QAV_QPRI_QTS_TOKEN))&&
        (!IS_IN_DEV_GROUP(dev, DEV_QAV_QPRI_RATE)))
    {
        DBG_INFO(("GT_NOT_SUPPORTED\n"));
        return GT_NOT_SUPPORTED;
    }
    ...
    if (!IS_IN_DEV_GROUP(dev,DEV_FE_AVB_FAMILY))
    {
      if  (!(IS_IN_DEV_GROUP(dev,DEV_QAV_1))&&(qtsToken>0x7fff))
      {
        DBG_INFO(("GT_BAD_PARAM\n"));
        return GT_BAD_PARAM;
      }
    ...
    if (!IS_IN_DEV_GROUP(dev,DEV_FE_AVB_FAMILY))
	{
      if  (IS_IN_DEV_GROUP(dev,DEV_QAV_1))
      {
        opData.ptpData = qtsToken&0xffff;
	  }
	  else
      {
...
)
{
    ...
    if (IS_IN_DEV_GROUP(dev,DEV_QAV_1))
	{
      *qtsToken =(GT_U16)opData.ptpData&0xffff;
	}
	else if (!IS_IN_DEV_GROUP(dev,DEV_FE_AVB_FAMILY))
	{
      ...
      *qtsToken =(GT_U16)opData.ptpData&0x7ff;
...
)
{
    ...
    if ((IS_IN_DEV_GROUP(dev,DEV_QAV_1))&&(burst>0x3fff))
    {
       DBG_INFO(("GT_BAD_PARAM\n"));
       return GT_BAD_PARAM;
    }
...
)
{
    ...
    if (IS_IN_DEV_GROUP(dev,DEV_QAV_1))
      *burst = (GT_U16)opData.ptpData&0x3fff;
	else
...
)
{
    ...
    return gqavSetPortQpriXQTSToken(dev, port, queue, rate);
}
/*******************************************************************************
...
)
{
    ...
    return gqavGetPortQpriXQTSToken(dev, port, queue, rate);
...
)
{
    ...
    return gqavSetPortQpriXBurstBytes(dev, port, queue, hiLimit);
}
/*******************************************************************************
...
)
{
    ...
    return gqavGetPortQpriXBurstBytes(dev, port, queue, hiLimit);
...
)
{
	...
	if (!IS_IN_DEV_GROUP(dev,DEV_QAV_ENABLE))
    {
        DBG_INFO(("GT_NOT_SUPPORTED\n"));
        return GT_NOT_SUPPORTED;
    }
...
)
{
    ...
    DBG_INFO(("gqavGetPortCurXQToken Called.\n"));
    ...
    *qtsToken =(GT_U16)opData.ptpData&0x03ff;
    *qtsToken <<=16;

    opData.ptpAddr = 0x10 + queue*4;
    if((retVal = ptpOperationPerform(dev, op, &opData)) != GT_OK)
    {
        DBG_INFO(("Failed reading QTS token for port %d queue %d.\n", port, queue));
        return GT_FAIL;
...
)
{
    ...
    DBG_INFO(("gqavGetPortXQHeadPacket Called.\n"));
    ...
    *packetByte =(GT_U16)opData.ptpData&0x03fff;
    DBG_INFO(("OK.\n"));
    return GT_OK;
...
)
{
    ...
    DBG_INFO(("gqavSetGlobalDisAvbTrans Called.\n"));
...
)
{
    ...
    DBG_INFO(("gqavGetGlobalDisAvbTrans Called.\n"));
...
*
* DESCRIPTION:
*        This routine set Disable Queue Scheduler Delays.
*
* INPUTS:
*        en - GT_TRUE, it indicates to the Queue Controller to disable applying Queue
*            Scheduler Delays .
*            GT_FALSE, the frames become available to the scheduler as soon as
               they get enqueued into the packet memory.
*
* OUTPUTS:
*        None.
*
...
)
{
    ...
    DBG_INFO(("gqavSetGlobalDisQSD Called.\n"));
...
)
{
    ...
    DBG_INFO(("gqavGetGlobalDisQSD Called.\n"));
...
)
{
    ...
    DBG_INFO(("gqavSetGlobalDisEnQDelay Called.\n"));
...
)
{
    ...
    DBG_INFO(("gqavGetGlobalDisEnQDelay Called.\n"));
...
)
{
    ...
    opData.ptpPort = hwPort;
    opData.ptpAddr = regOffset;
    opData.ptpBlock = GT_AVB_BLOCK_QAV;
...
)
{
    ...
    DBG_INFO(("gqbvGetReg Called.\n"));

    /* check if device supports this feature */
    if (!IS_IN_DEV_GROUP(dev,DEV_QBV))
    {
        DBG_INFO(("GT_NOT_SUPPORTED\n"));
        return GT_NOT_SUPPORTED;
    }
    ...
    opData.ptpPort = hwPort;
    opData.ptpAddr = regOffset;
    opData.ptpBlock = GT_AVB_BLOCK_QBV;
...
)
{
    ...
    DBG_INFO(("gqbvPortPointDataSet Called.\n"));
    ...
    {
        retVal = gqbvGetReg(dev, port, 2, &tmpData);
           if(retVal != GT_OK)
           {
  ...
  retVal = gqbvSetReg(dev, port, 2, tmpData);
  if(retVal != GT_OK)
  {
     DBG_INFO(("Failed.\n"));
     gtSemGive(dev,dev->tblRegsSem);
...
)
{
    ...
    GT_U32            tmpData;
    int count=0x10;
    DBG_INFO(("gqbvPortPointDataGet Called.\n"));
...
)
{
    GT_STATUS    retVal;         /* Functions return value.      */
	GT_U32       tmpData;
    DBG_INFO(("gqbvSetPortDelay Called.\n"));
  ...
  retVal = gqbvSetReg(dev, port, 0, tmpData);
  if(retVal != GT_OK)
  {
     DBG_INFO(("Failed.\n"));
     gtSemGive(dev,dev->tblRegsSem);
...
)
{
    GT_STATUS    retVal;         /* Functions return value.      */
	GT_U32       tmpData;
    DBG_INFO(("gqbvGetPortDelay Called.\n"));
  ...
  retVal = gqbvGetReg(dev, port, 0, &tmpData);
  if(retVal != GT_OK)
  {
     DBG_INFO(("Failed.\n"));
     gtSemGive(dev,dev->tblRegsSem);
...
*
* DESCRIPTION:
*        Set window time to QBV registers. 
*
* INPUTS:
*        port         - logical port number
*        windowTime -  QBV Port window Time.
*
* OUTPUTS:
*        None.
*
...
)
{
    GT_STATUS    retVal;         /* Functions return value.      */
	GT_U32       tmpData;
    DBG_INFO(("gqbvSetPortWindowTime Called.\n"));
   ...
   retVal = gqbvGetReg(dev, port, 3, &tmpData);
  if(retVal != GT_OK)
  {
     DBG_INFO(("Failed.\n"));
     gtSemGive(dev,dev->tblRegsSem);
  ...
  tmpData &= ~0x7fff;
  tmpData |=  (windowTime&0x7fff);

  retVal = gqbvSetReg(dev, port, 3, tmpData);
  if(retVal != GT_OK)
  {
     DBG_INFO(("Failed.\n"));
     gtSemGive(dev,dev->tblRegsSem);
...
*
* DESCRIPTION:
*        Get window time from QBV registers. 
*
* INPUTS:
*        port         - logical port number
*
...
)
{
    GT_STATUS    retVal;         /* Functions return value.      */
	GT_U32       tmpData;
    DBG_INFO(("gqbvGetPortWindowTime Called.\n"));
  ...
  *windowTime = (GT_U16)(tmpData&0x7fff);
...
)
{
    GT_STATUS    retVal;         /* Functions return value.      */
	GT_U32       tmpData;
    DBG_INFO(("gqbvSetPortGuardBand Called.\n"));
...
)
{
    GT_STATUS    retVal;         /* Functions return value.      */
	GT_U32       tmpData;
    DBG_INFO(("gqbvGetPortGuardBand Called.\n"));
  ...
  *guardBand = (GT_U16)((tmpData&0x8000)?1:0);
