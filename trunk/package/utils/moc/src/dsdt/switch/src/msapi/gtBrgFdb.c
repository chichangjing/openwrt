...
)
{
	...
	if (IS_IN_DEV_GROUP(dev,DEV_ATU_LIMIT_1))
		mask = 0x83FF;
	else
		mask = 0x80FF;
...
)
{
	...
	if (IS_IN_DEV_GROUP(dev,DEV_ATU_LIMIT_1))
		retVal = hwGetPortRegField(dev,hwPort, QD_REG_PORT_ATU_CONTROL, 0, 10, &data);
	else
...
)
{
    ...
    if (!IS_IN_DEV_GROUP(dev,DEV_MAC_AVB) ||
		IS_IN_DEV_GROUP(dev,DEV_88E6999_AVB_FAMILY) ||
		IS_IN_DEV_GROUP(dev,DEV_88E6390_AVB_FAMILY))
    {
        DBG_INFO(("GT_NOT_SUPPORTED\n"));
        return GT_NOT_SUPPORTED;
    }
...
)
{
    ...
    DBG_INFO(("gfdbDelAtuEntry Called.\n"));
    /* check if device supports this feature */
    if (!IS_IN_DEV_GROUP(dev,DEV_STATIC_ADDR))
    {
        DBG_INFO(("GT_NOT_SUPPORTED\n"));
...
*
* DESCRIPTION:
*       Get learn AVB bit. Learn on AVB frames. When this bit is cleared to a zero
...
)
{
    ...
    DBG_INFO(("gfdbGetLearnAVB Called.\n"));
 
    if (!IS_IN_DEV_GROUP(dev,DEV_LEARN_AVB))
    {
        DBG_INFO(("GT_NOT_SUPPORTED\n"));
        return GT_NOT_SUPPORTED;
    }
    ...
    /* Get the Learn AVB.              */
    retVal = hwGetGlobalRegField(dev,QD_REG_ATU_CONTROL,2,1,&data);
    if(retVal != GT_OK)
    {
        DBG_INFO(("Failed.\n"));
        return retVal;
    }
	BIT_2_BOOL(data,*learnAvb);
...
*
* DESCRIPTION:
*       Set learn AVB bit. Learn on AVB frames. When this bit is cleared to a zero
*       AVB flows are not learned from and Source Address refreshed are not done on 
*       AVB frames nor are the Learn2All messages sent for these flow.
*
* INPUTS:
...
)
{
    ...
    DBG_INFO(("gfdbSetLearnAVB Called.\n"));
	...
	BOOL_2_BIT(learnAvb,data);

    /* Set the Learn AVB.              */
    retVal = hwSetGlobalRegField(dev,QD_REG_ATU_CONTROL,2,1,data);
    if(retVal != GT_OK)
    {
        DBG_INFO(("Failed.\n"));
        return retVal;
...
)
{
    ...
    DBG_INFO(("gfdbSetATUTestMode Called.\n"));
 
    if (!IS_IN_DEV_GROUP(dev,DEV_ATU_TEST_MODE))
    {
        DBG_INFO(("GT_NOT_SUPPORTED\n"));
        return GT_NOT_SUPPORTED;
    }
	...
	BOOL_2_BIT(mode,data);
    /* Set the ATU test mode. */
    retVal = hwSetGlobalRegField(dev,QD_REG_ATU_OPERATION,3,1,data);
    if(retVal != GT_OK)
    {
        DBG_INFO(("Failed.\n"));
        return retVal;
...
)
{
    ...
    DBG_INFO(("gfdbGetATUTestMode Called.\n"));
    ...
    /* Get the ATU test mode. */
    retVal = hwGetGlobalRegField(dev,QD_REG_ATU_OPERATION,3,1,&data);
    if(retVal != GT_OK)
    {
        DBG_INFO(("Failed.\n"));
        return retVal;
...
)
{
    ...
    DBG_INFO(("gfdbSetMacFPri Called.\n"));
    ...
    /* Set the MAC frame priority. */
    retVal = hwSetGlobalRegField(dev,QD_REG_ATU_OPERATION,0,3,pri);
    if(retVal != GT_OK)
    {
        DBG_INFO(("Failed.\n"));
        return retVal;
...
)
{
    ...
    DBG_INFO(("gfdbGetMacFPri Called.\n"));
    ...
    /* Get the MAC frame priority. */
    retVal = hwGetGlobalRegField(dev,QD_REG_ATU_OPERATION,0,3,pri);
    if(retVal != GT_OK)
    {
        DBG_INFO(("Failed.\n"));
        return retVal;
                    ...
                    IS_IN_DEV_GROUP(dev,DEV_TRUNK))
                {
                    if (IS_IN_DEV_GROUP(dev,DEV_TRUNK) && entry->trunkMember)
                    {
                      if (IS_IN_DEV_GROUP(dev,DEV_TRUNK_1))
					  {
                        ...
                        data = (GT_U16)( 0x8000 | (((entry->portVec) & 0x1F) << 4) |
                             (((entry->entryState.ucEntryState) & 0xF)) );
					  }
					  else
					  {
      ...
      if(retVal != GT_OK)
      {
        ...
        case FLUSH_UNLOCKED_IN_DB:
                if (entry->entryState.ucEntryState == 0xF)
                {
                  if (IS_IN_DEV_GROUP(dev,DEV_TRUNK_1))
				  {
					  data = (GT_U16)(0xF |((opData->moveFrom & 0x1F) << 4) | ((opData->moveTo & 0x1F) << 9));
				  }
				  else
				  {
