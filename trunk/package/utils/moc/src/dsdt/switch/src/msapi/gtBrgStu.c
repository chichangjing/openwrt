  ...
  {
    {
            ...
            data2 &= 0xffc0;
            data2 |= (entry->portState[10] & 3) << 4;
            data2 |= (entry->portState[9] & 3) << 2;
            data2 |= (entry->portState[8] & 3) << 0;
            data1 |= (entry->portState[7] & 3) << 14;
            data1 |= (entry->portState[6] & 3) << 12;
            data1 |= (entry->portState[5] & 3) << 10;
            data1 |= (entry->portState[4] & 3) << 8;
            data1 |= (entry->portState[3] & 3) << 6;
            data1 |= (entry->portState[2] & 3) << 4;
            data1 |= (entry->portState[1] & 3) << 2;
            data1 |= (entry->portState[0] & 3) << 0;
            nStuData = 2;
    }
  }
...
)
{
  ...
  {
             entry->portState[10]  = (data2 >> 4) & 3 ;
             entry->portState[9]  = (data2 >> 2) & 3 ;
             entry->portState[8]  = (data2 >> 0) & 3 ;
             entry->portState[7]  = (data1 >> 14) & 3 ;
             entry->portState[6]  = (data1 >> 12) & 3 ;
             entry->portState[5]  = (data1 >> 10) & 3 ;
             entry->portState[4]  = (data1 >> 8) & 3 ;
             entry->portState[3]  = (data1 >> 6) & 3 ;
             entry->portState[2]  = (data1 >> 4) & 3 ;
             entry->portState[1]  = (data1 >> 2) & 3 ;
             entry->portState[0]  = (data1 >> 0) & 3 ;
  }
    return GT_OK;
}
