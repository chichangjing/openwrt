...
)
{
    ...
    if(IS_IN_DEV_GROUP(dev,DEV_VTU_EXT_INFO))
    {
       ...
       if(!(IS_IN_DEV_GROUP(dev,DEV_VTU_EXT_INFO_1)))
	   {
...
)
{
    ...
    /* check which Violation occurred */
    retVal = hwGetGlobalRegField(dev,QD_REG_VTU_OPERATION,4,2,&intCause);
    if(retVal != GT_OK)
    {
        DBG_INFO(("ERROR to read VTU OPERATION Register.\n"));
        return retVal;
    ...
    if (IS_IN_DEV_GROUP(dev,DEV_VTU_TBL_FULL))
    {
      retVal = hwGetGlobalRegField(dev,QD_REG_VTU_OPERATION,4,1,&intCause);
      if(retVal != GT_OK)
	  {
        DBG_INFO(("ERROR to read VTU OPERATION Register.\n"));
        return retVal;
      ...
      if(intCause)
	  {
        vtuIntStatus->vtuIntCause |= GT_VTU_TABLE_FULL_VIOLATION;
	  }
    }
...
)
{
    ...
    DBG_INFO(("gvtuGetViolation4 Called.\n"));
    ...
    if(IS_IN_DEV_GROUP(dev,DEV_802_1S_STU_1))
      vtuIntStatus->spid = entry.DBNum & 0x1F;
	else
      vtuIntStatus->spid = entry.DBNum & 0xF;
    vtuIntStatus->vid = entry.vid;
...
)
{
  ...
  if((vtuOp != FLUSH_ALL) && (vtuOp != GET_NEXT_ENTRY) && (vtuOp != SERVICE_VIOLATIONS))
  {
    ...
    else  /* after Opus */
	{
      ...
      /* get data and wirte to QD_REG_VTU_DATA1_REG (ports 0 to 7) */

      data =  (entry->vtuData.memberTagP[0] & 3)     |
              ((entry->vtuData.memberTagP[1] & 3)<<2) | 
              ((entry->vtuData.memberTagP[2] & 3)<<4) | 
              ((entry->vtuData.memberTagP[3] & 3)<<6) | 
              ((entry->vtuData.memberTagP[4] & 3)<<8) | 
              ((entry->vtuData.memberTagP[5] & 3)<<10) | 
              ((entry->vtuData.memberTagP[6] & 3)<<12) | 
              ((entry->vtuData.memberTagP[7] & 3)<<14);
      ...
      /* get data and wirte to QD_REG_VTU_DATA2_REG (ports 8 to 10) */

      data =  (entry->vtuData.memberTagP[8] & 3)   |
              ((entry->vtuData.memberTagP[9] & 3)<<2) | 
              ((entry->vtuData.memberTagP[10] & 3) << 4);
  ...
  if((vtuOp != FLUSH_ALL) && (vtuOp != SERVICE_VIOLATIONS) )
  {
      ...
      if (IS_IN_DEV_GROUP(dev,DEV_VTU_PAGE))
	  {
        retVal = hwReadGlobalReg(dev,(GT_U8)(QD_REG_VTU_VID_REG), &data);
        if(retVal != GT_OK)
        {
          gtSemGive(dev,dev->vtuRegsSem);
          return retVal;
  ...
  if((vtuOp == LOAD_PURGE_ENTRY) && (*valid == 1))
  {
      if(IS_IN_DEV_GROUP(dev,DEV_802_1S_STU))
      {
        data = entry->filter & 0x07;    
        data= ((entry->sid) | (data << 12)) & 0x703F; 
  ...
  */
  if (IS_IN_DEV_GROUP(dev,DEV_VTU_CONTROL))
  {
      retVal = hwReadGlobalReg(dev,(GT_U8)(QD_REG_VTU_OPERATION), &data);
      if(retVal != GT_OK)
      {
        gtSemGive(dev,dev->vtuRegsSem);
        return retVal;
  ...
  */
  ...
  if(vtuOp == SERVICE_VIOLATIONS)
  {
      ...
      if(IS_IN_DEV_GROUP(dev,DEV_802_1S_STU_1))
	  {
        retVal = hwGetGlobalRegField(dev,QD_REG_VTU_OPERATION,0,5,&data);
        entry->DBNum = (GT_U8)(data & 0x1F);
	  }
	  else
	  {
  ...
  if(vtuOp == GET_NEXT_ENTRY)
  {
    ...
    *valid   = (data >> 12) & 1;
    ...
    else  /* after Opus */
    {
      ...
      /* get data from data register for ports 0 to 2 */
      entry->vtuData.memberTagP[0]  =  data & 3 ;
      entry->vtuData.portStateP[0]  =  0; /* data & 3 ; */
      entry->vtuData.memberTagP[1]  = (data >> 2) & 3 ;
      entry->vtuData.portStateP[1]  = 0; /* (data >> 2) & 3 ; */
      entry->vtuData.memberTagP[2]  = (data >> 4) & 3 ;
      entry->vtuData.portStateP[2]  = 0; /* (data >> 4) & 3 ; */
      entry->vtuData.memberTagP[3]  = (data >> 6) & 3 ;
      entry->vtuData.portStateP[3]  = 0; /* (data >> 6) & 3 ; */
      entry->vtuData.memberTagP[4]  = (data >> 8) & 3 ;
      entry->vtuData.portStateP[4]  = 0; /* (data >> 8) & 3 ;*/
      entry->vtuData.memberTagP[5]  = (data >> 10) & 3 ;
      entry->vtuData.portStateP[5]  = 0; /* (data >> 10) & 3 ; */
      entry->vtuData.memberTagP[6]  = (data >> 12) & 3 ;
      entry->vtuData.portStateP[6]  = 0; /* (data >> 12) & 3 ; */
      entry->vtuData.memberTagP[7]  = (data >> 14) & 3 ;
      entry->vtuData.portStateP[7]  = 0; /* (data >> 14) & 3 ; */
      ...
      entry->vtuData.memberTagP[8]  =  data & 3 ;
      entry->vtuData.portStateP[8]  =  0; /* data & 3 ; */
      entry->vtuData.memberTagP[9]  = (data >> 2) & 3 ;
      entry->vtuData.portStateP[9]  = 0; /* (data >> 2) & 3 ; */
      entry->vtuData.memberTagP[10]  = (data >> 4) & 3 ;
      entry->vtuData.portStateP[10]  = 0; /* (data >> 4) & 3 ; */
...
/* VTU Control functions after Opus */
...
)
{
    ...
    DBG_INFO(("gvtuSetVTUControl Called.\n"));
    ...
    /* Set the Vtu control bits.                */
    retVal = hwSetGlobalRegField(dev,QD_REG_STU_SID_REG,12,4,vtuCtrl);
    if(retVal != GT_OK)
    {
        DBG_INFO(("Failed.\n"));
        return retVal;
...
)
{
    ...
    DBG_INFO(("gvtuGetVTUControl Called.\n"));
    ...
    /* Set the Vtu control bits.                */
    retVal = hwGetGlobalRegField(dev,QD_REG_STU_SID_REG,12,4, vtuCtrl);
    if(retVal != GT_OK)
    {
        DBG_INFO(("Failed.\n"));
        return retVal;
...
)
{
    ...
    DBG_INFO(("gvtuSetVTUMode Called.\n"));
    ...
    /* Set the Vtu mode bits.                */
    retVal = hwSetGlobalRegField(dev,QD_REG_VTU_OPERATION,10,2, mode&3);
    if(retVal != GT_OK)
    {
        DBG_INFO(("Failed.\n"));
        return retVal;
...
)
{
    ...
    DBG_INFO(("gvtuGetVTUMode Called.\n"));
    ...
    /* Get the Vtu mode bits.                */
    retVal = hwGetGlobalRegField(dev,QD_REG_VTU_OPERATION,10,2, mode);
    if(retVal != GT_OK)
    {
        DBG_INFO(("Failed.\n"));
        return retVal;
...
)
{
    ...
    DBG_INFO(("gatuGetATUbin Called.\n"));

    if (!IS_IN_DEV_GROUP(dev,DEV_ATU_BIN))
    {
        DBG_INFO(("GT_NOT_SUPPORTED\n"));
        return GT_NOT_SUPPORTED;
    }
    ...
    /* Get the Vtu mode bits.                */
    if ( IS_IN_DEV_GROUP(dev,DEV_ATU_BIN_2))
      retVal = hwGetGlobalRegField(dev,QD_REG_ATU_CONTROL,13,2, atuBin);
	else
      retVal = hwGetGlobalRegField(dev,QD_REG_VTU_OPERATION,14,2, atuBin);
    if(retVal != GT_OK)
    {
        DBG_INFO(("Failed.\n"));
        return retVal;
...
)
{
    ...
    DBG_INFO(("gvtuSetVTUPage Called.\n"));
    ...
    /* Set the Vtu mode bits.                */
    retVal = hwSetGlobalRegField(dev,QD_REG_VTU_OPERATION,13,1, page&1);
    if(retVal != GT_OK)
    {
        DBG_INFO(("Failed.\n"));
        return retVal;
...
)
{
    ...
    DBG_INFO(("gvtuGetVTUPage Called.\n"));
    ...
    /* Get the Vtu page bits.                */
    retVal = hwGetGlobalRegField(dev,QD_REG_VTU_OPERATION,13,1, page);
    if(retVal != GT_OK)
    {
        DBG_INFO(("Failed.\n"));
        return retVal;
