...
)
{
        ...
        (!(IS_IN_DEV_GROUP(dev,DEV_DEV_PHY_INTERRUPT))))
    {
      ...
      if (IS_IN_DEV_GROUP(dev,DEV_TCAM_INTERRUPT)) 
      {
        intMask = GT_INT_MASK;
      }
    }
...
)
{
    ...
    {
      if (IS_IN_DEV_GROUP(dev,DEV_VTU_TBL_FULL_2))
        retVal = gvtuGetViolation4(dev,vtuIntStatus);
	  else
...
)
{
    ...
    devIntStatus->phyInt = 0;
    devIntStatus->serInt = 0;
...
/*    devIntStatus->port = GT_PORT_2_LPORT((GT_U8)hwPort); */
    ...
    if (IS_IN_DEV_GROUP(dev,DEV_DEVICE_INT_TYPE1))
    {
        ...
        if (data & (0x7 << 8))
        {
            ...
            devIntStatus->serInt = GT_PORTVEC_2_LPORTVEC((data & (7<<8)));
        }
    }
    else  if (IS_IN_DEV_GROUP(dev,DEV_DEVICE_INT_TYPE2))  /* DEV_DEVICE_INT_TYPE2 */
    {
        ...
        {
            devIntStatus->devIntCause |= GT_DEV_INT_SERDES_LINK;
            devIntStatus->serInt = GT_SERVEC_2_LSERVEC((data & (0x3 << 11)) >> 7);
        }
    ...
    else  if (IS_IN_DEV_GROUP(dev,DEV_DEVICE_INT_TYPE3))  /* DEV_DEVICE_INT_TYPE3 */
    {
        if (data & 0x1F)
        {
            devIntStatus->devIntCause |= GT_DEV_INT_PHY;
    ...
    else  if (IS_IN_DEV_GROUP(dev,DEV_DEVICE_INT_TYPE4))  /* DEV_DEVICE_INT_TYPE4 */
    {
		/* chexk serdes */
        if (data & (0x1 << 11))
        {
            devIntStatus->devIntCause |= GT_DEV_INT_SERDES_LINK;
            devIntStatus->serInt = GT_SERVEC_2_LSERVEC((data & (0x1 << 11)) >> 7);
        }
    ...
    else  if (IS_IN_DEV_GROUP(dev,DEV_DEVICE_INT_TYPE5))  /* DEV_DEVICE_INT_TYPE5 */
#if 1
    {
        if (data & (0x3) )
        {
            devIntStatus->devIntCause |= GT_DEV_INT_SERDES_LINK;
/*            devIntStatus->serInt = GT_SERVEC_2_LSERVEC((data & (0x3))); */
            devIntStatus->serInt = data & (0x3);
        ...
        {
            devIntStatus->devIntCause |= GT_DEV_INT_PHY;
            devIntStatus->phyInt = GT_PORTVEC_2_LPORTVEC((data & (0x3)<<3));
        }
    }
#else
    {
        ...
        {
            devIntStatus->devIntCause |= GT_DEV_INT_SERDES_LINK;
            devIntStatus->serInt = GT_SERVEC_2_LSERVEC((data & (0x3 << 12)) >> 12);
        ...
        {
            devIntStatus->devIntCause |= GT_DEV_INT_PHY;
            devIntStatus->phyInt = GT_PORTVEC_2_LPORTVEC((data & 0x1b));
        }
    }
#endif
   else /* DEV_DEVICE_INT_TYPE6 */
    {
        if (data & 0x7FF)
        {
            devIntStatus->devIntCause |= GT_DEV_INT_PHY;
            devIntStatus->phyInt = GT_PORTVEC_2_LPORTVEC((data & 0x7FF));
        }
    }
...
)
{
    ...
    {
        serdesMask = mask = 1 << 11;    /* SERDES Port List */
        mask |= 0x1F;    /* Phy list */
        phyMask = 0x1F;
    }
    else  if (IS_IN_DEV_GROUP(dev,DEV_DEVICE_INT_TYPE5))  /* DEV_DEVICE_INT_TYPE5 */
...
/*        phyMask = mask = 0x18; */
        phyMask = mask = 0x1b;
        mask |= 0x3;    /* Phy list */
        serdesMask = 0x3;    /* SERDES Port List */
    }
#else
    {
        phyMask = mask = 0x1B;
        mask |= 0x1B;    /* Phy list */
        serdesMask = 0x3000;    /* SERDES Port List */
    }
#endif
   else /* DEV_DEVICE_INT_TYPE6 */
    {
    ...
    data = 0;
     ...
     if (event & GT_DEV_INT_SERDES_LINK)
     {
        ...
        if ((IS_IN_DEV_GROUP(dev,DEV_DEVICE_INT_TYPE2)) ||
            (IS_IN_DEV_GROUP(dev,DEV_DEVICE_INT_TYPE4))) 
        {
            pList = GT_LSERVEC_2_SERVEC(devInt->serPortList);
            if ((GT_U16)pList & (~serdesMask))
            {
                DBG_INFO(("GT_BAD_PARAM serPortList\n"));
                return GT_BAD_PARAM;
            }
            data = (GT_U16)pList;
        }
        else if (IS_IN_DEV_GROUP(dev,DEV_DEVICE_INT_TYPE5)) 
        {
            pList = GT_LSERVEC_2_SERVEC(devInt->serPortList);
            if ((GT_U16)pList & (~serdesMask))
            {
        ...
        {
            pList = GT_LPORTVEC_2_PORTVEC(devInt->serPortList);
            pList <<= 7;
            if ((GT_U16)pList & (~serdesMask))
            {
                DBG_INFO(("GT_BAD_PARAM serPortList\n"));
...
)
{
	...
	else
    {
		GT_U8 tmpData;
      /* Set the related bit. */
      retVal = gdevGetWatchDogCtrl(dev,0x13,&tmpData);
      if(retVal != GT_OK)
      {
        return GT_FAIL;
      }
	  ...
	  if(data==1)
	    tmpData |= 0x1;
	  tmpData = (GT_U8)(data &0xff);
      /* Get the related bit. */
      retVal = gdevSetWatchDogCtrl(dev,0x13,tmpData);
    }
...
)
{
    ...
    DBG_INFO(("gwdGetSWResetOnWD Called.\n"));
...
)
{
	...
	else
    {
	  GT_U8 tmpData;
      /* Set the related bit. */
      retVal = gdevGetWatchDogCtrl(dev,0x40,&tmpData);
      if(retVal != GT_OK)
      {
        return GT_FAIL;
      }
	  ...
	  if(data==1)
	    tmpData |= 0x80;
      retVal = gdevSetWatchDogCtrl(dev,0x40,tmpData);
    }
