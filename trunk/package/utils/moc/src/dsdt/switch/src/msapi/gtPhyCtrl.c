...
)
{
    ...
    else if(phyInfo->flag & GT_PHY_FIBER)
    {
        ...
        /* Read to Phy Control Register.  */
        if(hwReadPagedPhyReg(dev,hwPort,1,QD_PHY_CONTROL_REG,phyInfo->anyPage,&u16Data) != GT_OK)
            return GT_FAIL;
        u16Data |= QD_PHY_AUTONEGO;
...
/*#ifdef GT_USE_MAD */
    ...
    DBG_INFO(("gprtSetDTEDetect Called.\n"));
...
/*#ifdef GT_USE_MAD */
    ...
    DBG_INFO(("gprtSetDTEDetectDropWait Called.\n"));
...
/*#ifdef GT_USE_MAD */
    ...
    DBG_INFO(("gprtGetDTEDetectDropWait Called.\n"));
...
/*#ifdef GT_USE_MAD */
    ...
    if (en)
    {
#if 0  /* to avoid using Sema duplicated */
...
#else
      if((retVal=hwGetPhyRegField(dev,hwPort,17,10,1,&data)) != GT_OK)
	  {
        gtSemGive(dev,dev->phyRegsSem);
        return retVal;
	  }
      BIT_2_BOOL(data,link);
#endif
...
)
{
    ...
    DBG_INFO(("gprtGet45PhyReg Called.\n"));
...
/*    hwPort = GT_LPORT_2_PHY(port); */
    ...
    /* Get Phy Register. */
    if(phyXRegReadPPUEn(dev, hwPort, device, regAddr,&u16Data) != GT_OK)
    {
        DBG_INFO(("Failed.\n"));
        gtSemGive(dev,dev->phyRegsSem);
        return GT_FAIL;
...
)
{
    ...
    DBG_INFO(("gprtSet45PhyReg Called.\n"));
...
/*    hwPort = GT_LPORT_2_PHY(port); */
    ...
    /* Write to Phy Register */
    if(phyXRegWritePPUEn(dev,hwPort, device, regAddr,data) != GT_OK)
    {
        DBG_INFO(("Failed.\n"));
        gtSemGive(dev,dev->phyRegsSem);
        return GT_FAIL;
