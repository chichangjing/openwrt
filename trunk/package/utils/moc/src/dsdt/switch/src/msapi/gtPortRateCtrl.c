...
static GT_STATUS setEnhancedERate_1(GT_QD_DEV *dev, GT_LPORT port, GT_ERATE_TYPE *rateType)
{
    GT_STATUS    retVal;         /* Functions return value.      */
    GT_U16        data;
    GT_U32        rate, eDec;
    ...
    if (mode == GT_PIRL_ELIMIT_FRAME)    
    {
        ...
        else if((rate < 7600)  || (rate > 14880000))
        {
            return GT_BAD_PARAM;
        }
        else
    ...
    retVal = hwSetPortRegField(dev,phyPort,QD_REG_EGRESS_RATE_CTRL,0,14,(GT_U16)data );
    if(retVal != GT_OK)
       {
        DBG_INFO(("Failed.\n"));
           return retVal;
...
static GT_STATUS getEnhancedERate_1(GT_QD_DEV *dev, GT_LPORT port, GT_ERATE_TYPE *rateType)
{
    GT_STATUS    retVal;         /* Functions return value.      */
    GT_U16        rate, eDec;
    GT_PIRL_ELIMIT_MODE        mode;
    ...
    retVal = hwGetPortRegField(dev,phyPort,QD_REG_EGRESS_RATE_CTRL,0,14,&rate );
    if(retVal != GT_OK)
       {
        DBG_INFO(("Failed.\n"));
           return retVal;
...
)
{
    ...
    else if (IS_IN_DEV_GROUP(dev,DEV_ELIMIT_FRAME_BASED_1))
    {
        return setEnhancedERate_1(dev,port,rateType);
    }
...
)
{
    ...
    {
        return getEnhancedERate_1(dev,port,rateType);
    }
