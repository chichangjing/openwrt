...
)
{
    ...
    DBG_INFO(("gstatsGetPortCounters4 Called.\n"));
...
)
{
...
#endif
	if(dev->deviceId == 0x390 || dev->deviceId == 0x290 || dev->deviceId == 0x0a1 || dev->deviceId == 0x190 || dev->deviceId == 0x0a0)
		histoData = 0;
	else
		histoData &= 0xC00;
    ...
    switch (statsOp)
    {
        case STATS_FLUSH_ALL:
			...
			if (IS_IN_DEV_GROUP(dev,DEV_RMON_TYPE_5))
			{
              data = (1 << 15) | (GT_STATS_FLUSH_ALL << 12) | portNum | histoData | (1<<10);
              retVal = hwWriteGlobalReg(dev,QD_REG_STATS_OPERATION,data);
			}

			else if (IS_IN_DEV_GROUP(dev,DEV_RMON_TYPE_4))
			{
              data = (1 << 15) | (GT_STATS_FLUSH_ALL << 12) | portNum | histoData | (1<<9);
              retVal = hwWriteGlobalReg(dev,QD_REG_STATS_OPERATION,data);
			}
        ...
        case STATS_FLUSH_PORT:
            ...
            retVal = hwWriteGlobalReg(dev,QD_REG_STATS_OPERATION,data);
         	...
         	if (IS_IN_DEV_GROUP(dev,DEV_RMON_TYPE_5))
			{
              data = (1 << 15) | (GT_STATS_FLUSH_PORT << 12) | portNum | histoData | (1<<10);
              retVal = hwWriteGlobalReg(dev,QD_REG_STATS_OPERATION,data);
			}

			else if (IS_IN_DEV_GROUP(dev,DEV_RMON_TYPE_4))
			{
              data = (1 << 15) | (GT_STATS_FLUSH_PORT << 12) | portNum | histoData | (1<<9);
              retVal = hwWriteGlobalReg(dev,QD_REG_STATS_OPERATION,data);
			}
        ...
        case STATS_READ_COUNTER:
			bank = (counter & GT_TYPE4_BANK1)?1:0;
            retVal = statsCapture(dev,port, bank);
            if(retVal != GT_OK)
            {
                gtSemGive(dev,dev->statsRegsSem);
                return retVal;
        ...
        case STATS_READ_ALL:
		{
          ...
          for(bank=0; bank<bankSize; bank++)
		  {
            ...
            lastCounter = (bank==0)?(GT_U32)STATS4_Late : (GT_U32)STATS4_OutMGMT;
            startCounter = (bank==0)?(GT_U32)STATS4_InGoodOctetsLo : (GT_U32)STATS4_InDiscards;

			if(bank==1)
              statsData = (unsigned int *)statsData + STATS4_Late +1;
            for(tmpCounter=startCounter; tmpCounter<=lastCounter; tmpCounter++)
			{
              retVal = statsReadCounter(dev,tmpCounter,((GT_U32*)statsData+tmpCounter-startCounter));
              if(retVal != GT_OK)
			  {
                gtSemGive(dev,dev->statsRegsSem);
                return retVal;
...
)
{
	...
	if (IS_IN_DEV_GROUP(dev,DEV_RMON_TYPE_5))
      data = (1 << 15) | (GT_STATS_CAPTURE_PORT << 12) | portNum | histoData | (bank<<10);
    else if (IS_IN_DEV_GROUP(dev,DEV_RMON_TYPE_4))
      data = (1 << 15) | (GT_STATS_CAPTURE_PORT << 12) | portNum | histoData | (bank<<9);
	else
...
#endif
    data = (GT_U16)((1 << 15) | (GT_STATS_READ_COUNTER << 12) | (counter&0x7f) | histoData);
	if (counter & 0x80)
	{
