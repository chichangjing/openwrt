...
*
* DESCRIPTION:
*       This routine retrieves Px_MODE and analyze it.
*
* INPUTS:
...
)
{
	...
	/* Peridot does not support this feature. Opus may not either. */
...
)
{
    ...
    DBG_INFO(("gprtGetTxPauseEn Called.\n"));

    /* Only Peridot supports this. */
    if (!IS_IN_DEV_GROUP(dev,DEV_88E6390_FAMILY))
    {
        DBG_INFO(("GT_NOT_SUPPORTED\n"));
        return GT_NOT_SUPPORTED;
...
)
{
    ...
    DBG_INFO(("gprtGetRxPauseEn Called.\n"));
...
)
{
    ...
    /* Only Gigabit Switch supports this status. */
    if (!IS_IN_DEV_GROUP(dev,DEV_GIGABIT_SWITCH) || IS_IN_DEV_GROUP(dev,DEV_NO_HDFLOW_BIT))
    {
        DBG_INFO(("GT_NOT_SUPPORTED\n"));
        return GT_NOT_SUPPORTED;
    }
...
)
{
    ...
    DBG_INFO(("gprtGetDuplexFixed Called.\n"));

	/* Only Opus and Peridot support this feature */
    if (!IS_IN_DEV_GROUP(dev,DEV_DUPLEX_FIXED))
    {
        DBG_INFO(("GT_NOT_SUPPORTED\n"));
        return GT_NOT_SUPPORTED;
    }
...
)
{
	...
	/* Only Sapphire (88E6181 and 88E6183) support this feature */
    if (!IS_IN_DEV_GROUP(dev,DEV_SAPPHIRE_SWITCH))
    {
        DBG_INFO(("GT_NOT_SUPPORTED\n"));
        return GT_NOT_SUPPORTED;
    }
...
)
{
    ...
    if (IS_IN_DEV_GROUP(dev,DEV_Q_COUNTER_TABLE))
    {
	  ...
	  {
		GT_U16 data;
		if (!IS_IN_DEV_GROUP(dev,DEV_DEF_QPRI_1) && que > 3)
			data = (que-4) << 12;
		else
			data = que << 12;
		if((retVal = hwWritePortReg(dev,hwPort, QD_REG_Q_COUNTER, data)) != GT_OK)
          return retVal;
	  }
      else
	  { 
	    DBG_INFO(("Bad queue number.\n"));
        return GT_BAD_PARAM;
	  }
...
*        For Opus and Peridot, this routine gets size for each Qpri (0 ~ 7).
*
* INPUTS:
*        port - the logical port number
*
* OUTPUTS:
*        counts - egress queue size per QPri (should be 4 * 16 bytes, 8 * 16 bytes
...
)
{
	...
	{
		if((retVal = hwGetPortRegField(dev,hwPort, QD_REG_Q_COUNTER,0,9,&counts[4])) != GT_OK)
        return retVal;

		if((retVal = hwGetPortRegField(dev,hwPort, QD_REG_Q_COUNTER,0,9,&counts[5])) != GT_OK)
        return retVal;

		if((retVal = hwGetPortRegField(dev,hwPort, QD_REG_Q_COUNTER,0,9,&counts[6])) != GT_OK)
        return retVal;

		if((retVal = hwGetPortRegField(dev,hwPort, QD_REG_Q_COUNTER,0,9,&counts[7])) != GT_OK)
        return retVal;
	}
...
)
{
    ...
    {
        retVal = hwGetPortRegField(dev,hwPort, QD_REG_PORT_STATUS,0,4,&data);
    }
	else
	{
