...
)
{
    ...
    if (IS_IN_DEV_GROUP(dev,DEV_FRAME_PRIORITY_OVERRIDE_TABLE_1))
      data = (GT_U16)((1 << 15) | (fType << 8) | fPri | 0x4000);
    else
...
)
{
    ...
    if (entry->qPriEn)
	{
      if (IS_IN_DEV_GROUP(dev,DEV_FRAME_PRIORITY_OVERRIDE_TABLE_1))
	  {
        qPri = (GT_U16)((1 << 7) | ((entry->qPriority & 0x7) << 4));
	  }
	  else
	  {
...
)
{
    ...
    if (IS_IN_DEV_GROUP(dev,DEV_FRAME_PRIORITY_OVERRIDE_TABLE_1))
	{
	  ...
	  {
        entry->qPriEn = GT_TRUE;
        entry->qPriority = (data >> 4) & 0x7;
	  }
      else
	  {
        entry->qPriEn = GT_FALSE;
        entry->qPriority = (data >> 4) & 0x7; /* no meaning, but just in case */
	  }
    }
	else
	{
