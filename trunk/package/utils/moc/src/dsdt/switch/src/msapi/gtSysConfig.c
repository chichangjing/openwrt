...
/*        case MAD_88E3019: return ("MAD_88E3019 "); */
        ...
        case MAD_88E151x:  return ("MAD_88E151x ");
...
)
{
    ...
    switch(dev->deviceId)
    {
		...
		case GT_88E6390:  /* Peridot Full Featured Device */
				...
				dev->maxPhyNum = 16;
                dev->validPortVec = (1 << dev->numOfPorts) - 1;
                dev->validPhyVec = (1 << dev->numOfPorts) - 1;
                dev->validSerdesVec = 0x0000;
                dev->devName1 = DEV_88E6390;
				break;
		case GT_88E6390X:  /* Peridot Full Featured Device */
				dev->numOfPorts = 11;
				dev->maxPorts = 11;
				dev->maxPhyNum = 16;
                dev->validPortVec = (1 << dev->numOfPorts) - 1;
		...
		case GT_88E6190:  /* Peridot  No AVB, Z80 NIC, TCAM nor Cut Through */
                ...
                dev->devName1 = DEV_88E6190;
				break;
		case GT_88E6190X:  /* Peridot  No AVB, Z80 NIC, TCAM nor Cut Through */
				dev->numOfPorts = 11;
				dev->maxPorts = 11;
				dev->maxPhyNum = 16;
				dev->validPortVec = (1 << dev->numOfPorts) - 1;
    ...
    if((retVal = gtSemCreate(dev, &dev->multiAddrSem, GT_SEM_FULL)) != GT_OK)
    {
    ...
    if((retVal = gtSemCreate(dev, &dev->atuRegsSem, GT_SEM_FULL)) != GT_OK)
    {
        DBG_INFO(("semCreate Failed.\n"));
        qdUnloadDriver(dev);
        return GT_FAIL;
    ...
    if((retVal = gtSemCreate(dev, &dev->vtuRegsSem, GT_SEM_FULL)) != GT_OK)
    {
        DBG_INFO(("semCreate Failed.\n"));
        qdUnloadDriver(dev);
        return GT_FAIL;
    ...
    if((retVal = gtSemCreate(dev, &dev->statsRegsSem, GT_SEM_FULL)) != GT_OK)
    {
        DBG_INFO(("semCreate Failed.\n"));
        qdUnloadDriver(dev);
        return GT_FAIL;
    ...
    if((retVal = gtSemCreate(dev, &dev->pirlRegsSem, GT_SEM_FULL)) != GT_OK)
    {
        DBG_INFO(("semCreate Failed.\n"));
        qdUnloadDriver(dev);
        return GT_FAIL;
    ...
    if((retVal = gtSemCreate(dev, &dev->ptpRegsSem, GT_SEM_FULL)) != GT_OK)
    {
        DBG_INFO(("semCreate Failed.\n"));
        qdUnloadDriver(dev);
        return GT_FAIL;
    ...
    if((retVal = gtSemCreate(dev, &dev->tblRegsSem, GT_SEM_FULL)) != GT_OK)
    {
        DBG_INFO(("semCreate Failed.\n"));
        qdUnloadDriver(dev);
        return GT_FAIL;
    ...
    if((retVal = gtSemCreate(dev, &dev->eepromRegsSem, GT_SEM_FULL)) != GT_OK)
    {
        DBG_INFO(("semCreate Failed.\n"));
        qdUnloadDriver(dev);
        return GT_FAIL;
    ...
    if((retVal = gtSemCreate(dev, &dev->phyRegsSem, GT_SEM_FULL)) != GT_OK)
    {
        DBG_INFO(("semCreate Failed.\n"));
        qdUnloadDriver(dev);
        return GT_FAIL;
    ...
    if((retVal = gtSemCreate(dev, &dev->hwAccessRegsSem, GT_SEM_FULL)) != GT_OK)
    {
        DBG_INFO(("semCreate Failed.\n"));
        qdUnloadDriver(dev);
        return GT_FAIL;
...
#if 0 /* Port initializations is done by the customer */
...
static GT_BOOL gtRegister(GT_QD_DEV *dev, BSP_FUNCTIONS* pBSPFunctions)
{
    ...
    dev->rmuTxRx = pBSPFunctions->rmuTxRxPacket;
    dev->smiReadMii  = pBSPFunctions->rmuReadMii;
    dev->smiWriteMii = pBSPFunctions->rmuWriteMii;
#endif
...
GT_U32 qdLonglong2Long(unsigned long long data)
{
