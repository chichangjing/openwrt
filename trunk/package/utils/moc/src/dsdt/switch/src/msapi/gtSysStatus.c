...
*
* DESCRIPTION:
*       This routine get the Switch mode. These two bits return 
...
)
{
    ...
    if (!(IS_IN_DEV_GROUP(dev,DEV_SWITCH_MODE)) || 
		IS_IN_DEV_GROUP(dev,DEV_NO_SW_MODE))
    {
        DBG_INFO(("Not Supported.\n"));
        return GT_NOT_SUPPORTED;
    }
...
)
{
    ...
    if (IS_IN_DEV_GROUP(dev,DEV_OUT_Q_SIZE_2))
		/* get the counter */
    {
      retVal = hwGetGlobalRegField(dev,QD_REG_TOTAL_FREE_COUNTER,0,11,count);
   	}
    else if (IS_IN_DEV_GROUP(dev,DEV_OUT_Q_SIZE_1))
		/* get the counter */
    {
      retVal = hwGetGlobalRegField(dev,QD_REG_TOTAL_FREE_COUNTER,0,10,count);
   	}
	else
	{
