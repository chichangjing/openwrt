...
/*        	case 0x2600:    */    /* Peridot */
/*		case 0x1900:    */    /* Peridot */
/*		case 0xa10:	*/    /* Peridot 88E6390X*/
/*		case 0xa00:	*/    /*Peridot 88e6190X*/
...
static GT_BOOL fgtReadMii(GT_QD_DEV*dev, unsigned int phyAddr, unsigned int reg, unsigned int* value)
{
  GT_BOOL retVal;
#ifdef GT_RMGMT_ACCESS
  if((dev->use_smi==GT_TRUE)&&(dev->smiReadMii!=NULL))
  {
    retVal =  dev->smiReadMii(dev, phyAddr, reg, value);
  }
  else
#endif
...
static GT_BOOL fgtWriteMii(GT_QD_DEV*dev, unsigned int phyAddr, unsigned int reg, unsigned int value)
{
  GT_BOOL retVal;
#ifdef GT_RMGMT_ACCESS
  if((dev->use_smi==GT_TRUE)&&(dev->smiWriteMii!=NULL))
  {
    retVal = dev->smiWriteMii(dev, phyAddr, reg, value);
  }
  else
#endif
...
static GT_BOOL fgtAccessRegs(GT_QD_DEV* dev, HW_DEV_REG_ACCESS *regList)
{
  ...
  for (i=0; i<regList->entries; i++)
  {
    ...
    {
      case HW_REG_READ:
        retVal=fgtReadMii(dev, regList->rw_reg_list[i].addr,
                regList->rw_reg_list[i].reg, (unsigned int *)&(regList->rw_reg_list[i].data));
        break;
      case HW_REG_WRITE:
        retVal=fgtWriteMii(dev, regList->rw_reg_list[i].addr,
