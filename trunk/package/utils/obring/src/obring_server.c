...
static const struct blobmsg_policy port_status_policy[] = {
	[PORT_STATUS_NO] = { .name = "port_no", .type = BLOBMSG_TYPE_INT32 },
	[PORT_STATUS_MAN] = { .name = "port_manage", .type = BLOBMSG_TYPE_BOOL },
	[PORT_STATUS_OPT] = { .name = "port_operate", .type = BLOBMSG_TYPE_BOOL },
	[PORT_STATUS_CONN] = { .name = "port_connection", .type = BLOBMSG_TYPE_BOOL },
	[PORT_STATUS_SPD] = { .name = "port_speed", .type = BLOBMSG_TYPE_INT32 },
	[PORT_STATUS_DPL] = { .name = "port_duple", .type = BLOBMSG_TYPE_STRING },
	[PORT_STATUS_FC] = { .name = "port_flow_ctrl", .type = BLOBMSG_TYPE_BOOL },
};

struct port_status_get_request {
...
{
	struct port_status_get_request *req = container_of(t, struct port_status_get_request, timeout);
	char *data;
...
static int port_status_get(struct ubus_context *ctx, struct ubus_object *obj,
		      struct ubus_request_data *req, const char *method,
		      struct blob_attr *msg)
{
	struct port_status_get_request *hreq;
	...
	//blobmsg_parse(port_status_policy, ARRAY_SIZE(port_status_policy), tb, blob_data(msg), blob_len(msg));
	...
	ubus_defer_request(ctx, req, req);

	blob_buf_init(&b, 0);
	blobmsg_add_u8(&b, "port_status_manage", 1);
    blobmsg_add_u32(&b, "port_status_speed", 1000);
...
static const struct ubus_method test_methods[] = {
	UBUS_METHOD("port_status_get", port_status_get, port_status_policy),
